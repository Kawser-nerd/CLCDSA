...
namespace Program {
    class MainClass {
        ...
        void Solve() {
            io.i(out n);
            io.ini(out a, n);
            AssociativeArray<long> aa = new AssociativeArray<long>(a);
            long[] enable1 = aa.dic.Keys.Where(i => aa.dic[i] >= 4).ToArray();
            long[] enable2 = aa.dic.Keys.Where(i => aa.dic[i] >= 2).ToArray();
            enable1.Sort();
            Array.Reverse(enable1);
            enable2.Sort();
            Array.Reverse(enable2);
            long S1 = 0, S2 = 0;
            if (enable1.Length > 0)
                S1 = enable1[0] * enable1[0];
            if (enable2.Length > 1)
                S2 = enable2[0] * enable2[1];
