...
namespace Program {
    ...
    class Dijkstra : ShortestPath {//?????????,????
        ...
        public long MinCost(int t) {
            return cost[t];
        }
        public void Run(int f) {
            cost = new long[I];
            for (int i = 0; i < I; ++i) cost[i] = INF;
            cost[f] = 0;
            var pq = new PQueue<edge>(); pq.Enqueue(new edge(f, 0L));//(from,cost)
            while (!pq.Empty()) {
                var cur = pq.Dequeue();
                //if (cost[cur.Item1] != cur.Item2) continue;
                if (cost[cur.to] < cur.cost) continue;
                for (int i = 0; i < Adj[cur.to].Count; ++i) {
                    var tmp = Adj[cur.to][i];
                    if (cost[tmp.Item1] > cur.cost + tmp.Item2) {
                        cost[tmp.Item1] = cur.cost + tmp.Item2;
                        pq.Enqueue(new edge(tmp.Item1, cost[tmp.Item1]));
                    }
                }
            }
        }
