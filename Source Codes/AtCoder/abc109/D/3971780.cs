...
public class Hello
{
    ...
    public static void Main()
    {
        int[] HW = IntLine();
        int H = HW[0], W = HW[1];
        List<int>[] grid = new List<int>[H];
        for (int i = 0; i < H; i++) { grid[i] = IntLine().Select(v => v %= 2).ToList(); }
        int j = 0;
        bool flag = false;
        int[] memo = new int[2];
        ...
        for (int i = 0; i < H; i++)
        {
            for (j = (i % 2 == 0 ? 0 : W - 1); j != (i % 2 == 0 ? W : -1); j += (i % 2 == 0 ? +1 : -1))
            {
                if (flag) { ans.Add(new int[4] { memo[0] + 1, memo[1] + 1, i + 1, j + 1 }); }
                if (!flag && grid[i][j] == 1) { flag = true; }
                else if (flag && grid[i][j] == 1) { flag = false; }
                memo = new int[2] { i, j };
            }
        }
