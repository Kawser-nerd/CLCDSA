...
main(n){
	...
	f(i,n)f(j,m)if(p[(o=i*m+j)+1]);else if(a)x(o),x(o+m);else y(o),y(o+1);
	//puts(a|b?"NO":"YES");if(1-a-b)for(i=0;i<n*m;++i%m||puts(""))putchar(p[i]?:46);
	if(a|b)puts("NO");
	else{
		puts("YES");
		for(i=0;i<n;puts(""),i++)
			for(j=0;j<m;)putchar(p[i*m+j++]?:46);
	}
...
./Main.c:3:18: warning: type defaults to ‘int’ in declaration of ‘j’ [-Wimplicit-int]
 p[2<<19],a,b,m,i,j,o;
                  ^
./Main.c:3:20: warning: type defaults to ‘int’ in declaration of ‘o’ [-Wimplicit-int]
 p[2<<19],a,b,m,i,j,o;
                    ^
