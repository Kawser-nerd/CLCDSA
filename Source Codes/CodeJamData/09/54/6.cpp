...
#define NAME "D-small-attempt1"

vector<LL> a;

void gen(int mxlen = 13)
{
...
int main()
{
	...
	REP(tst,tests)
	{
		...
		add(0,a.front()-1,0);
		REP(i,SZ(a)-1)
			add(a[i],a[i+1]-1,(i+1)%2);
		add(a.back(),r+100,(SZ(a)-1+1)%2);
		REP(i,2)
			cnt[i]%=MOD;
		LL res = ((cnt[0]*cnt[0])%MOD+(cnt[1]*cnt[1])%MOD)%MOD;
		res = (res+MOD-(r-l+1)%MOD)%MOD;
		res = (res*inverse(2LL,MOD))%MOD;
