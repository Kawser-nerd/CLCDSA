	...
	internal class Program {
		...
		public class Calc
		{
			public void Solve() {
				...
				Func<long, long, long, long> getDiff = (a, b, c) => EMax(a, b, c) - EMin(a, b, c);

				Func<long, long, long> getMinimumDiv = (h, w) =>
				{
					...
					for (int ah = 1; ah <= h; ah++) {
						{
							...
							rt = Min(rt, getDiff(ah * w, bh * w, ch * w));
						}
						{
							//T
							var nh = h - ah;
							...
							rt = Min(rt, getDiff(ah * w, nh * bw, nh * cw));
						}
					}
				...
				Min(getMinimumDiv(H,W),getMinimumDiv(W,H)).WL();
