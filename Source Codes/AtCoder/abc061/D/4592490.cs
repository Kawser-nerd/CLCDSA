...
namespace ABC_D
{
    class D061
    {
        static void Main(string[] args)
        {
            ...
            for(int i = 0; i < M; i++)
            {
                int[] edge = Console.ReadLine().Split(' ').Select(z => int.Parse(z)).ToArray();
                g.AddEdge(edge[0], edge[1], -edge[2]);
            }
            double[] d;
            bool ok=g.BellmanFord(1, out d);
            string r = ok ? (-d[N]).ToString() : "inf";
            Console.WriteLine(r);
        }
...
namespace ContestLibraryD061
{
    ...
    class Graph
    {
        ...
        public bool BellmanFord(int idS,out double[] distances)
        {
            ...
            double d1 = distances[Vertices.Count];
            ...
            double d2= distances[Vertices.Count];
