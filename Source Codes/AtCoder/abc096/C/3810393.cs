...
namespace AtCoder {
    ...
    static class Program {
        static void Main() {
            var h = ReadInt();
            var w = ReadInt();
            var drawing = h.Times(ReadString).ToList();

            Func<int, int, bool> isBlack = (i, j) => {
            ...
            for (int i = 0; i < h; i++) {
                for (int j = 0; j < w; j++) {
                    if (drawing[i][j] == '.') continue;
                    if (isBlack(i, j - 1) || isBlack(i, j + 1) || isBlack(i - 1, j) || isBlack(i + 1, j)) continue;
