...
static class MainClass
{
    public static void Main(string[] args)
    {
        ...
        var ruisekiwa = new long[n + 1];
        for (int i = 0; i < n; i++)
        {
            ruisekiwa[i + 1] = ruisekiwa[i] + an[i];
        }
        var list = ruisekiwa.ToCountedDictionary();
        var counts = default(BigInteger);
        foreach (var item in list)
        {
            if (item.Value >= 2)
            {
                counts += ((long)item.Value * (item.Value - 1)) / 2;
            }
        }
