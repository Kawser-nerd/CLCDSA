...
int main() {
    ...
    for (int i = 0; i < n; i++){
        if (s[i] == 'B' || s[i] == '?'){
            ans += mod_pow(3, ql[i]+qr[i]) * a[i] % MOD * c[i] % MOD;
            ans %= MOD;
            ans += mod_pow(3, ql[i]+qr[i]-1) * ql[i] % MOD * c[i] % MOD;
            ans %= MOD;
            ans += mod_pow(3, ql[i]+qr[i]-1) * a[i] % MOD * qr[i] % MOD;
            ans %= MOD;
            ans += mod_pow(3, ql[i]+qr[i]-2) * ql[i] % MOD * qr[i] % MOD;
            ans %= MOD;
        }
    }
    cout << ans << endl;
