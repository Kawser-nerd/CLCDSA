...
class MatSeg {
	...
	void update(int i, double scale, double offset) {
		i += n - 1;
		dat[i] = new double[][] { { scale, 0 }, { offset, 1 } };
		while (i > 0) {
			i = i - 1 >> 1;
	...
	double[][] query(int from, int until) {
		return _query(from, until, 0, 0, n);
	}

	private double[][] _query(int a, int b, int k, int l, int r) {
		if (r <= a || b <= l)
			return ID;
		if (a <= l && r <= b)
			return dat[k];
		double[][] vl = _query(a, b, k * 2 + 1, l, l + r >> 1);
		double[][] vr = _query(a, b, k * 2 + 2, l + r >> 1, r);
		return combine(vl, vr);
	}

	final double[][] ID = new double[][] { { 1, 0 }, { 0, 1 } };

	final double[][] combine(double[][] x, double[][] y) {
		return new double[][] { //
...
public class Main {
	private static void solve() {
		...
		for (int i = 0; i < n2; i++) {
			compress.put(nas[i], i);
		}

		MatSeg st = new MatSeg(n2);
		double[] oishisas = new double[m + 1];
		oishisas[0] = 1;
		for (int i = 0; i < m; i++) {
			int idx = compress.get(ps[i]);
			st.update(idx, xs[i][0], xs[i][1]);
			double[][] mat = st.query(0, n2);
			oishisas[i + 1] = mat[0][0] + mat[1][0];
		}
		Arrays.sort(oishisas);
		out(oishisas[0]);
		out(oishisas[m]);
	}
