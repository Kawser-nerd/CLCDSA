...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		{
			sc.Read(out N);
			a = sc.ReadManyLines<int, int, int>(N);
		}
		void SolveOne()
		{
			var min = 0.0;
			...
			pr.WriteLine((min + max) / 2);
		}
		bool Check(double time)
		{
			var rect = new Rect(-1e5, 1e5, -1e5, 1e5);
			for (var i = 0; i < N; i++)
			{
				rect = rect.Intersect(Rect.Circle(a[i].Item1, a[i].Item2, time / a[i].Item3));
...
class Rect
{
	public double x, X;
	public double y, Y;
	public Rect(double x, double X, double y, double Y) { this.x = x; this.X = X; this.y = y; this.Y = Y; }
	public Rect Intersect(Rect p)
	{
		var nx = Math.Max(x, p.x);
		var nX = Math.Min(X, p.X);
		var ny = Math.Max(y, p.y);
		var nY = Math.Min(Y, p.Y);
		if (nx > nX || ny > nY) return null;
		else return new Rect(nx, nX, ny, nY);
	}
	public static Rect Circle(double x, double y, double r) { return new Rect(x - r, x + r, y - r, y + r); }
	public override string ToString()
	{
		return string.Format("{0}<x<{1}, {2}<y<{3}", x, X, y, Y);
	}
}
// [0, Size) ?????????
...
class Ameba
{
	...
	public Pair<AmebaState, double>[] a;
	public AmebaState m;
	...
	void Accept(AmebaState point, double value)
	{
...
class UnionFindTree
{
	...
	public UnionFindTree(int capacity)
	{
		...
		parent = new int[N];
		rank = new int[N];
		for (var i = 0; i < capacity; i++) parent[i] = i;
	}
