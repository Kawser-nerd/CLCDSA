...
double getarea(double r1,double r2,double d)
{
	double p=acos((sqr(r1)+sqr(d)-sqr(r2))/(2*r1*d));
	return (p-sin(2*p)/2.0)*sqr(r1);
}
double solve(double x1,double y1,double r1,double x2,double y2,double r2)
{
	double d=dist(x1,y1,x2,y2);
	if (r1+r2<=d+eps) return 0;
	if (fabs(r1-r2)>=d-eps) return sqr(min(r1,r2))*pi;
	return getarea(r1,r2,d)+getarea(r2,r1,d);
}
int main()
{
...
//	freopen("D-large.in","r",stdin);freopen("D-large.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int step=0;step<m;step++)
		{
			...
			r1=dist(x1,y1,xc,yc);
			r2=dist(x2,y2,xc,yc);
			double s=solve(x1,y1,r1,x2,y2,r2);
			printf(" %.18lf",s);
		}
		printf("\n");
	}
