...
public class Main {
	...
	public void run() {
		...
		int b = 0;
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < f[b ^ 1].length; ++j) {
				for (int k = 0; k < f[b ^ 1][j].length; ++k) {
					for (int l = 0; l < f[b ^ 1][j][k].length; ++l) {
						f[b ^ 1][j][k][l] = 0;
					}
				}
			}
			...
			for (int max = 0; max <= n; ++max) {
				for (int balance = Math.max(-k, max - k - 1); balance <= Math.min(k, max + 1); ++balance) {
					if ((s[i] == '1' || s[i] == '?') && balance + 1 <= max && -k <= balance + 1 && balance + 1 <= k
							&& max - (balance + 1) <= k && max >= balance + 1) {
					...
					if ((s[i] == '0' || s[i] == '?') && -k <= balance - 1 && balance - 1 <= k
							&& max - (balance - 1) <= k && max >= balance - 1) {
						f[b ^ 1][1][max][balance + n - 1] = add(f[b ^ 1][1][max][balance + n - 1],
								f[b][1][max][balance + n]);
						f[b ^ 1][0][max][balance + n - 1] = add(f[b ^ 1][0][max][balance + n - 1],
