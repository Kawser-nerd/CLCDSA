...
void input() {
  ...
  K = -1;
  tr (id, ite) ite->second = K++;
  rep (i, N) I[i] = id[I[i]];
}

int mem[20][20][1 << 15];

int search(int i, int t, int b) {
  if (t < 0) return INF;
  if (i == N) return t + __builtin_popcount(b);
  if (mem[i][t][b] != -1) return mem[i][t][b];
  int r = INF;

  if (E[i]) {  // Enter
    if (I[i] == -1) r = min(r, search(i + 1, t + 1, b));
    rep (k, K) {
      if (I[i] == -1 || I[i] == k) {
        if (!(b & (1 << k))) r = min(r, search(i + 1, t, b | (1 << k)));
      }
    }
  } else {  // Leave
    if (I[i] == -1) r = min(r, search(i + 1, t - 1, b));
...
void solve() {
  ...
  rep (t, N + 1) rep (b, 1 << K) ans = min(ans, search(0, t, b));
