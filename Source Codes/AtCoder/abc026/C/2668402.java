        ...
        try {
            ...
            for (int i = 0; i < arr.length; i++) {
                for (Employee employee1 : employees) {
                    if (employee1.id==arr[i]) {
                        employee1.subs.add(findById(employees, i+2));
                    }
                }
            }
            ...
            for (Employee employee : employees) {
                if (employee.subs.size()==0) {
                    employee.salary=1;
                } else {
                    int max = 0;
                    int min = INF;
                    for (Employee sub : employee.subs) {
                        max=Math.max(max, sub.salary);
                        min=Math.min(min, sub.salary);
                    }
                    employee.salary = max+min+1;
                }
            }

            System.out.println(employees.get(employees.size()-1).salary);
