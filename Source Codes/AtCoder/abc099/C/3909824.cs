...
namespace AtCoder {
    ...
    static class Program {
        static void Main() {
            ...
            var sixes = 1L.Iterate(Mul(6L)).TakeWhile(Leq((long) n));
            var nines = 1L.Iterate(Mul(9L)).TakeWhile(Leq((long) n));

            // ??????????
            var coins = sixes.Concat(nines).ToList();
            coins.Sort();

            var dp = new int[n + 1]; // dp[x] ... x???????????????
            FromTo(1, n).ForEach(i =>
                dp[i] = coins.TakeWhile(Leq((long) i)).Select(x => 1 + dp[i - x]).Min()
            );
            dp[n].WriteLine();
