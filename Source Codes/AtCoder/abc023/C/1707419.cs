...
namespace CS_Contest {
	...
	internal class Program {
		...
		public class Calc {
			public void Solve() {
				int R = Cin.NextInt(), C = Cin.NextInt(), K = Cin.NextInt(), N = Cin.NextInt();
				List<Tuple<int, int>> point = new List<Tuple<int, int>>();
				int[] V = new int[R+1], H = new int[C+1], Vert = new int[N+1], Hori = new int[N+1];
				REP(N, _ => {
					int r = Cin.NextInt(), c = Cin.NextInt();
					//r--;c--;
					V[r]++;
					H[c]++;
					point.Add(new Tuple<int, int>(r, c));
				});
				for (int i = 1; i <= R; i++) Vert[V[i]]++;
				for (int i = 1; i <= C; i++) Hori[H[i]]++;

				long ans = 0;
				REP(K+1, _ => ans += Vert[K-_] * Hori[_]);
				foreach (var item in point) {
					var cnt = V[item.Item1] + H[item.Item2];
					//???????K???????
					if (cnt == K) ans--;
					//???????K+1???????
					if (cnt == K + 1) ans++;
