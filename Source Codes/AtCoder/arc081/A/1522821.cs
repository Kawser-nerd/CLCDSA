...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		long Calc()
		{
			...
			foreach (var a in A) { if (c.ContainsKey(a)) c[a]++; else c[a] = 1; }
			Array.Sort(A);
			var m = -1;
			for (var i = N - 1; i >= 0; i--)
				if (c[A[i]] >= 4) return (long)A[i] * A[i];
				else if (c[A[i]] >= 2) { m = i; break; }
			if (m < 0) return 0;
			var n = -1;
			for (var i = m - 1; i >= 0; i--) if (A[i] < A[m] && c[A[i]] >= 2) { n = i; break; }
			if (n < 0) return 0;
			return A[m] * (long)A[n];
		}
