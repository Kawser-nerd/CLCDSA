...
double solve(int n,Point p[],double x0)
{
	double R=0;
	for (int i=1;i<n;i++)
		if (p[i].x<=x0)
		{
			R+=(p[i-1].y+p[i].y)/2.0*(p[i].x-p[i-1].x);
		}
		else
		{
			double y0=((x0-p[i-1].x)*p[i].y+(p[i].x-x0)*p[i-1].y)/(p[i].x-p[i-1].x);
			R+=(p[i-1].y+y0)/2.0*(x0-p[i-1].x);
			break;
		}
	return R;
}
...
void solve()
{
	double total=solve(m,b,w)-solve(n,a,w);
	double last=0;
	for (int i=1;i<k;i++)
	{
		double exp=total*i/k;
		double H=0,T=w;
		for (int step=0;step<100;step++)
		{
			double M=(H+T)/2;
			double s1=solve(n,a,M);
			double s2=solve(m,b,M);
			double s=s2-s1;
			if (s>exp)
				T=M;
			else
...
//	freopen("A-small-attempt2.in","r",stdin);freopen("A-small-attempt2.out","w",stdout);
	char large_outputfilename[100]; sprintf(large_outputfilename,"A-large%d.out",d_testcase); freopen("A-large.in","r",stdin);freopen(large_outputfilename,"w",stdout);
	int testcase;
	scanf("%d",&testcase);
