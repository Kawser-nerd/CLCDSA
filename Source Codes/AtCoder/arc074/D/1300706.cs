...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		{
			sc.Read(out Y, out X);
			map = sc.Board(X, Y, (c, x, y) =>
			{
				switch (c)
				{
					case 'S': sx = x; sy = y; return false;
					case 'T': tx = x; ty = y; return false;
					case 'o': return true;
		...
		class Edge
		{
			...
			public Edge Reverse;
			public Edge(int to, int cap, Edge rev)
			{
				To = to;
				Capacity = cap;
				Reverse = rev;
