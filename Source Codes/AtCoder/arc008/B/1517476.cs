...
namespace Program {
    class MainClass {
        ...
        void Solve() {
            io.i(out N, out M);
            io.i(out name, out kit);

            var aaName = new AssociativeArray<char>(name.ToCharArray());
            var aaKit = new AssociativeArray<char>(kit.ToCharArray());

            int ans = 1;

            foreach (var kp in aaName.dic) {
                if (!aaKit.conK(kp.Key)) {
                    ans = -1;
                    break;
                }

                while (kp.Value > aaKit.dic[kp.Key] * ans) ans++;
    ...
    class AssociativeArray<T> : IEnumerable {//hack:???????
        public Dictionary<T, int> dic;
        public KeyValuePair<T, int> kp;
