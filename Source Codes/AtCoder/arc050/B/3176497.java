...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        long r = goldSectionSearchTotu(0, min(R/x,B));
        System.out.println(calcg(r));
    }

    public static long calcg(long v) {
        long r = R - v * x;
        long b = B - v;
    ...
    public static long goldSectionSearchTotu(long left, long right) {
        ...
        while (right - left > 10) {
            if (fl > fr) {
                ...
                rm = lm;
                fr = fl;
                lm = left + (long) ((right - left) / (GRATIO + 1.0));
                fl = calcg(lm);
            } else {
                left = lm;
                lm = rm;
                fl = fr;
                rm = lm + (long) ((right - lm) / (GRATIO + 1.0));
                fr = calcg(rm);
            }
        }
        if (left > right) {
