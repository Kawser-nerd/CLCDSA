...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			Array.Sort(t, (x, y) => Comp(x, y));
			var min = Eval(t);
			/*Array.Sort(t, (x, y) =>
			{
				int dx = (x.Item1 - x.Item2).CompareTo(0), dy = (y.Item1 - y.Item2).CompareTo(0);
				if (dx != dy) return dx.CompareTo(dy);
				else if (dx > 0) return y.Item1.CompareTo(x.Item1);
				else return x.Item1.CompareTo(y.Item1);
			});*/
			//Array.Sort(t, (x, y) => CompAfo(x, y));
			min = Math.Min(min, Eval(t));
			pr.WriteLine(min);
		}
		long Eval(Tuple<int, int>[] t)
		{
			var max = 0L;
			var temp = 0L;
			foreach (var m in t)
		...
		int Comp(Tuple<int, int> x, Tuple<int, int> y)
		{
			var tmp = Math.Max(x.Item1, x.Item1 - (long)x.Item2 + y.Item1).CompareTo(Math.Max(y.Item1, y.Item1 - (long)y.Item2 + x.Item1));
			if (tmp == 0) return y.Item2.CompareTo(x.Item2);
			return tmp;
		}
		int CompAfo(Tuple<int, int> x, Tuple<int, int> y)
		{
			var tmp = Math.Max(x.Item1, x.Item1 - (long)x.Item2 + y.Item1).CompareTo(Math.Max(y.Item1, y.Item1 - (long)y.Item2 + x.Item1));
			if (tmp == 0) return y.Item2.CompareTo(x.Item2);
			return tmp;
