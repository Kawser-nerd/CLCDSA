...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			...
			Q = sc.Int;
			qs = sc.ReadManyLines<int, int, int>(Q);
		}
		void SolveOne()
		{
			var es = new List<int>[N];
			for (var i = 0; i < N; i++) es[i] = new List<int> { i };
			foreach (var r in rs)
			{
				int a = r.Item1 - 1, b = r.Item2 - 1;
				es[b].Add(a);
				es[a].Add(b);
			}
			var col = new int[Q];
			...
			for (var v = 0; v < N; v++) for (var d = 0; d <= D; d++) dp[v, d] = -1;
			for (var i = 0; i < Q; i++)
			{
				col[i] = qs[i].Item3;
				int v = qs[i].Item1 - 1, d = qs[i].Item2;
				dp[v, d] = Max(dp[v, d], i);
			}
			for (var d = D; d > 0; d--) for (var v = 0; v < N; v++) foreach (var u in es[v]) dp[u, d - 1] = Max(dp[u, d - 1], dp[v, d]);
			for (var v = 0; v < N; v++) pr.WriteLine(dp[v, 0] >= 0 ? col[dp[v, 0]] : 0);
		}
