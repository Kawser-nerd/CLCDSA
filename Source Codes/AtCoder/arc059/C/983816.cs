...
public class Program
{
    ...
    public void Solve()
    {
        ...
        int Bmax = B.Max();
        var powSum = new int[Bmax + 1, C + 1];
        for (int x = 1; x <= Bmax; x++)
            for (long n = 0, p = 1; n <= C; n++, p = p * x % Mod)
                powSum[x, n] = (int)((powSum[x - 1, n] + p) % Mod);

        var dp = new int[C + 1];
        dp[0] = 1;
        for (int i = 0; i < N; i++)
        {
            var nextdp = new int[C + 1];
            for (int sum = 0; sum <= C; sum++)
                if (dp[sum] > 0)
                    for (int add = 0; sum + add <= C; add++)
                    {
                        long v = Mod + powSum[B[i], add] - powSum[A[i] - 1, add];
                        Add(ref nextdp[sum + add], (int)(v % Mod * dp[sum] % Mod));
                    }
            dp = nextdp;
        }
