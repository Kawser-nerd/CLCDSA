...
namespace ProgrammingContest
{
    ...
    class MainClass : IDisposable
    {
        ...
        void Solve()
        {
            ...
            var T = Sc.Next();
            int L = S.Length;
            var sCnt = S.ToLookup(c => c).ToDictionary(g => g.Key, g => g.Count()).Select(p => p.Value).OrderBy(v => v);
            var tCnt = T.ToLookup(c => c).ToDictionary(g => g.Key, g => g.Count()).Select(p => p.Value).OrderBy(v => v);
            Wr.WriteLine(sCnt.SequenceEqual(tCnt) ? "Yes" : "No");
        }
