...
public class Main {
    ...
    public static void main(String[] args){
        ...
        for(int i=0;i<n;i++){
            minw[c[i]]=Math.min(minw[c[i]],w[i]);
        }
        ...
        int m1=0;
        for(int i=0;i<n;i++){
            if(minw[i]<minw[m1]) m1=i;
        }
        int m2=0;
        for(int i=0;i<n;i++){
            if((m1!=i) && minw[i]<minw[m2]) m2=i;
        }
        
        int[] counter=new int[n];
        for(int i=0;i<n;i++){
            if(c[i]==m1){
                if((w[i]+minw[m2])<=y) counter[c[i]]++;
            }else{
                if((w[i]+minw[m1])<=y) counter[c[i]]++;
            }
        }
        ...
        long den=1;
        for(int i=0;i<n;i++){
            den=(den*fact[counter[i]])%mod;
            sum+=counter[i];
        }
        long num=fact[(int)sum];
        long res=(num*inv(den,mod))%mod;
        pl(res);
        flush();
