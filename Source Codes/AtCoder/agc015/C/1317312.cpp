...
int main(){
  ...
  nblue.emplace_back(m+1,0);
  nb_tate.emplace_back(m+1,0);
  nb_yoko.emplace_back(m+1,0);
  for(int i = 1; i < (n+1); ++i){
    nblue.emplace_back(nblue[i-1]);
    nb_tate.emplace_back(nb_tate[i-1]);
    nb_yoko.emplace_back(nb_yoko[i-1]);
    for(int j = 1; j < (m+1); ++j){
