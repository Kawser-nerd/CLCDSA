        ...
        for (int i = 0; i < C; i++)
            if (d[i].Count() >= 2)
            {
                ...
                for (int j = 1; j < sortd.Length; j++)
                {
                    ...
                    else { ret.Add(new Ba { s = ns, t = nt }); ns = sortd[j].s; nt = sortd[j].t; }
                    if (j == sortd.Length - 1 && wns) ret.Add(new Ba { s = ns, t = nt });
                    else if (j == sortd.Length - 1 && !wns) ret.Add(new Ba { s = sortd[j].s, t = sortd[j].t });
                }
            }
            else if (d[i].Count() == 1) ret.Add(new Ba { s = d[i][0].s, t = d[i][0].t });
        return ret;
    }
}