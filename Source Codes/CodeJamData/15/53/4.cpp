...
void search(double t, double x) {
  if (t > ans) return;
  if (count(done, done + N, true) == N) {
    ans = t;
    return;
  }
  // printf("t=%f, x=%f\n", t, x);

  pair<double, int> minL(1E9, -1), minR(1E9, -1);
  rep (i, N) {
    if (done[i]) continue;

    double qx = P[i] + S[i] * t * (P[i] > 0 ? +1 : -1);
    double dt = fabs(x - qx) / (myS - S[i]);

    if (qx < x) minL = min(minL, mp(dt, i));
    else        minR = min(minR, mp(dt, i));
  }

  if (minL.second != -1) {
    done[minL.second] = true;
    search(t + minL.first, x - myS * minL.first);
    done[minL.second] = false;
  }
  if (minR.second != -1) {
    done[minR.second] = true;
    search(t + minR.first, x + myS * minR.first);
    done[minR.second] = false;
  }
}
