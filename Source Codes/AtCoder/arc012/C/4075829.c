...
./Main.c:5:23: warning: implicit declaration of function ‘gets’ [-Wimplicit-function-declaration]
  for(int i=0;i<19;i++)gets(b[i]);
                       ^
./Main.c:34:104: warning: implicit declaration of function ‘exit’ [-Wimplicit-function-declaration]
  for(int i=0;i<19;i++)for(int j=0;j<19;j++)if(bc-wc==1?!WC&&cnt[i][j]==BC:bc-wc==0&&!BC&&cnt[i][j]==WC)exit(!puts("YES"));
                                                                                                        ^
./Main.c:34:104: warning: incompatible implicit declaration of built-in function ‘exit’
./Main.c:34:104: note: include ‘<stdlib.h>’ or provide a declaration of ‘exit’
./Main.c:34:110: warning: implicit declaration of function ‘puts’ [-Wimplicit-function-declaration]
  for(int i=0;i<19;i++)for(int j=0;j<19;j++)if(bc-wc==1?!WC&&cnt[i][j]==BC:bc-wc==0&&!BC&&cnt[i][j]==WC)exit(!puts("YES"));
                              ...