...
struct RealCase : public Case
{
    ...
    void getCar( const pair<int,int> &trip, int &a, multiset<int> &atA, multiset<int> &atB );
...
{
    VS vs = SPLITC(s, ":");
    return 60*PARSE<int>(vs[0]) + PARSE<int>(vs[1]);
}

pair<int,int> getTrip()
{
    VS vs = SPLITC(LINE());
    return MP(parseTime(vs[0]), parseTime(vs[1]));
}
...
RealCase::RealCase()
{
    ...
    NA = PARSE<int>(vs[0]);
    NB = PARSE<int>(vs[1]);
    REP(_, NA) A.PB(getTrip());
    REP(_, NB) B.PB(getTrip());
}

void RealCase::getCar(
    const pair<int,int> &trip, int &a, multiset<int> &atA, multiset<int> &atB )
{
    if (atA.empty() || *atA.begin() > trip.first) ++a; else  atA.erase(atA.begin());
    atB.insert(trip.second + T);
}
...
void RealCase::solve(Result &result)
{
    ...
    SORT(B);
    int a = 0, b = 0, i = 0, j = 0;
    multiset<int> atA, atB;
    while (i < NA || j < NB)
    {
        if (j == NB || (i < NA && A[i].first <= B[j].first))
            getCar(A[i++], a, atA, atB);
        else
            getCar(B[j++], b, atB, atA);
    }
    ostringstream oss;
    oss << a << ' ' << b;
    result.set(oss.str());
