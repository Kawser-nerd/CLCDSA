...
namespace AtCoder {
    class Program {
        static void Main() {
            ...
            Func<string, int> intersectionLength = (str) => str.Zip(t, (x, y) => x == y).TakeWhile(x => x).Count();

            var intseclen = Range(0, 101).Select(x => s.Skip(x).Show()).Select(intersectionLength).Max();

            (2 * n - intseclen).WriteLine();
        }
