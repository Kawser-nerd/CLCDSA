...
private:
	static inline value_type merge(const value_type& l, const value_type& r)
	{
		...
		auto lout_max = l.max + l.diff;
		if (lout_max < r.min) {
			res.min = res.max = r.min - l.diff;
			return res;
		}
		else {
			res.max = std::min(l.max, r.max - l.diff);
		}
		auto lout_min = l.min + l.diff;
		if (lout_min > r.max) {
			res.min = res.max = r.max - l.diff;
			return res;
		}
		else {
			res.min = std::max(l.min, r.min - l.diff);
		}
