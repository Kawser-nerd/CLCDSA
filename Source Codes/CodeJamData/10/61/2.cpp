...
int n;
char a[maxsize];
int f[maxsize][maxsize][4];

int solve(int s,int t,char key)
{
	if (s>t) return 0;
	int &ret=f[s][t][key-'a'];
	if (ret>=0) return ret;
	if (a[s]==key)
		return ret=solve(s+1,t,key);
	if (a[t]==key)
		return ret=solve(s,t-1,key);
	ret=100000000;
	checkmin(ret,solve(s+1,t,a[s])+2);
	for (int i=s;i<t;i++)
		checkmin(ret,solve(s,i,key)+solve(i+1,t,key));
	return ret;
}
