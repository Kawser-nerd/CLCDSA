...
def parser_mat1(filepath):
    ...
    linenum = 0
    count = 0
    pat = re.compile(r"(local_variable_declarator) (\-)\w*")
    pat1 = re.compile(r"\bidentifier\w*\b")
...
def parser_mat2(filepath):
    ...
    count = 0
    lcount = 0
    pat = re.compile(r"(statement) (->)\w*")
    #pat1 = re.compile(r"\bunan\w*\b")
    pat1 = re.compile(r"\bif|IF\(\w*\b")
    pat2 = re.compile(r"\bfor|FOR|while|WHILE\(\w*\b")
    ...
    with open(filepath,'r') as file_object:
        ...
        #for line in file_object:
            ...
            if pat.search(line) != None:
                ...
                #print("Before" + line)
                mt = line.partition("->")
                if pat2.search(mt[2])!=None:
                 #   print("After loop" + mt[2])
                    lcount += 1
                    myloops.append((linenum, line.rstrip('\n')))
                elif (pat1.search(mt[2])== None):# and (pat2.search(mt[2])==None)):
                  #  print("After" + mt[2])
                    count += 1
                    myexpressions.append((linenum, line.rstrip('\n')))
    #for err in myloops:
    #    print("Loop Line " + str(err[0]) + ": " + err[1])
...
def parser_mat3(filepath):
    ...
    count = 0
    param = []
    pat = re.compile(r"(arg_declaration) (\-)\w*")
...
def parser_mat5():#filepath):
    ...
    for i in range(0, len(operand1)):
        ...
        for j in range(0, len(mt)):
            ...
            #m1 = re.compile(r"(\b\w*\(\))(;)")
            if ((m.search(ct) == None)):  # or (m.search(ct)== None)):
                m2 = re.compile(r"\b\w*\(")
...
if __name__ == '__main__':
    ...
    #Variables declared
    #for filepath1 in myfile:
    filepath = 'D:/CloneData/AtCoder/abc003/B/4742320.txt'
    myexpressions = []
    myloops = []
    myoperators = []
