...
//	freopen("B-small-attempt2.in","r",stdin);freopen("B-small-attempt2.out","w",stdout);
	if (0)
	{
		...
		for (int d=0;d<50;d++)
		{
			int n=1000;
			int r[1000];
			for (int i=0;i<n;i++) r[i]=random()%100000+1;
			double s=0;
			for (int i=0;i<n;i++) s+=pi*sqr((double)r[i]);
			s*=5;
			int A;
			while (1)
			{
				A=random()%((int)sqrt((double)(s)));
				if (1e9*A>s) break;
			}
			int B=(int)(s/A+1);
			if (rand()&1) swap(A,B);
	...
	for (int case_id=1;case_id<=testcase;case_id++)
	{
		...
		scanf("%d%d%d",&n,&size_x,&size_y);
		for (int i=0;i<n;i++) scanf("%d",&r[i]);
		for (int i=0;i<n;i++) q[i]=MP(r[i],i);
		sort(q,q+n);
		reverse(q,q+n);
		while (1)
		{
			...
			{
				//if (step%100==0) printf("%d ",step);
				int k=q[step].second;
				vector<int> allx;
				allx.push_back(0);
				allx.push_back(size_x);
				for (int i=0;i<n;i++) if (visited[i]) allx.push_back(ox[i]+r[i]+r[k]);
				bool ok=false;
				sort(allx.begin(),allx.end());
				for (int i=0;!ok && i<SIZE(allx);i++) if (i==0 || allx[i]!=allx[i-1])
				{
					int x0=allx[i];
					if (x0<0 || x0>size_x) continue;
					vector<int> p1,p2;
					for (int j=0;j<n;j++) if (visited[j] && abs(ox[j]-x0)<r[j]+r[k])
					{
						p1.push_back(oy[j]-r[j]-r[k]);
						p2.push_back(oy[j]+r[j]+r[k]);
					}
					vector<int> ally;
					ally.push_back(0);
					ally.push_back(size_y);
					for (int j=0;j<SIZE(p2);j++) ally.push_back(p2[j]);
					sort(p1.begin(),p1.end());
					sort(p2.begin(),p2.end());
					sort(ally.begin(),ally.end());
					int l1=0,l2=0;
					for (int j=0;!ok && j<SIZE(ally);j++) if (j==0 || ally[j]!=ally[j-1])
					{
						int y0=ally[j];
						if (y0<0 || y0>size_y) continue;
						for (;l1<SIZE(p1) && p1[l1]<y0;l1++);
						for (;l2<SIZE(p2) && p2[l2]<=y0;l2++);
						if (l1<=l2)
						{
							ok=true;
							visited[k]=true;
				...
				if (!ok) { is_good=false; break; }
			}
			if (is_good) break;
			for (int i=0;i<n;i++) swap(q[i],q[i+rand()%(n-i)]);
		}
		for (int i=0;i<n;i++) printf(" %d %d",ox[i],oy[i]);
		printf("\n");
		/*
		for (int i=0;i<n;i++) for (int j=i+1;j<n;j++)
		{
			int64 d2=sqr((int64)ox[i]-ox[j])+sqr((int64)oy[i]-oy[j]);
			if (d2<sqr((int64)r[i]+r[j]))
			{
				printf("ERROR\n");
				exit(0);
			}
