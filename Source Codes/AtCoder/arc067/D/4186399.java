...
public class Main{
	public static void main(String[] args){
		...
		for(int i=0;i<key.length;i++) key[i]=keys[i];
		long[] vs=new long[key.length];
		for(int i=0;i<key.length;i++) vs[i]=map.get(keys[i]);
		for(int i=0;i<n;i++){
			long sum=0;
			for(int j=0;j<n;j++){
				int ind=Arrays.binarySearch(key,6000*i+j);
				long ad=0;
				if(ind<0)ad=0;
				else ad=vs[ind];
				sum+=ad;
        	...
        	used = total - free;
			//if(used >= (1024*1024*80)) System.gc();
		}
