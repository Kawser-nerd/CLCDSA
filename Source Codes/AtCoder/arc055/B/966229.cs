...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			used = new bool[N, K + 1, 2];
			pr.WriteLine(Calc(0, 0, false));
		}
		double Calc(int i, int j, bool ok)
		{
			if (j > K) return 0;
			else if (i == N) return ok ? 1 : 0;
			else if (!used[i, j, ok ? 1 : 0])
			{
				var val = i * Calc(i + 1, j, ok) / (i + 1);
				val += Max(Calc(i + 1, j + 1, true), Calc(i + 1, j, false)) / (i + 1);
				dp[i, j, ok ? 1 : 0] = val;
				used[i, j, ok ? 1 : 0] = true;
			}
			return dp[i, j, ok ? 1 : 0];
		}
	}
}
class Tree2
