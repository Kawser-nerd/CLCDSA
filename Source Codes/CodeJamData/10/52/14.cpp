...
static const int S = 100010;
int pot[200020];

void solve() {
  ll maxb = *max_element(B, B + N);

  ll def = L / maxb;
  ll rest = L - def * maxb;

  for (int x = -maxb; x <= maxb; ++x) pot[S + x] = INF;
  deque<pair<int, int> > que;
  
  pot[S - rest] = 0;
  que.push_back(mp(-rest, 0));

  while (!que.empty()) {
    int x = que.front().first;
    int p = que.front().second;
    que.pop_front();
    if (p > pot[S + x]) continue;

    if (x == 0) break;

    rep (i, N) {
      int tx = x + B[i] - maxb;
      if (tx < -maxb || maxb < tx) continue;
      if (pot[S + tx] <= p) continue;
      pot[S + tx] = p;
      que.push_front(mp(tx, p));
    }

    rep (i, N) {
      int tx = x + B[i];
      if (tx < -maxb || maxb < tx) continue;
      if (pot[S + tx] <= p + 1) continue;
      pot[S + tx] = p + 1;
      que.push_back(mp(tx, p + 1));
    }
  }

  if (pot[S] < INF) printf("%lld\n", def + pot[S]);
