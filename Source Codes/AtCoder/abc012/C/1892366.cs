	...
	internal class Program {
		...
		public class Calc {
			public void Solve() {
				...
				var list = Enumerable.Range(1, 9).Where(_ => sum % _ == 0).Where(_=>sum/_<=9).Select(_ => new Vector2(x: sum / _, y: _))
					.OrderBy(x => x.X).ThenBy(y => y.Y).ToList();
				list.Select(_=>_.X+" x "+_.Y).WL();
				return;
			}
