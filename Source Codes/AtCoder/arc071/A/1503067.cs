...
namespace AtCoder {
    class MainClass {
        ...
        void Solve() {
            ...
            var all = new AssociativeArray<char>();
            REP(n, i => all.Add(S[i].ToCharArray()));
            var keys = all.Keys();

            var per = new AssociativeArray<char>[n];
            REP(n, i => {
                per[i] = new AssociativeArray<char>();
                per[i].Add(S[i].ToCharArray());
            });

            REP(keys.Length, i => {
                int tmp = int.MaxValue;
                REP(n, j => tmp = Math.Min(tmp, per[j].Val(keys[i])));
                all.Set(keys[i], tmp);
            });

            string ans = "";
            REP(keys.Length, i => {
                ans += keys[i].ToString().Repeat(all.Val(keys[i]));
            });

            ans = ans.ToCharArray().Sort().toString();
    ...
    class IO {
        ...
        public char nextChar => char.Parse(next);
    ...
    static class ExprTree {
        static Expression<Func<int, bool>> e = n => n > 0;
        static BinaryExpression lt = (BinaryExpression)e.Body;
        static ParameterExpression en = (ParameterExpression)lt.Left;
        static ConstantExpression zero = (ConstantExpression)lt.Right;
    }
    #endregion
    #region other
    class Mat {
        ...
        public int[] DigValArr(int n) {
            ...
            for (int i = 0; i < ret.Length; i++) ret[i] = DigVal(n, i + 1);
            return ret;
        }
    ...
    class AssociativeArray<T> {//hack:???????
        ...
        public void Remove(params T[] a) {
            for (int i = 0; i < a.Length; i++) if (dic.ContainsKey(a[i])) dic.Remove(a[i]);
        }
    ...
    static class BitEx {
        public static bool Any(this BitArray b) {
            foreach (bool f in b) if (f) return true; return false;
        }
        public static bool All(this BitArray b) {
            foreach (bool f in b) if (!f) return false; return true;
        }
        public static bool None(this BitArray b) {
            return !Any(b);
        }
