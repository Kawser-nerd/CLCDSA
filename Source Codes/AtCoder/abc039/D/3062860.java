    ...
    public static void solve() {
        ...
        for (int hi = 1; hi < H + 1; hi++) {
            for (int wi = 1; wi < W + 1; wi++) {
                ...
                for (int m = 0; m < 8; m++) {
                    if (ban[hi + y8[m]][wi + x8[m]] == '.') {
                        flag = false;
                        System.err.println("hi : "+hi + " wi :" + wi);
                        System.err.println(y8[m] + " " + x8[m]);
        ...
        for (int hi = 1; hi < H + 1; hi++) {
            for (int wi = 1; wi < W + 1; wi++) {
                if (ban[hi][wi] != add[hi][wi]) same = false;
            }
        }
        if (same) {
