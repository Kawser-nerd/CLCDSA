...
public class Main {
	private static void solve() {
		...
		int n2 = n;
		ArrayList<Integer> pfs = new ArrayList<Integer>(); // pfs[i] < 30;
		...
		for (int i = 0; i < maxf; i++) {
			if (i <= 1) {
				...
				invFacts[i] = (int) modinv(facts[i], mod);
			}
		}
		...
		for (int pf : pfs) {
			int p = pf + m - 1;
			int q = pf;
			long numer = (long) facts[p];
			long denom = (long) invFacts[q] * invFacts[p - q] % mod;
