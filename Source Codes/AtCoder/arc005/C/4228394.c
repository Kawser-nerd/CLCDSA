...
main(R,h,w){
	...
	//for(;R>L;L++)for(r=4;r;)(c=d[x=a[L%_]][y=b[L%_]]+(s[x+=v[r]][y+=v[--r]]%7?1:_))<d[x][y]&&(d[a[R%_]=x][b[R++%_]=y]=c);
	for(;r?:(r=4,R>++L);)(c=d[x=a[L%_]][y=b[L%_]]+(s[x+=v[r]][y+=v[--r]]%7?1:_))<d[x][y]&&(d[a[R%_]=x][b[R++%_]=y]=c);
