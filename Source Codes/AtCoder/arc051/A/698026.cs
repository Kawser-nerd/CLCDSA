...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			sc.Read(out cx, out cy, out r);
			sc.Read(out lx, out ly, out rx, out ry);
		}
		void SolveOne()
		{
			var c = new Circle2D(new Vector2D(cx, cy), r);
			var rect = new Polygon();
			rect.Add(new Vector2D(lx, ly));
			rect.Add(new Vector2D(rx, ly));
			rect.Add(new Vector2D(rx, ry));
			rect.Add(new Vector2D(lx, ry));
			rect.Add(rect[0]);
			var cInR = lx <= cx - r && cx + r <= rx && ly <= cy - r && cy + r <= ry;
			var rInC = true;
			for (var i = 0; rInC && i < 4; i++) rInC &= (rect[i] - c.C).Norm2 <= r * r;
			pr.WriteLine(!cInR);
			pr.WriteLine(!rInC);
		}
	}
