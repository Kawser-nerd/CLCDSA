...
public class Main {
    ...
    public static void main(String[] args) {
		...
		double asd = 0;
		for (int i = 0; i < n; i++)
			asd = Math.max(asd, Math.sqrt((acx-ax[i])*(acx-ax[i]) + (acy-ay[i])*(acy-ay[i])));
		...
		double bsd = 0;
		for (int i = 0; i < n; i++)
			bsd = Math.max(bsd, Math.sqrt((bcx-bx[i])*(bcx-bx[i]) + (bcy-by[i])*(bcy-by[i])));

		System.out.println(bsd/asd);
