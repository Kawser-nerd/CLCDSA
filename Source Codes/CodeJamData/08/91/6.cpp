...
int delta[10002][10002];

void solveCase(int cnum) {
  int res = 0;
  
  FOR(y,0,10002) FOR(x,0,10002) delta[y][x] = 0;
  ...
  FOR(k,0,N) {
    ...
    int x = pers[0];
    int y = pers[1];
    int size = 10000 - x - y - pers[2];
    
//  printf("y %d to %d\n", y, y+size);
    FOR(u,0,size+1) {
      delta[y+u][x]++;
      delta[y+u][x+size+1-u]--;
//    if(y+u == 3000) printf("  %d to %d\n", x, x+size+1-u);
      }
    }
