...
template <class T> void printVector(std::vector<T> u){
  const int k = u.size();
  for(int i = 0; i < k - 1;i++) cout << u.at(i) << ' ';
  cout << u.at(k - 1) << BR;
}


int main(void){
  int h, w, n, idx = 0; cin >> h >> w >> n;
  Vint A(n);
  REP(i, n) cin >> A.at(i);
  Wint data(h, Vint(w));
  REP(i, h * w){
    int p = i / w, q = i % w;
    int index = (p % 2 == 0) ? q : w - 1 - q;
    data[p][index] = idx + 1;
    A.at(idx)--;
    if(A.at(idx) == 0) idx++;
  }
  vREP(ele, data) printVector(ele);
  return 0;
}