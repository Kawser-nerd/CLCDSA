...
int alternate(vector<int> a) {
  ...
  if (c[0] != c[1]) return INF;
  else return c[0];
}

void solve() {
  // puts("\n"); rep (i, N) { rep (j, N) printf("%d", M[i][j]); puts(""); }

  int ans;

  vector<int> s0 = M[0], s1 = flip(s0);
  vector<int> rows(N);
  rep (i, N) {
    if (M[i] == s0) rows[i] = 0;
    else if (M[i] == s1) rows[i] = 1;
    else goto impossible;
  }

  {
    int ans_r = min(alternate(rows), alternate(flip(rows)));
    int ans_c = min(alternate(s0), alternate(s1));
    ans = ans_r + ans_c;
  }
