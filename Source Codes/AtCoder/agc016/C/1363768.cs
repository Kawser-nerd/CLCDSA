...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			sc.Read(out Y, out X, out y, out x);
		}
		int[,] map;
		void SolveOne()
		{
			var t = YesNo(X, Y, x, y);
			pr.WriteLine(t ? "Yes" : "No");
			if (t)
			{
				for (var y = 0; y < Y; y++)
				{
					for (var x = 0; x < X; x++) pr.Write("{0}{1}", map[x, y], x == X - 1 ? "" : " ");
					pr.WriteLine();
				}
			}
		}
		...
		bool YesNo(int X, int Y, int x, int y)
		{
			...
			for (var q = 0; q < Y; q++)
			{
				for (var i = 0; i < N; i++)
				{
					for (var j = 0; j < rem; j++) map[i * x + j, q] = (int)a;
					for (var j = rem; j < x; j++) map[i * x + j, q] = -(int)b;
				}
				for (var i = N * x; i < X; i++) map[i, q] = (int)c;
			}
			return true;
		}
