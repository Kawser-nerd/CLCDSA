...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			foreach (var r in rs)
			{
				int u = r.Item1 - 1, v = r.Item2 - 1;
				es[Math.Min(u, v)].Add(Math.Max(u, v));
			}
			var ok = new List<int>();
			for (var i = N - 1; i >= 0; i--)
			{
				foreach (var j in es[i]) uf.UniteCategory(i, j);
				if (uf.IsSameCategory(i, S - 1)) ok.Add(i);
			}
			ok.Sort();
			foreach (var i in ok) pr.WriteLine(i + 1);
		}
	}
}
