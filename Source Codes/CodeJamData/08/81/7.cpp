// NOTE: replace all throw bad_rational(); with return; or return *this; in /usr/include/boost/rational.hpp before compiling
...
#define x real()
#define y imag()
typedef complex<s64> pt;
typedef complex<rational<s64> > rpt;

pt a_[3];
pt b_[3];

rpt a[3] = {rpt(1, 0), rpt(1, 0), rpt(1, 0)};
rpt b[3] = {rpt(1, 0), rpt(1, 0), rpt(1, 0)};

// m = (bi - bj) / (ai - aj) = (bi - bj) (ai - aj)* / |ai - aj|
rpt compute_m(int ai, int aj, int bi, int bj)
{
	rpt bd = b[ai] - b[bj];
	rpt ad = a[ai] - a[aj];
	return bd / ad;
	/*
	rpt n = bd * conj(ad);
	rational<s64> d = norm(ad);
	return rpt(rational<s64>(n.real(), d), rational<s64>(n.imag(), d));
	*/
}

// k = b - a * m
rpt compute_k(rpt m, int ai, int bi)
{
	return b[bi] - a[ai] * m;
//	return rpt(
//	b[bi].x - (a[ai].x * m.x - a[ai].y * m.y),
//	b[bi].y - (a[ai].x * m.y + a[ai].y * m.x)
//	);
}
...
void solve_case(int casen)
{
	...
	in >> a_[0].x >> a_[0].y >> a_[1].x >> a_[1].y >> a_[2].x >> a_[2].y;
	in >> b_[0].x >> b_[0].y >> b_[1].x >> b_[1].y >> b_[2].x >> b_[2].y;
	for(int i = 0; i < 3; ++i)
	{
		a[i] = a_[i];
		b[i] = b_[i];
...
//				out << " OOEEO " << endl;
				...
				out << "Case #" << (casen + 1) << ": " << setprecision(12) << (double)s.real().numerator() / (double)s.real().denominator() << ' ' << (double)s.imag().numerator() / (double)s.imag().denominator() << endl;
				return;
	...
	out << "Case #" << (casen + 1) << ": " << "No Solution" << endl;
	return;	
}
