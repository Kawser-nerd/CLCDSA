...
public class Main {
    ...
    public static void solve() {
        ...
        double ans = 100000.0;
        for (int i = 0; i < N - 1; i++) {
            double dist = distanceDotAndLine(start, vec[i], vec[i + 1]);
            ans = Math.min(dist, ans);
        }
        double dist = distanceDotAndLine(start, vec[N - 1], vec[0]);
        ans = Math.min(dist, ans);
    ...
    static double distanceDotAndLine(Vec P, Vec A, Vec B) {
        ...
        double L = distanceVertex(A, B); // AB????
