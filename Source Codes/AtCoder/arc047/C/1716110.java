...
public class Main implements Runnable {
	...
	public void run() {
		...
		for (int i = n; i >= 1; --i) {
			if (f[i - 1]) {
				System.out.println(tree.kth(i).key);
				tree.delete(tree.kth(i));
			} else if (f[i - 2] && a[i - 1] > 0) {
				System.out.println(tree.kth(a[i - 1]).key);
				tree.delete(tree.kth(a[i - 1]));
			} else {
				System.out.println(tree.kth(a[i - 1] + 1).key);
				tree.delete(tree.kth(a[i - 1] + 1));
			}
		}
	}
