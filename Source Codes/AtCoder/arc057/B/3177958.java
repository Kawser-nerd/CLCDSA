...
//                long needWin = (i == 0) ? 1 : dp[i][j] * rui[i + 1] / rui[i] + 1 - j;
//                //??????                              //???100%??????????????????
//                if (dp[i][j] + needWin < K + 1 && needWin < rui[i + 1] + 1)
//                    dp[i + 1][j + 1] = Math.min(dp[i + 1][j + 1], dp[i][j] + needWin);
                if (i == 0) {
                    dp[i + 1][j + 1] = min(dp[i + 1][j + 1], dp[i][j] + 1);
                } else {
                    long atleast = (long) dp[i][j] * rui[i + 1] / rui[i] + 1;
