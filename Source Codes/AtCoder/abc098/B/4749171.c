c[2];i;m;main(j,s)char*s;{gets(s);for(gets(s);s[++i];*c&=c[1],m=fmax(__builtin_popcount(*c),m))for(*c=c[1]=j=0;s[j];)c[j<i]|=1<<s[j++]-97;printf("%d",m);} ./Main.c:1:1: warning: data definition has no type or storage class
 c[2];i;m;main(j,s)char*s;{gets(s);for(gets(s);s[++i];*c&=c[1],m=fmax(__builtin_popcount(*c),m))for(*c=c[1]=j=0;s[j];)c[j<i]|=1<<s[j++]-97;printf("%d",m);}
 ^
