...
i64 histgram(int n, const vector<int> &h) {
    vector<int> L(n), R(n);
    ...
    i64 res = 0;
    for (int i = 0; i < n; ++i) {
        res = max(res, (i64)h[i] * (R[i] - L[i]));
    }

    return res;
}

i64 histgram2(int n, const vector<int> &h) {
    vector<int> L(n), R(n);
    ...
    i64 res = 0;
    for (int i = 0; i < n; ++i) {
        res = max(res, (i64)(h[i] + 1) * (R[i] - L[i] + 1));
    }
...
int main() {
    ...
    auto a = make_vector<int, 2>(h - 1, w - 1);
