...
class AC {
    ...
    void Solve() {
        ...
        m = io.Int;
        io.ini(out t, m);
        var blue = new AssociativeArray<string>(s);
        var red  = new AssociativeArray<string>(t);
        var ans = 0;
        foreach (var key in s.Concat(t)) {
            var b = blue.Val(key);
            var r = red.Val(key);
            var score = b - r;
            ans = Max(ans, score);
...
class AssociativeArray<T> {
    ...
    public AssociativeArray(params T[] a) : this() { Add(a); }
    public void Set(T k, int v) => dic[k] = v;
    public void Add(params T[] a) { foreach (var v in a) { if (!dic.ContainsKey(v)) dic[v] = 0; dic[v]++; } }
    public void Remove(params T[] a) { foreach (var v in a) { if (!dic.ContainsKey(v)) dic.Remove(v); } }//?
    public int[] Values => dic.Values.ToArray();
    public T[] Keys() => dic.Keys.ToArray();
    public int Val(T k, int def = 0) => dic.ContainsKey(k) ? dic[k] : def;
    ...
    public T MaxValueKey => SearchFstKeyByVal(MaxVal);
    public T MinValueKey => SearchFstKeyByVal(MinVal);
    public T[] MaxValueKeys => SearchKeysByValue(MaxVal);
    public T[] MinValueKeys => SearchKeysByValue(MinVal);
    public bool AnyKeyContains(params T[] k) => k.Any(key => dic.ContainsKey(key));
    public bool AllKeyContains(params T[] k) => k.All(key => dic.ContainsKey(key));
    public void Show() {
        Console.WriteLine("--- SHOW ---");
        foreach (var v in dic) Console.WriteLine(v.Key + "\t:\t" + v.Value);
        Console.WriteLine("--- ---- ---");
    }
    public T[] ValueSortedKey(bool inc = true) =>
            (inc ? (dic.OrderBy(kp => kp.Value)) : (dic.OrderByDescending(kp => kp.Value)))
            .ThenBy(kp => kp.Key).Select(kp => kp.Key).ToArray();
    private T SearchFstKeyByVal(int val) => dic.First(kp => kp.Value == val).Key;
    private T[] SearchKeysByValue(int val) => dic.Where(kp => kp.Value == val).Select(kp => kp.Key).ToArray();
}