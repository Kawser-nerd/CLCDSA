...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			for (var i = 0; i < M; i++) if (A[i] % 2 != 0) odd.Add(i);
			if (odd.Count > 2) pr.WriteLine("Impossible");
			else
			{
				if (odd.Count == 2) A.Swap(odd[1], M - 1);
				if (odd.Count >= 1) A.Swap(odd[0], 0);
				pr.WriteLine(string.Join(" ", A));
				if (M == 1 && A[0] == 1) { pr.WriteLine(1); pr.WriteLine(1); }
				else if (M == 1) { pr.WriteLine(2); pr.WriteLine((A[0] - 1) + " " + 1); }
				else if (A[0] == 1) { pr.WriteLine(M - 1); for (var i = 1; i < M - 1; i++) pr.Write(A[i] + " "); pr.WriteLine(A[M - 1] + 1); }
				else { pr.WriteLine(M); pr.WriteLine(A[0] - 1); for (var i = 1; i < M - 1; i++) pr.Write(A[i] + " "); pr.WriteLine(A[M - 1] + 1); }
			}
		}
	}
}
