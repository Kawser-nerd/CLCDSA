	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			else
			{
				...
				log10 -= 4;
				pow10 = 10000;
				var arr = new int[log10];
				var lim = new int[log10];
				{
					var n = M / pow10;
					for (var i = 0; i < log10; i++) { lim[i] = (int)(n % 10); n /= 10; }
				}
				arr[0] = 1;
				var sum = 1111 % K;
				for (var i = 1; i < pow10; i++) ans += Check(i, oldK);
				while (true)
				{
					var i = log10 - 1;
					while (i >= 0 && arr[i] == lim[i]) i--;
					if (i < 0)
					{
						for (var j = (M / pow10) * pow10; j <= M; j++) ans += Check(j, oldK);
						break;
					}
					else
					{
		...
		int Check(long n, long K)
		{
			var a = n % K;
			var s = n.ToString();
			var b = 0;
			for (var k = 0; k < s.Length; k++) b += s[k] - '0';
			return a == (b % K) ? 1 : 0;
		}
	}
}
