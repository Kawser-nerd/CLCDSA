...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		int Count() { var sum = 0; for (var v = 0; v < N; v++) if (depth[v] <= K / 2) sum++; return sum; }
		int[] depth;
		void DFS(int u) { depth = new int[N]; DFS(-1, u, 0); }
		void DFS(int u, int v) { depth = new int[N]; DFS(-1, u, 0); DFS(-1, v, 0); }
		void DFS(int p, int u, int d)
		{
			depth[u] = d;
			foreach (var v in es[u]) if (v != p) DFS(u, v, d + 1);
		}
	}
}
