...
partial class Solution
{
    ...
    void Solve()
    {
        ...
        List<Tuple<int, int>> vines = new List<Tuple<int, int>>();
        for (int i = 0; i < N; i++)
        {
            var line = SplitLine_int();
            vines.Add(Tuple.Create(line[0], line[1]));
        }
        int D = ReadLine_int();
        int[] maxlen = new int[N];
        maxlen[0] = vines[0].Item1;
        string result = "NO";
        if (maxlen[0] * 2 >= D) { result = "YES"; goto end; }
        int v2 = 1;
        for (int v1 = 0; v1 < N; v1++)
        {
            for ( ; v2 < N; v2++)
            {
                int dist = vines[v2].Item1 - vines[v1].Item1;
                if (maxlen[v1] < dist) break;
                dist = Math.Min(dist, vines[v2].Item2);
                Debug.Assert(maxlen[v2] == 0);
                maxlen[v2] = dist;
                if (vines[v2].Item1 + dist >= D) { result = "YES"; goto end; }
            }
