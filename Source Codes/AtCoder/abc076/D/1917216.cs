...
namespace Program{
    class MainClass{
        ...
        void Solve(){
            ...
            io.ini(out vn, N);
            var time = 0;
            var nStart = new int[N + 2];
            var tn2 = new int[N + 2];
            var vn2 = new int[N + 2];
            var vx = new double[N + 2];
            N.REP(i =>{
                nStart[i + 1] = time;                
                tn2[i + 1] = tn[i];
                vn2[i + 1] = vn[i];
            ...
            (time *2 ).REP(t =>{
                ...
                (N + 2).REP(n =>{                      
                    if(curTime < nStart[n]){
                        vx[n] = vn2[n] + nStart[n] - curTime;
                    }
                    else if(nStart[n] <= curTime && curTime <= nStart[n] + tn2[n]){
                        vx[n] = vn2[n];
                    }
                    else{
                        vx[n] = vn2[n] + curTime - (nStart[n] + tn2[n]);
                    }
        ...
        void Test(){
            var arr = Enumerable.Range(5, 10).Select(v => v * 2).ToArray();
            io.o(arr.testUpperBound(16));
            io.o(arr.testLowerBound(16));
        }
        #region MockMacro
