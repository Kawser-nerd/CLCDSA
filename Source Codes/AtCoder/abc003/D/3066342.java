...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        for (int m = 1; m < (1 << 4); m++) {
            ...
            if (bitGet(m, 3)) nx--;
            //????????
            //modNcr(nx * ny, d) * (nx*ny-d,l);
            if (nx < 0 || ny < 0) continue;
            long part = modMul(modNcr(nx * ny, d), modNcr(nx * ny - d, l));
            if (Integer.bitCount(m) % 2 == 0) res = modDiff(res, part);
            else res = modSum(res, part);
        }
        long ans = modDiff(modMul(modNcr(x * y, d), modNcr(x * y - d, l)), res);
        ans = modMul(ans, (r - x + 1), (c - y + 1));
