...
int main() {
	...
	cerr << '\n';
	*/
	list<pair<long, long>> s, b;
	int cnt = 0;
	long long sum = 0;
	for (int i = 0; i < k; i++) {
		if (!kind[a[i].second]) {
			cnt++;
			s.push_back(make_pair(INF, a[i].second));
		} else {
			s.push_front(a[i]);
		}
		kind[a[i].second]++;
...
//	cerr << cnt << ' ' << sum << '\n';
	...
	while (!b.empty() && s.front().first != INF) {
		if (!kind[(*b.begin()).second]) {
			sum -= s.front().first * 1LL;
			sum += b.front().first * 1LL;
			kind[s.front().second]--;
			kind[b.front().second]++;
			s.pop_front();
			cnt++;
			s.push_back(make_pair(INF, (*b.begin()).second));
