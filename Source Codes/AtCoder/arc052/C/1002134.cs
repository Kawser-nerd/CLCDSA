...
class MinSegTree
{
    public static readonly int Default = (int)1e9;
    ...
    // [L, R)
    public int Min(int L, int R) { return Rec(0, 0, N, L, R); }
    private int Rec(int index, int currL, int currR, int L, int R)
    {
        if (currL >= R || currR <= L) return Default;
        if (currL >= L && currR <= R) return array[index];
        int mid = (currL + currR) / 2;
        int vl = Rec(index * 2 + 1, currL, mid, L, R);
        int vr = Rec(index * 2 + 2, mid, currR, L, R);
