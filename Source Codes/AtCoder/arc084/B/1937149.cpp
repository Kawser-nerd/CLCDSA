...
int main(void){
	int K;cin>>K;
	vector<int>ans(K,mod);
	deque<pair<int,int>>que;que.pub(mp(1,1));
	while(!que.empty()){
		int it=que.front().fir,aa=que.front().sec;que.pof();
		if(it==0){cout<<aa<<endl;RE;}
		if(ans[it]<=aa){continue;}
		que.puf(mp((it*10)%K,aa));
		que.pub(mp((it+1)%K,aa+1));
		ans[it]=aa;
	}
}