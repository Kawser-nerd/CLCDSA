...
namespace Program {
    class MainClass {
        ...
        void Solve() {
            ...
            var aaE = new AssociativeArray<int>(E);
            var aaL = new AssociativeArray<int>(L);

            int same = 0;
            int ans = 0;
            foreach(var kp in aaE.dic){
                if (aaL.conK(kp.Key))
                    same++;
            }

            if (same == 6) ans = 1;
            else if (same == 5 && aaL.conK(B)) ans = 2;
            else if (same == 5) ans = 3;
            else if (same == 4) ans = 4;
            else if (same == 3) ans = 5;
