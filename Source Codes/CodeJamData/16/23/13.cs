...
";
        ...
        private int Process (bool[] processed, HashSet<string> firsts,  HashSet<string> seconds, ref int count)
        {
            int nonProcessed = 0;
            var aff = new Dictionary<string, int>();
            ...
            for (int i = 0; i < N; i++)
            {
                if (!processed[i])
                {
                    string str = f[i];
                    int j;
                    if (!aff.TryGetValue(str, out j))
                        aff[str] = 1;
                    else
                        aff[str] = j + 1;

                    str = s[i];
                    if (!ass.TryGetValue(str, out j))
                        ass[str] = 1;
                    else
                        ass[str] = j + 1;
                    nonProcessed++;
            ...
            for (int i = 0; i < N; i++)
            {
                if (!processed[i] && firsts.Contains(f[i]) && seconds.Contains(s[i]))
                {
                    nonProcessed--;
                    processed[i] = true;
                    count++;
            ...
            if (nonProcessed > 0)
            {
                ...
                return Process(processed, firsts, seconds, ref count);
            }
            return nonProcessed;
        }


        private int CheckCombination(int combination)
        {
          
            int count = 0;
            HashSet<string> trueF = new HashSet<string>();
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                {
                    count++;
                    if (!trueF.Contains(f[i]) || !trueS.Contains(s[i]))
                    {
                        return -1;
                    }
                }
        ...
        protected override string SolveSmall()
        {
            int min = 0;
            /*
            f = new List<string>(new string[] { "a", "a", "a", "a" });
            s = new List<string>(new string[] { "1", "1", "1", "1" });
            N = f.Count;
            */
            for ( int i = 0; i< Math.Pow ( 2, N); i++)
            {
                var a = CheckCombination(i);
                if (a > min)
                    min = a;
            }
            return min.ToString();
