...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		{
			sc.Read(out N, out K);
			w = sc.ReadManyManyLines<int>(N, N);
		}
		long[] memo;
		bool[] used;
		long[] sum;
		void SolveOne()
		{
			...
			for (var i = 1; i < (1 << N); i++)
			{
				...
				for (var j = 0; j < N; j++) if ((i & (1 << j)) != 0) tmp.Add(j);
				foreach (var a in tmp) foreach (var b in tmp) sum[i] += w[a, b];
				sum[i] >>= 1;
			}
			pr.WriteLine(Max((1 << N) - 1));
		}
		long Max(int set)
		{
			if (!used[set])
			{
				memo[set] = K;
				var sub = set ^ (1 << Func.MSB((uint)set));
				while (sub > 0)
				{
					var c = set ^ sub;
					memo[set] = Math.Max(memo[set], Max(sub) + Max(c) - (sum[set] - sum[sub] - sum[c]));
					sub = (sub - 1) & set;
				}
				used[set] = true;
			}
