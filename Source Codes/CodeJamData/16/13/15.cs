...
Case #4: 6";
        ...
        {
            N = r;
            bffs = r.ReadSpaceSeparatedIntegers();
            for (int i = 0; i < bffs.Length; i++)
                bffs[i]--;
        }


        private int CalculateSizeReversed ( int previous , bool []inList, int first)
        {
            int maxSize = 0;
            for ( int i = 0 ; i < N; i++)
            {
                if (!inList[i])
                {
                    ...
                    if ( bffs[i] == previous)
                    {
                        size = 1 + CalculateSizeReversed(i, inList, first);
                    }
                    else
                    {
                        size = CalculateSize(i, inList, first, -1);
                    }
                    inList[i] = false;
                    if (maxSize < size)
                        maxSize = size;
        ...
        private int CalculateSize (int i ,bool[] inList , int first, int previous)
        {
            var bestFriend = bffs[i];
            if (inList[bestFriend])
            {
                if ( bestFriend == previous)
                {
                    
                    return 1 + CalculateSizeReversed ( i , inList , first);
            ...
            {
                inList[bestFriend] = true;
                var result = 1 + CalculateSize(bestFriend, inList, first,i);
                inList[bestFriend] = false;
                return result;
            }
        }

        /*
        int CalculateBruteForce ( bool[] inList)
        {

            List<int>[] groups = new List<int>[N];
            for ( int i =0 ; i < N ; i++)
            {
                groups [i] = new List<int> ();
                for ( int j = 0; j < N ; j++)
                {
                    if ( bffs[j] == i || bffs[i] == j)
                        groups[i].Add ( j);
                }
            }
        ...
        protected override string SolveLarge()
        {
            ...
            for ( int i = 0; i < N ; i++)
            {
                bool [] removed = CreateArray<bool>(N, false);
                removed [i] = true;
                int size = CalculateSize(i , removed, i,-1);
                if (size > maxSize)
                    maxSize = size;
            }
