...
int main() {
    ...
    for ( int i = 1; i < 1+D; i++ ) {
        for ( int u = i; u+D <= H*W; u += D ) {
            cost[u+D] = abs( nums[u+D].first - nums[u].first ) + abs( nums[u+D].second - nums[u].second );
            cost[u+D] += cost[u];
            //cout4( u, cost[u],u+D,cost[u+D]);
        }
    }
