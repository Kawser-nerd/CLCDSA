...
int main() {
  ...
  vvi last_tm_ch;
  rep (i, n) {
    if (last_tm_ch.size() == 0) {
      last_tm_ch.push_back((vi){stc[i][1], stc[i][2]});
      continue;
    }
    sort(all(last_tm_ch), [](auto x, auto y) {
        return x[0] > y[0];
        });

    bool flg = false;
    rep (j, (int)last_tm_ch.size()) {
      if (last_tm_ch[j][1] == stc[i][2]) {
        last_tm_ch[j][0] = stc[i][1];
        flg = true;
        break;
      } else {
        if (last_tm_ch[j][0] < stc[i][0]) {
          last_tm_ch[j][0] = stc[i][1];
          last_tm_ch[j][1] = stc[i][2];
          flg = true;
          break;
        }
      }
