...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			for (var c = 0; c < 26; c++) first[c] = N;
			var next = new int[N, 26];
			var end = new int[N];
			for (var i = N - 1; i >= 0; i--)
			{
				var flag = true;
				...
				for (var c = 0; c < 26; c++)
					if (first[c] < N) next[i, c] = first[c];
					else
					{
						next[i, c] = -1;
						if (flag) { end[i] = c; flag = false; }
					}
				first[S[i] - 'a'] = i;
			}
			for (var c = 0; c < 26; c++) if (first[c] == N) { pr.WriteLine((char)('a' + c)); return; }
			var len = new int[N];
			var good = new int[N];
			for (var i = N - 1; i >= 0; i--)
			{
				len[i] = N + 1;
				if (end[i] >= 0) len[i] = 2;
				else for (var c = 0; c < 26; c++) if (len[i] > len[next[i, c]] + 1) { len[i] = len[next[i, c]] + 1; good[i] = c; }
			}
			var best = 0;
			for (var c = 0; c < 26; c++) if (len[first[best]] > len[first[c]]) best = c;
			var ans = new string[26];
			var x = first[best];
			var s = new char[len[x]];
			{
				var i = 0;
				s[i++] = (char)(best + 'a');
				while (true)
				{
					if (end[x] >= 0) { s[i++] = (char)(end[x] + 'a'); break; }
					else { s[i++] = (char)(good[x] + 'a'); x = next[x, good[x]]; }
				}
			}
			pr.WriteLine(new string(s));
		}
