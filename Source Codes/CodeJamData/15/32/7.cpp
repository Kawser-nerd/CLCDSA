...
vector<int> kmp;
pair<int, int> adj[110][110];

pair<int, int> go(int p, char c) {
  int k = 0;
  while (p > 0 && L[p] != c) p = kmp[p];
  if (L[p] == c) ++p;
  if (p == LN) {
    ++k;
    p = kmp[p];
...
double solve_max() {
  ...
  rep (i, S) {
    pair<int, int> a;
    rep (j, KN) a = std::max(a, adj[pos][j]);
    ans += a.first;
    pos = a.second;
  }
  return ans;
...
double solve_avg() {
  ...
  double ans = 0.0;
  rep (iter, S) {
    vector<double> tps(LN, 0.0);
    rep (pos, LN) {
      double p = ps[pos] / KN;
      rep (j, KN) {
        ans += p * adj[pos][j].first;
        tps[adj[pos][j].second] += p;
      }
    }
    ps = tps;
  }
...
void solve() {
  ...
  rep (i, LN) rep (j, KN) {
    adj[i][j] = go(i, K[j]);
    assert(0 <= adj[i][j].second);
    assert(adj[i][j].second < LN);
  }
