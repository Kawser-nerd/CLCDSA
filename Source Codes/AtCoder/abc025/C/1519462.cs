...
namespace Program {
    class MainClass {
        ...
        void Solve() {
            ...
            io.o(ans.Item1, ans.Item2);

        }

        Tuple<int, int> rec(int turn) {
            if (turn == 9)
                return cal();
            int tmpChokudai = -1;
            int tmpChokuko = -1;
            for (int i = 0; i < G.GetLength(0); i++)
                for (int j = 0; j < G.GetLength(1); j++) {
                    ...
                    G[i, j] = 0;
                    if (turn % 2 == 0) {
                        if (tmpChokudai < tmp.Item1) {
                            tmpChokudai = tmp.Item1;
                            tmpChokuko = tmp.Item2;
                        }
                    } else {
                        if (tmpChokuko < tmp.Item2) {
                            tmpChokudai = tmp.Item1;
                            tmpChokuko = tmp.Item2;
                        }
                    }
                }
            return Tuple.Create(tmpChokudai, tmpChokuko);
        }

        Tuple<int, int> cal() {
            int tmpChokudai = 0;
            int tmpChokuko = 0;
            b.GetLength(0).REP(i => b.GetLength(1).REP(j => {
                if (G[i, j] == G[i + 1, j])
                    tmpChokudai += b[i, j];
                else
                    tmpChokuko += b[i, j];
            ...
            c.GetLength(0).REP(i => c.GetLength(1).REP(j => {
                if (G[i, j] == G[i, j + 1])
                    tmpChokudai += c[i, j];
                else
                    tmpChokuko += c[i, j];
