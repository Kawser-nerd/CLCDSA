...
public class C {
	...
	class ParallelSolver implements Runnable {
		...
		public void run() {
			...
			int SS = J + S, TT = SS + 1;
			MinimumCostFlow flow = new MinimumCostFlow(TT + 1);
			for(int i = 0; i < J; i++) {
				flow.addEdge(SS, i, K, 0);
			}
			for(int i = 0; i < S; i++) {
				flow.addEdge(J + i, TT, K, 100 - mp.get(i).size());
			}
			for(int i = 0; i < J; i++) {
				for(int j = 0; j < S; j++) {
					flow.addEdge(i, J + j, 1, 0);
				}
			}
			flow.MinCostFlow(SS, TT, 1<<29);
			res = new ArrayList<>();
			for(int i = 0; i < J; i++) {
				for(MinimumCostFlow.Edge e : flow.G.get(i)) {
					if(e.to >= J && e.to < J + S) {
						System.err.println(i + " " + e.to + " " + e.cap);
//						if(e.cap < flow.G.get(e.to).get(e.rev).cap) {
						if(e.cap == 0) {
							for(int[] ls : mp.get(e.to - J)) {
...
//				System.err.println("add: " + from + " " + to + " " + cap);
