...
public:
	...
	pair<ll,T> more_kthsum(int s, int e, int k, int b = 0) {//???????k?
		if (k <= 0) return { 0,0 };
		int eo = bitv[b].rank(e, 1), so = bitv[b].rank(s, 1), cou = eo - so;
		if (b == bitsize - 1) return { min(k,cou),1 };

		if (cou >= k) {
			pair<ll, T>  ret = more_kthsum(border[b + 1] + so, border[b + 1] + so + cou, k, b + 1);
			return {ret.first + (ll)min(k, cou) * ret.second * 2, ret.second * 2};
		}
		else {
			pair<ll, T> ret = more_kthsum(s - so, (s - so) + (e - s - cou), k - cou, b + 1);
			return { ret.first + (bitrui[b][e] - bitrui[b][s]) + (ll)min(k, max(0,cou - k)) * ret.second * 2, ret.second * 2};
		}
	}
	ll less_kthsum(int s, int e, int k, int b = 0) {//???????k?

		return (rui[e] - rui[s]) - more_kthsum(s, e, (e - s) - k).first;
	}
...
signed main() {
	...
	rep(i, n, n*2+1) {
		ll b = wv.more_kthsum(0, i, n).first, c = wv.less_kthsum(i, n * 3, n);
