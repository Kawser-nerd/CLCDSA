...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			...
			map = sc.Board(X, Y, (c, x, y) => { if (c == 'S') { sx = x; sy = y; } return c != '#'; });
		}
		int[] dx = new[] { 1, 0, -1, 0, 1 };
		bool IsIn(int x, int y) => 0 <= x && 0 <= y && x < X && y < Y;
		void SolveOne()
		{
			var d = new int[X, Y];
			for (var x = 0; x < X; x++) for (var y = 0; y < Y; y++) d[x, y] = Inf;
			d[sx, sy] = 0;
			...
			while (q.Count > 0)
			{
				...
				{
					var x = t.Item1 + dx[i];
					var y = t.Item2 + dx[i + 1];
					if (IsIn(x, y) && map[x, y] && d[x, y] == Inf)
					{
			...
			var min = Inf;
			for (var x = 0; x < X; x++) for (var y = 0; y < Y; y++) if (d[x, y] <= K) min = Min(min, x, y, X - x - 1, Y - y - 1);
			pr.WriteLine((min + K - 1) / K + 1);
		}
