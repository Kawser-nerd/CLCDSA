...
namespace CS_Contest {
		...
		public class Calc {
			public void Solve() {
				...
				while (queue.Any()) {
					...
					foreach (var item in list.Select(x => data * 10 + x)) queue.Enqueue(item);
				}
				return;
			}
	...
	public static class Utils {
		...
		public static Ll ReadLongs() => ReadLine().Split().Select(long.Parse).ToList();
		...
		public static void ForeachWith<T>(IEnumerable<T> collection,Action<int> act) {
			foreach (var item in collection.ToIndexEnumerable()) {
				act(item.Index);
			}
		}
