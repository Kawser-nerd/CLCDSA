...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			sc.Read(out N, out A, out B, out C, out D);
		}
		int[] f, g;
		int[,] dp, p, e;
		bool[,] used;
		void SolveOne()
		{
			f = new int[N + 1];
			f[0] = 1;
			for (var i = 1; i <= N; i++) f[i] = (int)((long)i * f[i - 1] % Mod);
			g = new int[N + 1];
			for (var i = 0; i <= N; i++) g[i] = (int)Inverse(f[i], Mod);
			p = new int[N + 1, N + 1];
			for (var n = 0; n <= N; n++) for (var m = 0; m <= n; m++) p[n, m] = (int)((long)f[n] * g[n - m] % Mod);
			e = new int[B + 1, D + 1];
			for (var b = 0; b <= B; b++)
			{
				e[b, 0] = 1;
				for (var k = 1; k <= D; k++) e[b, k] = (int)(e[b, k - 1] * (long)g[b] % Mod);
				for (var k = 1; k <= D; k++) e[b, k] = (int)(e[b, k] * (long)g[k] % Mod);
			}
			dp = new int[N + 1, B - A + 1];
			used = new bool[N + 1, B - A + 1];
			pr.WriteLine(Calc(N, B));
		}
		int Calc(int N, int B)
		{
			if (B < A) return N == 0 ? 1 : 0;
			if (N == 0) return 1;
			if (!used[N, B - A])
			{
				used[N, B - A] = true;
				var ans = (long)Calc(N, B - 1);
				for (var k = C; k <= D && k * B <= N; k++) ans += Calc(N - k * B, B - 1) * (long)p[N, k * B] % Mod * e[B, k] % Mod;
				dp[N, B - A] = (int)(ans % Mod);
			}
			return dp[N, B - A];
		}
