...
class AC {
    ...
    void Solve() {
        io.i(out n, out s);
        s.ToCharArray().Aggregate(new Tuple <int, int>(0, 0), (acc, x) => {
            if(x == 'I') return new Tuple <int, int>(acc.Item1 + 1, MAX(acc.Item2, acc.Item1 + 1));
            if(x == 'D') return new Tuple <int, int>(acc.Item1 - 1, MAX(acc.Item2, acc.Item1 - 1));
            return acc;
        }).Item2.Prt();
    }
    /*------------------------------------------------------------------*/
