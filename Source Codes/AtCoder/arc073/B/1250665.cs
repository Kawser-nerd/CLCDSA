...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			var w0 = ws[0].Item1;
			var mW = 3 * N;
			var dp = new int[N + 1, N + 1, mW + 1];
			for (var j = 0; j <= N; j++) for (var w = 0; w <= mW; w++) if (j != 0 || w != 0) dp[0, j, w] = -Inf;
			for (var i = 1; i <= N; i++)
			{
				var t = ws[i - 1].Item1 - w0;
				for (var j = i + 1; j <= N; j++) for (var w = 0; w <= mW; w++) dp[i, j, w] = -Inf;
				for (var j = 0; j <= i; j++) for (var w = 0; w <= mW; w++)
					{
						// dp[i, j, w] = max(dp[i-1,j,w], dp[i-1,j-1,w-w[i]]+v[i])
						dp[i, j, w] = dp[i - 1, j, w];
						if (j > 0 && w >= t) dp[i, j, w] = Max(dp[i, j, w], dp[i - 1, j - 1, w - t] + ws[i - 1].Item2);
					}
			}
			var ans = 0;
			for (var j = 0; j <= N; j++) for (var w = 0; w <= mW; w++) if (w + (long)j * w0 <= W) ans = Max(ans, dp[N, j, w]);
			pr.WriteLine(ans);
		}
