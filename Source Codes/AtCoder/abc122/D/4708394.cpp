...
const char CHARS[4] = {'A', 'G', 'C', 'T'};

inline bool judge(int a, int b, int c){
  if(a == 0 and b == 1 and c == 2) return false;
  if(a == 1 and b == 0 and c == 2) return false;
  if(a == 0 and b == 2 and c == 1) return false;
  return true;
}

// A???????
inline bool judge_A(int a, int b, int c){
  if(c == 2){
    if(a == 1 or b == 1) return false;
  }
  else if(c == 1 and a == 0 and b == 2) return false;
  return true;
}
...
int main(void){
  int n; cin >> n;
  // "AGC"?????, ?????????????
  // ==> "AGC", "AXGC", "AGXC", "GAC", "ACG"?????
  // DP[idx][a][b][c] := ??idx+3????????,  ??? CHARS[a, b, c]???
  int DP[n-2][4][4][4];
  REP(i, n-2) REP(j, 4) REP(k, 4) REP(l, 4) DP[i][j][k][l] = 0;
  REP(i, 4) REP(j, 4) REP(k, 4) DP[0][i][j][k] = (judge(i, j, k)) ? 1 : 0;
  for(int idx = 0; idx < n-3; idx++){
    // ??dp
    // i = 0
    REP(j, 4) REP(k, 4) REP(new_i, 4) {
      if(judge_A(j, k, new_i)) DP[idx + 1][j][k][new_i] = (DP[idx + 1][j][k][new_i] + DP[idx][0][j][k]) % MOD;
    }
    for(int i = 1; i < 4; i++) REP(j, 4) REP(k, 4) REP(new_i, 4){
      if(judge(j, k, new_i)) DP[idx + 1][j][k][new_i] = (DP[idx + 1][j][k][new_i] + DP[idx][i][j][k]) % MOD;
    }
  }
  int res = 0;
  REP(i, 4) REP(j, 4) REP(k, 4) res = (res + DP[n-3][i][j][k]) % MOD;
  cout << res << BR;
  return 0;
}