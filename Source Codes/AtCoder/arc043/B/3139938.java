...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        A = nia(N);
        sort(A);
        long[] cou3 = new long[N];
        for (int i = N - 1; i >= 0; i--) {
            cou3[i] = N - lowerBound(A, A[i] * 2);
        }
        for (int i = N - 2; i >= 0; i--) {
            cou3[i] += cou3[i + 1];
            cou3[i] %= MOD;
        }
        long[] cou2 = new long[N];
        for (int i = 0; i < N; i++) {
            int ind = lowerBound(A, A[i] * 2);
            if (ind < N) cou2[i] = cou3[ind];
        }
        for (int i = N - 2; i >= 0; i--) {
            cou2[i] += cou2[i + 1];
            cou2[i] %= MOD;
        }
        long[] cou1 = new long[N];
        for (int i = 0; i < N; i++) {
            int ind = lowerBound(A, A[i] * 2);
            if (ind < N) cou1[i] = cou2[ind];
        }
        long res = modSum(cou1);
