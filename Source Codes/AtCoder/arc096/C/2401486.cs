...
class Solve{
    ...
    void Calc(){
        ...
        for(int i=0;i<=N;i++){
            long c = Pow(2,Free(N-i),Define.mod);
            long d = 0;
            long e = Pow(2,N-i,Define.mod);
            long ee = 1;
            for(int j=0;j<=i;j++){
                d += ee * DP[i,j] % Define.mod;
                d %= Define.mod;
                ee *= e;
                ee %= Define.mod;
            }
    ...
    long Free(int x){
        return Pow(2,x,Define.mod-1);
    }
    long Pow(long X,long r,long mod){
        if(r == 0){
            return 1;
        }
        if(r % 2 == 0){
            long T = Pow(X,r/2,mod);
            return T*T%mod;
        }
        else{
            return X * Pow(X,r-1,mod) % mod;
        }
    }
}
class Fact{
