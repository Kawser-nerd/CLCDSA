...
public class Main {
    ...
    public static void main(String[] args){
        ...
        int start=0;
        int s1=0;
        while(true){//pl(String.valueOf(ca).substring(start,ca.length));flush();
            while(ca[start]=='0') start++;
            boolean isInc=true;
            int s2=start-1;
            for(int i=s1+1;i<ca.length;i++){
                if(ca[i]>ca[s1]){
                    s2=s1;
                    s1=i;
                }else if(ca[i]==ca[s1]){
            ...
            if(isInc){
                ...
                start=s2+2;
                s1=Math.max(s1,start);
                
                int ind=ca.length-1;
                while(ca[ind]=='9') ind--;
                ca[ind]=(char)(ca[ind]+1);
                while(ind<ca.length-1) ca[++ind]='0';
            }
            res++;
        }
        pl(res);
    ...
    static void brute(String s){
        ...
        while(true){//pl(s);
            ...
            char target=s.charAt(i);
                if(target>s.charAt(s1)){
                    s2=s1;
                    s1=i;
                }else if(target==s.charAt(s1)){
                    s1=i;
                }else if(target<s.charAt(s1)){
                    isInc=false;
                    break;
            ...
            if(isInc){
                ...
                String don="";
                for(int i=0;i<=s2;i++) don+=s.charAt(i);
                if(s.charAt(s2+1)>'1') don+=(char)(s.charAt(s2+1)-1);
                for(int i=s2+2;i<s.length();i++) don+='9';
                //pl(don);
                b=b.subtract(new BigInteger(don));
                s=b.toString();
            }
            res++;
        }
    ...
    static void tc(){
        String s="";
        for(int i=0;i<100;i++) s+=(int)(Math.random()*10);
        if(s.charAt(0)=='0') s="1"+s;
        pl(s);
        flush();
        try{Thread.sleep(20);}catch(Exception e){}
    }
