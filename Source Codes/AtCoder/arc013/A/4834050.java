...
public class Main{
	...
	public static void main(String[] args){
		...
		int R = sc.nextInt();
		int max = 0;
		max = Math.max(sl(P,Q,R),max);
		max = Math.max(sl(P,Q,R),max);
		max = Math.max(sl(Q,P,R),max);
		max = Math.max(sl(Q,R,P),max);
		max = Math.max(sl(R,P,Q),max);
		max = Math.max(sl(R,Q,P),max);
		System.out.println(max);
	}
	private static int sl(int p, int q, int r) {
		return (N/p) * (M/q) * (L/r);
	}
