...
public class Main {
	...
	private static void solve() {
		...
		for (int i = 0; i < m; i++) {
			...
			kujis[i] = nis2(c, 2);
			for (int j = 0; j < c; j++) {
				probs[i][kujis[i][j][0] - 1] = kujis[i][j][1] * 0.01;
			}
		}
		Main.costs = costs;
		Main.kujis = kujis;
		out(solve(n, m, costs, probs));
	}

	static double solve(int n, int m, int[] costs, double[][] probs) {
		int N = 1 << n;
		double[] dp = new double[N];
		return memoBitDp(n, m, 0, N, costs, probs, dp);
	}

	static double memoBitDp(int n, int m, int S, int N, int[] costs, double[][] probs, double[] dp) {
		if (S == N - 1)
			return 0;
		if (dp[S] > 0)
			return dp[S];
		...
		for (int j = 0; j < m; j++) {
			double numer = costs[j];
			double denom = 0;
			for (int i = 0; i < n; i++) {
				if ((1 << i & S) == 0) { // kaburanai
					denom += probs[j][i];
					numer += probs[j][i] * memoBitDp(n, m, S | 1 << i, N, costs, probs, dp);
				}
			}
			if (numer / denom < min) {
				min = numer / denom;
