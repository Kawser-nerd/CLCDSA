    ...
    public static void main(String[] args){
        ...
        while(size>0){
            ...
            if(jun[cur]){
                ...
                int[] cdps=new int[al[cur].length];
                int csize=0;
                for(int chi:al[cur]){
                    cdps[csize++]=dp[chi];
                }
                Arrays.sort(cdps);
                int max=al[cur].length+1;
                int tail=al[cur].length;
                for(int i=0;i<al[cur].length;i++){
                    max=Math.max(max,cdps[i]+tail);
                    tail--;
                }
                dp[cur]=max;
            }else{
        ...
        pl(dp[1]-1);
        //for(int i=0;i<n+1;i++) pl(Arrays.toString(al[i]));
        flush();
    }
