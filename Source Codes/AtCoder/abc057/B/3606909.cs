...
namespace Csharp
{
    public class Program
    {
        ...
        private void Solve(Input cin)
        {
            ...
            var checkpoints = new Tuple<int, int>[M];

            for (int i = 0; i < N; i++)
            {
                int t1 = cin.ReadInt,
                    t2 = cin.ReadInt;
                students[i] = Tuple.Create(t1, t2);
            }

            for (int i = 0; i < M; i++) {
                int t1 = cin.ReadInt,
                    t2 = cin.ReadInt;
                checkpoints[i] = Tuple.Create(t1, t2);
            }
            ...
            foreach (var student in students)
            {
                ...
                foreach (var checkpoint in checkpoints) {
                    ...
                    var k = kyori(student, checkpoint);
                    if (min_d > k)
                    {
                        min = at;
                        min_d = k;
        ...
        int kyori(Tuple<int, int> tuple1, Tuple<int, int> tuple2)
        {
            return Abs(tuple1.Item1 - tuple2.Item1) + Abs(tuple1.Item2 - tuple2.Item2);
        }
