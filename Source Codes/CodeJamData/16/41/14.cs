...
namespace CodeJam
{
    ...
    public class A : SolverBase
    {
        ...
        private string Solve(string input)
        {
            ...
            int[] nrps = StringToIA(input);
            int n = nrps[0];
            int r = nrps[1];
            int p = nrps[2];
            int s = nrps[3];
            if (Math.Abs(r - p) > 1 || Math.Abs(p - s) > 1 || Math.Abs(s - r) > 1)
            {
                return imposs;
            }
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                res = sb.ToString();
                int len = res.Length;
                if (res.Substring(0, len / 2).CompareTo(res.Substring(len / 2)) > 0)
                {
                    res = res.Substring(len / 2) + res.Substring(0, len / 2);
                }
            }
    ...
    public class RunMain
    {
        static void Main(string[] args)
        {
            ...
            new A().Wrapping(dir + "A-small-attempt1.in", dir + "out.txt");
