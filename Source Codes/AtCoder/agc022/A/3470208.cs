...
namespace competitive_programming
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            var ret = "";
            var used = new HashSet<char>(s);
            var add = Enumerable.Range(0, 26).Select(v => (char?)('a' + v)).FirstOrDefault(c => !used.Contains(c.Value));
            if (add == null)
            {
                ret = GetAns(s);
            }
        ...
        {
            var ret = Enumerable.Range(0, 26 - s.Length).Select(i => ' ').ToArray().Concat(s.ToArray()).ToArray();

            Dfs(ref ret, 25);

            return ret == null ? "-1" : new string(ret.Where(c => c <= 'z' && c >= 'a').ToArray());
        }

        static void Dfs(ref char[] ret, int idx)
        {
            ...
            // find
            var ch = (char)(ret[idx] + 1);
            while (ch != 'z' && usedChars.Contains(ch)) ch++;

            if (usedChars.Contains(ch) || ch > 'z')
            {
                if(idx == 0)
                {
                    ret = null;
