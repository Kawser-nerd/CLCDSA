...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        dpw = new int[N];
        dpb = new int[N];
        System.out.println(modSum(pw(-1, 0), pb(-1, 0)));
    }

    static int dpw[];
    static int dpb[];

    public static long pw(int p, int i) {
        //??
        if (dpw[i] > 0) return dpw[i];
        if (nodes[i].size() == 1 && nodes[i].get(0) == p) {
            return 1;
        }
        long res = 1;
        for (Integer to : nodes[i]) {
            if (to == p) continue;
            long part = modSum(pw(i, to), pb(i, to));
            res = modMul(res, part);
        }
        return dpw[i] = (int) res;
    }

    public static long pb(int p, int i) {
        if (dpb[i] > 0) return dpb[i];
        //??
        if (nodes[i].size() == 1 && nodes[i].get(0) == p) {
            return 1;
        }
        ...
        for (Integer to : nodes[i]) {
            if (to == p) continue;
            res = modMul(res, pw(i, to));
        }
        return dpb[i] = (int) res;
    }
