...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			for (var i = 0; i < N; i++) if (bs[i].Item1 > bs[i].Item2) bs[i] = new Tuple<int, int>(bs[i].Item2, bs[i].Item1);
			Array.Sort(bs);
			var x = bs.Select(a => a.Item1).ToList();
			var y = bs.Select(a => a.Item2).ToList();
			var m = Min(x.Min(), y.Min());
			var M = Max(x.Max(), y.Max());
			// lm = m, rM = M
			int lM = -Inf, rm = Inf;
			for (var i = 0; i < N; i++)
			{
				lM = Max(lM, Min(x[i], y[i]));
				rm = Min(rm, Max(x[i], y[i]));
			}
			var ans = (lM - m) * (long)(M - rm);
			// lm = m, lM = M
			var yr = new Tuple<int, int>[N];
			var xr = new Tuple<int, int>[N];
			yr[0] = new Tuple<int, int>(y[0], y[0]);
			for (var i = 1; i < N; i++)
			{
				if (y[i] < yr[i - 1].Item1) yr[i] = new Tuple<int, int>(y[i], yr[i - 1].Item2);
				else if (y[i] > yr[i - 1].Item2) yr[i] = new Tuple<int, int>(yr[i - 1].Item1, y[i]);
				else yr[i] = yr[i - 1];
			}
			xr[N - 1] = new Tuple<int, int>(x[N - 1], x[N - 1]);
			for (var i = N - 2; i >= 0; i--)
			{
				if (x[i] < xr[i + 1].Item1) xr[i] = new Tuple<int, int>(x[i], xr[i + 1].Item2);
				else if (x[i] > yr[i + 1].Item2) xr[i] = new Tuple<int, int>(xr[i + 1].Item1, x[i]);
				else xr[i] = xr[i + 1];
			}
			var R = Min(xr[0].Item2 - xr[0].Item1, yr[N - 1].Item2 - yr[N - 1].Item1);
			for (var i = 0; i < N - 1; i++)
			{
				var a = yr[i];
				var b = xr[i + 1];
				R = Min(R, Max(a.Item2, b.Item2) - Min(a.Item1, b.Item1));
			}
			ans = Min(ans, R * (long)(M - m));
			pr.WriteLine(ans);
		}
