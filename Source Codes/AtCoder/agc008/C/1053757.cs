...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			if (a[0] >= 1 && a[3] >= 1 && a[4] >= 1) pr.WriteLine(a[1] + Max(f(a[0], a[3], a[4]), 3 + f(a[0] - 1, a[3] - 1, a[4] - 1)));
			else pr.WriteLine(a[1] + f(a[0], a[3], a[4]));
		}
		long f(params long[] x)
		{
			var s = 0L;
			foreach (var y in x) s += (y / 2) * 2;
			return s;
		}
