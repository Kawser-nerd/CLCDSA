...
public class Program
{
    ...
    class RollingHash
    {
        ...
        readonly int[][] Hash = new int[2][], Pow = new int[2][];
        ...
        public RollingHash(string A, int seed = -1)
        {
            ...
            for (int i = 0; i < 2; i++)
                Init(random.Next(10000, 20000), Mod[i], ref A, ref Hash[i], ref Pow[i]);
        }
        void Init(long x, long mod, ref string A, ref int[] hash, ref int[] pow)
        {
            hash = new int[N + 1];
            pow = new int[N + 1];
            pow[0] = 1;
            for (int i = 0; i < N; i++)
            {
                hash[i + 1] = (int)((hash[i] + (long)pow[i] * A[i]) % mod);
                pow[i + 1] = (int)(pow[i] * x % mod);
            }
        }
        public long this[int L, int R] { get { return Get(L, R); } }
        public long Get(int L, int R)
        {
            return (((long)Hash[0][R] - Hash[0][L] + Mod[0]) * Pow[0][N - L] % Mod[0] << 32) +
                    ((long)Hash[1][R] - Hash[1][L] + Mod[1]) * Pow[1][N - L] % Mod[1];
        }
    }
