...
//	freopen("..\\C-small-attempt1.in","r",stdin);freopen("..\\C-small-attempt1.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int i=1;i<=sizeX;i++)
			if (sizeY%3==0)
				for (int j=2;j<=sizeY;j+=3)
					S[i]+=A[i][j];
			else if (sizeY%3==2)
				for (int j=1;j<=sizeY;j+=3)
					S[i]+=A[i][j];
			else 
				for (int j=1;j<=sizeY;j+=3)
					S[i]+=A[i][j];
		result=0;
		for (int first=0;first<=sizeY;first++)
		{
			R[1]=first;
			...
			bool bad=false;
			for (int i=1;i<=sizeX;i++) if (R[i]<0) bad=true;
			if (R[sizeX-1]+R[sizeX]!=S[sizeX]) bad=true;
			if (bad) continue;
			checkmax(result,R[(sizeX+1)/2]);
		}
		printf("%d\n",result);
		fflush(stdout);
	}
