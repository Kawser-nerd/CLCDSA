...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        N = ni();
        A = nlt(N, N);
        RectangleSum rect = new RectangleSum(A);
        long[][] memo = new long[51][51];
        fill(memo, 0);
        for (int h = 1; h < 51; h++) {
            for (int w = 1; w < 51; w++) {
                for (int lx = 0; lx < 51; lx++) {
                    for (int uy = 0; uy < 51; uy++) {
                        long res = rect.getSum(lx, lx + w, uy, uy + h);
                        memo[h][w] = Math.max(memo[h][w], res);
                    }
                }
            }
        }
...
} ./Main.java:85: warning: '_' used as an identifier
        for (int _ = 0; _ < Q; _++) {
                 ^
