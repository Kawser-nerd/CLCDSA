...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			sc.Read(out x1, out y1, out x2, out y2);
			N = sc.Int;
			ft = sc.ReadManyLines<int, int>(N);
		}
		void SolveOne()
		{
			pr.WriteLine("{0:f16}", Calc());
		}
		struct P : IComparable<P>
		{
			public int X, Y;
			public P(int x, int y) { X = x; Y = y; }
			public int CompareTo(P q) => X.CompareTo(q.X);
			public void S(int x, int y) { X *= x; Y *= y; }
			public override string ToString() => $"({X}, {Y})";
		}
		double Calc()
		{
			var g = new P(x2 - x1, y2 - y1);
			var sx = g.X >= 0 ? 1 : -1;
			var sy = g.Y >= 0 ? 1 : -1;
			g.S(sx, sy);
			if (g.X < 0 || g.Y < 0) throw new Exception();
			var ps = new List<P>();
			foreach (var f in ft)
			{
				var t = new P(f.Item1 - x1, f.Item2 - y1);
				t.S(sx, sy);
				if (0 <= t.X && t.X <= g.X && 0 <= t.Y && t.Y <= g.Y) ps.Add(t);
			}
			ps.Sort();
			if (g.X == 0 || g.Y == 0) return (g.X + g.Y) * 100.0 + (ps.Count > 0 ? 10 * PI - 20 : 0);
			var l = ps.Select(p => p.Y).ToArray();
			var c = LongestIncreasingSubsequence(l);
			if (c == Min(g.X, g.Y) + 1) return (g.X + g.Y) * 100.0 - (c - 1) * (20 - 5 * PI) + 10 * PI - 20;
			return (g.X + g.Y) * 100.0 - c * (20 - 5 * PI);
		}
...
class AVLTree<T> : IEnumerable<T>, ICollection<T>, ICollection, IEnumerable
{
	...
	public static void CheckAVL(Random rand, int N)
	{
		...
		//Console.WriteLine(avl);
		Console.WriteLine(insertErrors && deleteErrors ? "?\t??, ???????????????."
										: insertErrors ? "×\t??(?????)??????????."
													   : "×\t??(?????)??????????.");
		Console.WriteLine(avl.IsBalanced() ? "?\tAVL?????????????."
										   : "×\tAVL?????????????.");
		Console.WriteLine(avl.IsValidBinarySearchTree() ? "?\tAVL??????????????."
														: "×\tAVL???????????????.");
