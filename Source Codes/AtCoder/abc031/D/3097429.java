...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        for (int m = 0; m < (pow(3, K+1)); m++) {
            int[] len = new int[K + 1];
            for (int tm = m, i = 0; tm > 0 && i < K; i++) {
                len[i + 1] = tm % 3 + 1;
                tm /= 3;
            }
            ...
            for (int i = 0; i < N; i++) {
                int sum = 0;
                for (int si = 0; si < s[i].length; si++) {
                    sum += len[s[i][si]];
                }
                if (sum != word[i].length()) ok = false;
            }
            if (!ok) continue;
            //set??
            String[] dic = new String[K + 1];
            for (int i = 0; i < N; i++) {
                for (int si = 0, p = 0; si < s[i].length; si++) {
                    int v = s[i][si];
                    String ns = word[i].substring(p, p + len[v]);
                    p += len[v];
                    if (dic[v] == null) {
                        dic[v] = ns;
                    } else {
