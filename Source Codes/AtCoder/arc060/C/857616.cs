...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		{
			var r = new int[N];
			for (var i = 0; i < N; i++) r[i] = Func.FirstBinary(i, N, k => x[k] > x[i] + L) - 1;
			var rn = new int[20][]; // rn[x][i]=r[...[r[i]]...] r? 2^x?
			rn[0] = r;
			for (var x = 1; x < 20; x++)
			{
				rn[x] = new int[N];
				for (var i = 0; i < N; i++) rn[x][i] = rn[x - 1][rn[x - 1][i]];
			}
			foreach (var q in qs)
			{
				var a = q.Item1 - 1;
				var b = q.Item2 - 1;
				if (a > b) Func.Swap(ref a, ref b);
				pr.WriteLine(Func.FirstBinary(1, N, d =>
				{
					var p = a;
