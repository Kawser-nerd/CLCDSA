...
namespace CodeJam
{
    ...
    class InfiniteHouseOfPancakes : BaseGoogleJamProblem
    {
        ...
        int SolveLarge ( List<int> pancakes, int previousMax, int currentPancake, int maxValue)
        {
            string key = string.Format ( "{0}_{1}", previousMax, currentPancake);
            int result;
            if (currentPancake >= pancakes.Count)
                return previousMax;
            if (cache.TryGetValue(key, out result))
                return result;
            int min = maxValue;
            for (int i = currentPancake; i < pancakes.Count; i++)
            {
                var pancake = pancakes[i];
                if (pancake > 3 )
                {
                    for ( int j =1 ; j*j <= pancake; j++)
                    {
                        int newMax = pancake / j;
                        if (pancake % j > 0)
                            newMax++;
                        newMax = Max(newMax, previousMax);
                        var v = j - 1 + SolveLarge(pancakes, newMax, i + 1, maxValue) ;
                        if (v > min)
                        {
                            cache.Add(key, min);
                            return min;
                ...
                previousMax = Max(previousMax, pancake);
            }
            cache.Add(key, min);
            return min;
        }

        int SolveSmall(List<int> pancakes)
        {
            ...
            for (int i = 0; i < pancakes.Count; i++)
            {
                if (pancakes[i] > 3)
                {
                    ...
                    min = Min(min, 1 + SolveSmall(newPancakes));
                }
            }
            ...
            min = Min(min, 1 + SolveSmall(pancakes));
            cache.Add(key, min);
            return min;
        }
        ...
        int SolveSmall(int[] pancakes)
        {
            ...
            list.Sort(Compare);
            return SolveSmall(list);
        }
        int SolveLarge(int[] pancakes)
        {
            cache.Clear();
            var list = new List<int>(pancakes);
            list.Sort();
            return SolveLarge(list,0,0,list.Last());
        }
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            problem.Solve("c:\\temp\\B-large.in");
        }
    }
