...
public class Main {
    ...
    private void solve() {
        try {
            int N = sc.nextInt();
            ArrayList<Integer> dp = new ArrayList<>(Collections.nCopies(N, Integer.MAX_VALUE));
            int cnt = 0;

            for (int i = 0; i < N; i++) {
                int val = sc.nextInt();
                int i1 = lower_bound(dp, val);
                if (dp.get(i1) != Integer.MAX_VALUE) cnt++;
                dp.set(i1, val);
            }
    ...
    private static List<Integer> toListSorted(int[] arr) {
