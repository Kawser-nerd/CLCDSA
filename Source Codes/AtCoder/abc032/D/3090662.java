...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        if (max(w) <= 1000) {
            long[][] dv = new long[N + 1][N * 1000 + 1];
            fill(dv, -1);
            dv[0][0] = 0;
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < dv[0].length; j++) {
                    if (dv[i][j] < 0) continue;
                    int wi = (int) w[i];
                    int vi = (int) v[i];
                    dv[i + 1][j] = Math.max(dv[i + 1][j], dv[i][j]);
                    if (wi + j <= W) dv[i + 1][j + wi] = Math.max(dv[i + 1][j + wi], dv[i][j] + vi);
                }
            }
            System.out.println(max(dv));
            return;
        }
        long[][] dw = new long[N + 1][N * 1000 + 1];
        fill(dw, LINF);
        dw[0][0] = 0;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < dw[0].length; j++) {
                if (dw[i][j] >= LINF) continue;
                long wi = w[i];
                int vi = (int) v[i];
                dw[i + 1][j] = Math.min(dw[i + 1][j], dw[i][j]);
                if (dw[i][j] + wi <= W) {
                    dw[i + 1][j + vi] = Math.min(dw[i + 1][j + vi], dw[i][j] + wi);
                    chMax(j + vi);
                }
            }
        }
    ...
    static ArrayList<P> lis1;
    static ArrayList<P> lis2;
    ...
    public static void s1() {
        ...
        for (int m = 0; m < (1 << r); m++) {
            ...
            lis2.add(new P(sumv, sumw));
        }
        Collections.sort(lis1);
        Collections.sort(lis2);
        long flo = -1;
        for (int i = 0; i < lis1.size(); i++) {
            if (lis1.get(i).x <= flo) lis1.remove(i--);
            else flo = max(flo, lis1.get(i).x);
        }
        flo = -1;
        for (int i = 0; i < lis2.size(); i++) {
            if (lis2.get(i).x <= flo) lis2.remove(i--);
            else flo = max(flo, lis2.get(i).x);
        }
        long res = 0;
        int N1 = lis1.size();
        int N2 = lis2.size();
        for (int i = 0; i < N1; i++) {
            if (lis1.get(i).y > W) continue;
            int resi = mgr(0, N2, lis1.get(i).y);
            long v1 = lis1.get(i).x;
            long v2 = lis2.get(resi).x;
            res = Math.max(res, v1 + v2);
        }
        System.out.println(res);
    }

    public static boolean calc(long va, long w) {
        //?????????????????
        int i = (int) va;
        long w2 = lis2.get(i).y;
        return w + w2 <= W;
    }
