...
public sealed partial class Program
{
    ...
    void Consume(ref int x, out bool isPortrait)
    {
        if (ss[0][x] == ss[1][x])
        {
            isPortrait = true;
    ...
    {
        var x = 0;
        var isPortrait = default(bool);

        Consume(ref x, out isPortrait);

        var k = isPortrait ? 3L : 6L;
        var previousIsPortrait = isPortrait;
    ...
    void Read()
    {
        var a = scanner;
        n = a.N();
        ss = 2.MakeArray(_ => input.ReadLine());
    }
