...
namespace ConsoleApplication
{
    class MainClass
    {
        ...
        void Solve() {
            ...
            //100
            if (D + L == X * Y) {
                var xl = (R - X + 1L) * (C - Y + 1L);
                var dl = mat.C(X * Y, Math.Min(D, L));
                ans = (xl * dl) % mat.mod;
            }
            //101
            else {
                Func<long, long, long> f1 = (a, b) => mat.C(a * b, D);
                Func<long, long, long> f2 = (a, b) => mat.C(a * b - D, L);
                Func<long, long, long> f = (a, b) => (f1(a, b) * f2(a, b)) % mat.mod;
                ...
                ans %= mat.mod;
                ans = (((ans * (R - X + 1L)) % mat.mod) * (C - Y + 1L)) % mat.mod;
            }
            //ret
            io.o(ans);
        }
    ...
    class Mat
    {
        ...
        public long Gcd(long a, long b) => a < b ? Gcd(b, a) : b == 0 ? a : Gcd(b, a % b);
        public long Gcd(params long[] a) => a.Aggregate((v, n) => Gcd(v, n));
    ...
    static class ArrayEx
    {
        ...
        public static int testLowerBound<T>(this T[] a, T x) where T : IComparable
            => a.BinarySearch(-1, a.Length, (mid) => a[mid].CompareTo(x) >= 0);
        public static int testUpperBound<T>(this T[] a, T x) where T : IComparable
            => a.BinarySearch(-1, a.Length, (mid) => a[mid].CompareTo(x) > 0);
        ...
        public static int ItemsInRange<T>(this T[] a, T from, T to) where T : IComparable
            => a.UpperBound(to) - a.LowerBound(from);
    }
    static class FunctionalEx
    {
        public static Func<T, TV> Compose<T, TU, TV>(this Func<TU, TV> f, Func<T, TU> g) { return x => f(g(x)); }
    }
    static class BitEx
    {
