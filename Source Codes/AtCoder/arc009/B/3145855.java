...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        P[] p = new P[N];
        for (int i = 0; i < N; i++) {
            p[i] = new P(replace(ori[i], hen), i);
        }
        sort(p);
        for (int i = 0; i < N; i++) {
            System.out.println(ori[p[i].y]);
        }
    }

    public static int replace(int N, int[] hen) {
        String res = N + "";
        for (int i = 0; i < 10; i++) {
            res = res.replace(i + "", (char) ('a' + hen[i]) + "");
        }
        for (int i = 0; i < 10; i++) {
            res = res.replace((char) ('a' + i) + "", i + "");
        }
