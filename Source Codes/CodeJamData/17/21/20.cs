            ...
            public List<string> SolveAll(int threadId, StreamReader sr) {
                ...
                for (int testCaseIx = 0; testCaseIx < TestCaseCount; testCaseIx++) {
                    ...
                    for (int i = 0; i < N; i++) {
                        ...
                        Horse h = new Horse { K = hl[0], S = hl[1] };
                        hs.Add(h);
                    }
                    // ------------ 読み込み ------------
                    #region
                    ...
                    #endregion
                    // ------------ 呼び出し ------------
                    string output = Solve(testCaseIx, D, N, hs);
                    // ------------ 呼び出し ------------
                    outputs.Add(output);
                }
                return outputs;
            }
            string Solve(int testCaseIx, int D, int N, List<Horse> hs) {
                double max = double.MaxValue;
                foreach(var h in hs) {
                    double hrs = (double)(D - h.K) / h.S;
                    double s = (double)D / hrs;
                    if (s < max) {
                        max = s;
                    }
                }
