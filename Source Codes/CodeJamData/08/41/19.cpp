...
RealCase::RealCase()
{
    ...
    C = VI(M, -1);
    I = VI(M, -1);
    REP(n, (M-1)/2) cin >> G[n] >> C[n];
    FOR(n, (M-1)/2, M) cin >> I[n];
}

const int inf = 999999999;

int RealCase::make(int n, int goal)
{
    if (I[n] >= 0) return I[n] == goal ? 0 : inf;
    ...
    if (G[n] == 0 || C[n]) {    /* OR gate */
        if (goal == 0) {
            res = min(res, (G[n] != 0) + make(c1, goal) + make(c2, goal));
        } else {
            res = min(res, (G[n] != 0) + min(make(c1, goal), make(c2, goal)));
        }
    }
    if (G[n] == 1 || C[n]) {
        if (goal == 0) {
            res = min(res, (G[n] != 1) + min(make(c1, goal), make(c2, goal)));
        } else {
            res = min(res, (G[n] != 1) + make(c1, goal) + make(c2, goal));
        }
    }
    return m = res;
}
