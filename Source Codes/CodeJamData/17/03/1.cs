        ...
        class Solver {
            public List<string> SolveAll(int threadId, StreamReader sr) {
                ...
                for (int testCaseIx = 0; testCaseIx < TestCaseCount; testCaseIx++) {
                    // ------------ 読み込み ------------
                    List<long> cs = SplitToLongs(sr.ReadLine());
                    // ------------ 読み込み ------------
                    #region
                    ...
                    #endregion
                    // ------------ 呼び出し ------------
                    string output = Solve(testCaseIx, cs[0], cs[1]);
                    // ------------ 呼び出し ------------
                    outputs.Add(output);
                }
                return outputs;
            ...
            class StallsCount {
                public long N;
                public long Count;
                public StallsCount(long n, long count) {
                    N = n;
                    Count = count;
                }
            ...
            string Solve(int testCaseIx, long N, long K) {
                List<StallsCount> ss = new List<StallsCount>();
                ss.Add(new StallsCount(N, 1));
                long k = K;
                for(;;) {
                    StallsCount s = ss[0];
                    long min = (s.N - 1) / 2;
                    ...
                    ss.RemoveAt(0);
                    if (k > s.Count) {
                        StallsCount smin = new StallsCount(min, s.Count);
                        StallsCount smax = new StallsCount(max, s.Count);
                        ss.Add(smin);
                        ss.Add(smax);
                        k -= s.Count;
                        ss = ss.OrderByDescending(s1 => s1.N).ToList();
                        for (int i = 0; i < ss.Count - 1; i++) {
                            if (ss[i].N == ss[i + 1].N) {
                                ss[i].Count += ss[i + 1].Count;
                                ss.RemoveAt(i + 1);
                                i--;
                            }
                        }
            ...
            string Naive(int testCaseIx, long N, long K) {
                ...
                for (long k = 0; k < K; k++) {
                    ...
                    s.Add(smin);
                    s.Add(smax);
                    s = s.OrderByDescending(si => si).ToList();
                }
                return smax + " " + smin;
            }
        }
