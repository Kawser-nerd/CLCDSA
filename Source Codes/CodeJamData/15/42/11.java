...
//	static String INPATH = BASEPATH + TParallel.class.getSimpleName().charAt(0) + (LARGE ? "-large-practice.in" : "-small-practice.in");
	...
	{
		Arrays.sort(rc, new Comparator<long[]>() {
		...
		// (num-rc[i][0]*
		if(num < 0){
			for(int i = 0;i < n-1 && rc[i][1]-x < 0;i++){
				if(num - rc[i][0]*(rc[i][1]-x) >= 0){
					double rem = (double)num/(rc[i][1]-x)/rc[i][0];
					double dden = den - rc[i][0]*rem;
					out.printf("%.12f\n", v/dden);
					return;
				}
				num -= rc[i][0]*(rc[i][1]-x);
			...
			for(int i = n-1;i >= 1 && rc[i][1]-x > 0;i--){
				if(num - rc[i][0]*(rc[i][1]-x) <= 0){
					double rem = (double)num/(rc[i][1]-x)/rc[i][0];
					double dden = den - rc[i][0]*rem;
					out.printf("%.12f\n", v/dden);
