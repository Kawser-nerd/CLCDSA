...
namespace Practice
{
    class Program
    {
        ...
        private static int Salary( List<int>[] g, int myIdx)
        {
            List<int> sub = g[myIdx];
            if( sub.Count == 0) { return 1; }
            else
            {
                int min = int.MaxValue;
                int max = 0;
                foreach(int subIdx in sub)
                {
                    int s = Salary(g, subIdx);
                    min = Math.Min(min, s);
                    max = Math.Max(max, s);
                }
                return max + min + 1;
