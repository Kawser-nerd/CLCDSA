...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		{
			var sum = new int[N + 1];
			for (var i = 0; i < N; i++) sum[i + 1] = sum[i] + a[i];
			if (sum[N] == K) { pr.WriteLine(1); return; }
			var dict = new int[N + 1];
			dict.MemberSet(Func.Inf);
			dict[0] = 0;
			for (var i = 0; i < N; i++)
			{
				var next = new int[N + 1];
				next.MemberSet(Func.Inf);
				for (var j = 0; j <= N; j++)
					if (dict[j] < Func.Inf)
					{
						next[j] = Math.Min(next[j], dict[j]);
						var z = i == 0 ? 1 : (int)(((long)dict[j] * a[i]) / sum[i]) + 1;
						if (z <= a[i]) next[j + 1] = Math.Min(next[j + 1], dict[j] + z);
					}
				dict = next;
			}
			for (var i = N; i >= 0; i--) if (dict[i] <= K) { pr.WriteLine(i); return; }
		}
	}
}
