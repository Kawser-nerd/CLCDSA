...
int solve(int p,int q,int prev)
{
	int state=(p*n+q)*n+prev;
	if (M.find(state)!=M.end())
		return M[state];
	int R=1;
	int id=ID(p,q);
	for (int i=0;i<SIZE(g[id]);i++)
	{
		int key=g[id][i];
		if (key==prev)
			continue;
		int other=a[key][0]+a[key][1]+a[key][2]-p-q;
		int t1=solve(other,p,key);
		int t2=solve(other,q,key);
		checkmax(R,t1+t2);
	}
	return M[state]=R;
}
...
//	freopen("B-small-attempt2.in","r",stdin);freopen("B-small-attempt2.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int i=0;i<n-2;i++)
		{
			g[ID(a[i][0],a[i][1])].push_back(i);
			g[ID(a[i][1],a[i][2])].push_back(i);
			g[ID(a[i][2],a[i][0])].push_back(i);
		}
		M.clear();
		int R=0;
		for (int i=0;i<n-2;i++)
		{
			int R1=solve(a[i][0],a[i][1],i);
			int R2=solve(a[i][1],a[i][2],i);
			int R3=solve(a[i][2],a[i][0],i);
			checkmax(R,R1+R2+R3);
		}
		printf(" %d\n",R);
/*
		if (n<=9)
		{
			...
			for (int i=3;i<n;i++)
			{
				G[i][a[i]]=G[a[i]][i]=true;
				G[i][b[i]]=G[b[i]][i]=true;
			}
			int P[10];
			for (int i=0;i<n;i++)
