...
void solve() {
  ...
  int t = 1, s = 0;
  while (t > 0) {
    // fprintf(stderr, "%d %d : ", t, s);
    // rep (i, has.size()) fprintf(stderr, "%d ", has[i]); fprintf(stderr, "\n");

    // T > 0 -> use it !
    rep (i, has.size()) {
      if (T[has[i]] > 0) {
        // fprintf(stderr, "really good: %d (%d)\n", has[i], T[has[i]]);
        t += T[has[i]];
        s += S[has[i]];
        rep (c, C[has[i]]) if (nxt < K) has.push_back(nxt++);
    ...
    // Hope!!
    {
      pair<int, int> tmp = mp(-1, 0);
      rep (i, has.size()) tmp = max(tmp, mp(C[has[i]], S[has[i]]));
      if (tmp.first <= 0) {
        break;  // Die!!
      }
      rep (i, has.size()) {
        if (tmp == mp(C[has[i]], S[has[i]])) {
          has.erase(has.begin() + i);
          break;
        }
      }
      ...
      --t;
      s += tmp.second;
      rep (c, tmp.first) if (nxt < K) has.push_back(nxt++);
    }
