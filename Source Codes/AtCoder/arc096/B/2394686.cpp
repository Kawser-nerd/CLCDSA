...
int main() {
    ...
    set<pair<i64, i64>> m1, m2;

    for(int i : rep(0, n + 2)) {
        m1.insert({b[i] - (c - x[i]), c - x[i]});
        m2.insert({a[i] - x[i], x[i]});
    }

    i64 ans = 0;

    for(int i : rep(0, n + 1)) {
        m1.erase({b[i] - (c - x[i]), c - x[i]});
        ans = max(ans, m1.rbegin()->first + a[i] - x[i] * 2);
    }

    for(int i : rep(1, n + 2) | reversed) {
        m2.erase({a[i] - x[i], x[i]});
        ans = max(ans, m2.rbegin()->first + b[i] - (c - x[i]) * 2);
    }
    out(ans);
}