...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int c = io.nextInt();
        int k = io.nextInt();
        char map[][] = io.charMap(r, c);
        // map[r][c]?????????????????????????
        int cnt[][][] = new int[r][c][2];
		for (int y = 0; y < r; y++) {
			for (int x = 0; x < c; x++) {
				int i = 0;
				while (y-i>=0 && map[y-i][x]=='o') {
					cnt[y][x][0]++;
					i++;
				}
				int j = 0;
				while (y+j<r && map[y+j][x]=='o') {
					cnt[y][x][1]++;
					j++;
				}
			}
		...
		for (int y = k-1; y <= r-k; y++) {
			r: for (int x = k-1; x <= c-k; x++) {
				for (int i = 0; i < k; i++) {
					if (cnt[y][x-i][0]<k-i || cnt[y][x-i][1]<k-i
							|| cnt[y][x+i][0]<k-i || cnt[y][x+i][1]<k-i) continue r;
				}
				ans++;
			}
		}
