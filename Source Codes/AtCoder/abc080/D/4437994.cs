...
namespace Practice
{
    class Program
    {
        ...
        static void Solve()
        {
            ...
            var t = new int[n];
            var c = new int[n];
            var ctimeTable = new int[kMax,C+1];
            for(int i=0;i<n;++i)
            {
                o = ReadAndParseIntArr();
                for(int j=o[0]-1;j<=o[1]-1;++j)
                {
                    ctimeTable[j, o[2]] = 1;
                }
            }
            ...
            int ans = 0;
            for(int i=0;i<ctimeTable.GetLength(0);++i)
            {
                int overrap = 0;
                for(int j=0;j<ctimeTable.GetLength(1);++j)
                {
                    if(ctimeTable[i,j] == 1)
                    {
                        overrap++;
