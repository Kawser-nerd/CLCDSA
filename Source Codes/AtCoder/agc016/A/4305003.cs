...
namespace Practice
{
    class Program
    {
        ...
        static void Solve()
        {
            ...
            for(char c = 'a'; c <= 'z';++c)
            {
                ans = Math.Min(ans, Shrink(s.ToCharArray(), c));
            }
            
            Console.WriteLine(ans);
        }
        private static int Shrink(char[] a, char target)
        {
            int ans = 0;

            var work = a;
            var work2 = new char[work.Length];
            int len = work.Length;


            while (!IsSame(work, len))
            {
                --len;
                for (int i = 0; i < len; ++i)
                {
                    char c = work[i];
                    if (work[i] == target) { c = target; }
                    else if (work[i + 1] == target) { c = target; }
                    work2[i] = c;
                }
        ...
        private static bool IsSame(char[] a, int len)
        {
            for(int i=1;i<len;++i)
            {
