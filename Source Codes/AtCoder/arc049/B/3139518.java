...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        boolean zero = true;
        for (int i = 1; i < N; i++) {
            if (x[i - 1] != x[i]) zero = false;
            if (y[i - 1] != y[i]) zero = false;
        }
        if (zero) {
    ...
    public static boolean nol(double sec) {
        {
            ...
            double pl = w[0][0], pr = w[0][1];
            for (int i = 1; i < N; i++) {
                double nl = w[i][0], nr = w[i][1];
                if (pr < nl || nr < pl) return false;
                pl = Math.max(pl, nl);
                pr = Math.min(pr, nr);
            }
        }
        {
            ...
            double pd = h[0][0], pu = h[0][1];
            for (int i = 1; i < N; i++) {
                double nd = h[i][0], nu = h[i][1];
                if (pu < nd || nu < pd) return false;
                pd = Math.max(pd, nd);
                pu = Math.min(pu, nu);
            }
        }
    ...
    public static String notE(double d) {
...
} ./Main.java:86: warning: '_' used as an identifier
        for (int _ = 0; _ < 1000; _++) {
                 ^
