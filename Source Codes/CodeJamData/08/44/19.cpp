...
struct RealCase : public Case
{
    ...
    int calc(int l, int off, int av);
    inline char get(int b, int o);

    int K, N, B;  // K, SZ(S), SZ(S)/K
    string S;
};

Case *read_case() { return new RealCase(); }
RealCase::RealCase() { cin >> K >> S; }
...
inline char RealCase::get(int b, int o)
{
    return S[b*K + perm[o]];
}

int RealCase::calc(int l, int off, int av)
{
    int &m = memo[l][off][av];
    if (m >= 0) return m;
    ...
    if (av == 0) {
        int res = 0;
        FOR(b, 1, B) res += (get(b - 1, K-1) != get(b, 0));
        //assert(off == K);
        return m = res;
    } else {
        //assert(off < K);
        ...
        REP(x, K) if (av&BIT(x)) {
            perm[off] = x;
            int v = cost[l][x];
            //REP(b, B) v += (get(b, off) != get(b, off - 1));
            v += calc(x, off + 1, av^BIT(x));
            res = min(res, v);
        }
        return m = res;
    }
...
void RealCase::solve(Result &result)
{
    ...
    REP(i, K) REP(j, K) if (i != j) {
        perm[0] = i;
        perm[1] = j;
        REP(b, B) cost[i][j] += (get(b,0) != get(b,1));
    }
    ...
    REP(x, K) {
        SET(memo, -1);
        perm[0] = x;
        res = min(res, calc(x, 1, ((1<<K)-1)^BIT(x)));
    }
    result.set(STR(res + 1));
}
