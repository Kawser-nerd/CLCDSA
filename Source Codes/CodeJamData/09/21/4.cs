...
class Tree
{
    ...
    public Tree A, B;
    public double Interpret(string[] features)
    {
        if (feature == null) return w;
        if (features.Contains(feature)) 
            return w * A.Interpret(features);
        else
            return w * B.Interpret(features);
    }
}
...
partial class Solution
{
    ...
    Tree ParseTree(string[] input, ref int pos)
    {
        Debug.Assert(input[pos++] == "(");
        Tree t = new Tree();
        t.w = Double.Parse(input[pos++], CultureInfo.InvariantCulture);
        if (input[pos] == ")") { pos += 1; return t; }
        t.feature = input[pos++];
        t.A = ParseTree(input, ref pos);
        t.B = ParseTree(input, ref pos);
        Debug.Assert(input[pos++] == ")");
        return t;
    }
    ...
    void Solve()
    {
        ...
        lines = lines.Replace("(", " ( ");
        lines = lines.Replace(")", " ) ");
        string[] parsed = lines.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
        int pos = 0;
        Tree t = ParseTree(parsed, ref pos);

        WriteLine("Case #{0}:", cas);
        int A = ReadLine_int();
        for (int i = 0; i < A; i++)
        {
            string[] input = SplitLine();
            string[] features = input.Skip(2).ToArray();
            WriteLine("{0:0.0000000}", t.Interpret(features));
        }
