...
public sealed partial class Program
{
    ...
    private int ScoreCore(int picker, int i)
    {
        var last = Math.Abs(hand[1 - picker][i] - hand[picker][n - 1]);
        ...
        if (picker == X)
        {
            ...
            {
                min = Math.Max(min, Score(1 - picker, j));
            }

            return Math.Max(min, last);
        }
        else
        {
            ...
            for (var j = i + 1; j < n - 1; j++)
            {
                max = Math.Min(max, Score(1 - picker, j));
            }
    ...
    private int Score(int picker, int i)
    {
        var score = memo[picker][i];

        if (score == -1)
        {
            score = ScoreCore(picker, i);
            memo[picker][i] = score;
        }
    ...
    private long Solve()
    {
        memo = 2.MakeArray(p => n.MakeArray(i => -1));
        return Score(X, 0);
    }
    ...
    private void Read()
    {
        ...
        var z = a.N();
        var w = a.N();
        var vs = m.MakeArray(i => a.N());

        n = m + 1;
        hand = 2.MakeArray(p => n.MakeArray(i => i == 0 ? (p == X ? z : w) : vs[i - 1]));
    }
