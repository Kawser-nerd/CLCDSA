...
partial class Solution
{
    ...
    void Solve()
    {
        ...
        int[] x0 = SplitLine_int().ToArray();
        int[] v = SplitLine_int().ToArray();

        List<Chick> chicks = new List<Chick>();
        for (int i = 0; i < x0.Length; i++)
        {
            chicks.Add(new Chick { start = x0[i], end = x0[i] + v[i]*T });
        }
        foreach (Chick c in chicks)
        {
            if (c.end < B) { c.cross = 2000000000; continue; }
            foreach (Chick c2 in chicks)
            {
                if (c2.start > c.start && c2.end < c.end && c2.end<B) c.cross += 1;
            }
        }
        chicks = chicks.OrderBy(c => c.cross).ToList();
        if (K>0 && chicks[K-1].cross==2000000000)
            WriteLine("Case #{0}: {1}", cas, "IMPOSSIBLE");
        else
            WriteLine("Case #{0}: {1}", cas, chicks.Take(K).Select(c=>c.cross).Sum());
    }
