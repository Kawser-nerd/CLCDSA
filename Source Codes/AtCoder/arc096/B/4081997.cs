...
namespace Contest
{
    ...
    class Program
    {
        ...
        private void Scan()
        {
            ...
            N = sc.NextInt();
            C = sc.NextLong();
            sc.GetLongColumn(N, out x, out v);
        }
        ...
        public void Solve()
        {
            ...
            Console.WriteLine(Math.Max(0, Math.Max(Right(), Left())));
        }
        ...
        private long Right()
        {
            ...
            for (int i = N - 1; i >= 0; i--)
            {
                var l = LeftSum(i) - (C - x[i]);
                left[i] = Math.Max(left[i + 1], l);
            }

            ans = left[0];
            for (int i = 1; i <= N; i++)
            {
                var r = RightSum(i) - x[i - 1] * 2 + left[i];
                ans = Math.Max(ans, r);
            }
        ...
        private long Left()
        {
            ...
            for (int i = 1; i <= N; i++)
            {
                var r = RightSum(i) - x[i - 1];
                right[i] = Math.Max(right[i - 1], r);
            }
            ...
            for (int i = N - 1; i >= 0; i--)
            {
                var l = LeftSum(i) - 2 * (C - x[i]) + right[i];
                ans = Math.Max(ans, l);
            }
