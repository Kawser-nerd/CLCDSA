...
namespace AtCoder {
    class MainClass {
        ...
        #region MockMacro
        //Hack Update
        //Action<int, Action<int>> R = (n, act) => { for (int i = 0; i < n; i++) act(i); };
        ...
        void RFOR(int a, int b, Action<int> act) { for (int i = a; i >= b; i--) act(i); }
        void RFOR(int a, int b, Action<int> act, Func<int, bool> con) { for (int i = a; i >= b && con(i); i--) act(i); }
