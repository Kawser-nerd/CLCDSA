...
namespace ConsoleApplication {
    class MainClass {
        ...
        void Solve() {
            ...
            long addDmg = A - B;
            Func <long, bool> check = (num) => {
                long boarder = B * num;
                long addAtkNum = 0;
                for(int i = 0; i < h.Length; ++i) {
                    var left = h[i] - boarder;
                    if(left > 0) addAtkNum += left / addDmg + (left % addDmg == 0 ? 0 : 1);
                }
                return addAtkNum <= num;
            };
            
            io.o(h.Sort().BinarySearch(-1, (int)1e9, check));            
        }
