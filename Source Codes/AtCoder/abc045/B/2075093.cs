...
namespace SandboxCSharp
{
    class Solver
    {
        public void Solve()
        {
            var a = io.ScanString().Reverse().ToList();
            var b = io.ScanString().Reverse().ToList();
            var c = io.ScanString().Reverse().ToList();
            ...
            while (true)
            {
                if (state == 0)
                {
                    if (a.Count == 0) { io.WriteLine("A"); Exit(); }
                    state = a.Last()- 'a';
                    a.RemoveAt(a.Count - 1);
                }
                else if (state == 1)
                {
                    if (b.Count == 0) { io.WriteLine("B"); Exit(); }
                    state = b.Last() - 'a';
                ...
                else if (state == 2)
                {
                    ...
                    if (c.Count == 0) { io.WriteLine("C"); Exit(); }
                    state = c.Last() - 'a';
