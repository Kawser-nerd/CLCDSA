...
int main() {
    ...
    auto pos = make_vector<int, 2>(26, 0);

    for (int i : rep(0, n)) {
        pos[a[i] - 'a'].push_back(i);
    }

    const int inf = 1LL << 26;

    vector<int> dp(n + 1, inf);
    dp[n] = 1;
    for (int i : rep(0, n) | reversed) {
        for (int j : rep(0, 26)) {
            auto it = lower_bound(pos[j], i);
            if (it == pos[j].end()) {
                dp[i] = 1;
            } else {
                dp[i] = min(dp[i], dp[*it + 1] + 1);
            }
        }
    }
    ...
    while (p < n) {
        for (int i : rep(0, 26)) {
            auto it = lower_bound(pos[i], p);
            if (it == pos[i].end()) {
                ans += 'a' + i;
                p = n;
                break;
            } else if (dp[p] == dp[*it + 1] + 1) {
                ans += 'a' + i;
                p = *it + 1;
                break;
            }
