#define $ i>>--r&1|r>4&r<20&(i>>r-5^i>>r+5)|r%5>0&r%5<4&(i>>r-1^i>>r+1)||(p[r=i|1<<r]+=p[i],p[r]%=j=1e9+7)
p[1<<25],x[26],G[20],i,j,r;main(a){for(*p=1;~scanf("%d",&a);)a?x[a]=++r:(G[j++]=++r);for(;i<1<<25;i++)if(r=x[__builtin_popcount(i)+1])$;else for(a=0;r=G[a++];$);printf("%d\n",p[i-1]);} ./Main.c:2:1: warning: data definition has no type or storage class
 p[1<<25],x[26],G[20],i,j,r;main(a){for(*p=1;~scanf("%d",&a);)a?x[a]=++r:(G[j++]=++r);for(;i<1<<25;i++)if(r=x[__builtin_popcount(i)+1])$;else for(a=0;r=G[a++];$);printf("%d\n",p[i-1]);}
 ^
