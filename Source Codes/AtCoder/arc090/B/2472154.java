...
public class Main {
    ...
    public static void main(String[] args){
    	int n=ni(),m=ni();
    	int[] r=new int[m],l=new int[m];
    	int[] d=new int[m];
    	boolean[] isd=new boolean[n+1];
    	Arrays.fill(isd,true);
    	int count=0;
    	for(int i=0;i<m;i++){
    		l[i]=ni();
    		r[i]=ni();
    	...
    	while(count<n){
    		...
    		//pl(Arrays.toString(pos));
    	}
    	boolean res=true;
    	for(int i=0;i<m;i++){
    		int sub=pos[r[i]]-pos[l[i]];
    		if(sub!=d[i]){
    			res=false;
    			break;
    		}
	...
	static int[][][] ad(int n,int[] a,int[] b,int[] d){
		int[][][] res = new int[n+1][][];
		int[] c=new int[n+1];
		...
		for(int i=0;i<a.length;i++){
			int from=a[i],to=b[i];
			res[from][c[from]][1]=d[i];
			res[to][c[to]][1]=-d[i];
			res[from][c[from]++][0]=to;
			res[to][c[to]++][0]=from;
		}
		return res;
	}
