...
namespace GoogleCodeJam2017
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var reader = new StreamReader("input.txt"))
            {
                ...
                {
                    var task = new RollerCoasterScheduling(i + 1);
    ...
    #region ProbB
    class RollerCoasterScheduling : GCJTask
    {
        public RollerCoasterScheduling(int caseNumber) : base(caseNumber)
        {
        }
        ...
        public override void ReadInput(StreamReader input)
        {
            ...
            M = int.Parse(tokens[2]);

            tickets = new List<Tuple<int, int>>(M);
            for(int i = 0; i < M; i++)
            {
                tokens = input.ReadLine().Trim().Split();
                var iTokens = tokens.Select(item => int.Parse(item) - 1).ToArray();
                tickets.Add(Tuple.Create(iTokens[0], iTokens[1]));
            }
        }
        ...
        {
            int[] seats = new int[N];
            int[] customers = new int[C];

            minRides = 0;
            foreach(var ticket in tickets)
            {
                seats[ticket.Item1]++;
                customers[ticket.Item2]++;
                minRides = Math.Max(minRides, customers[ticket.Item2]);
            }

            int totalTickets = 0;
            for(int i = 0; i < N; i++)
            {
                totalTickets += seats[i];
                int reqRiders = (int)Math.Ceiling(totalTickets / (i + 1.0));
                minRides = Math.Max(reqRiders, minRides);
            }

            minProm = 0;
            for(int i = 0; i < N; i++)
            {
                minProm += Math.Max(0, seats[i] - minRides);
            }
        ...
        public override void WriteOutput(StreamWriter output)
        {
            output.WriteLine($"Case #{caseNumber}: {minRides} {minProm}");
        }
    }
    #endregion
}
