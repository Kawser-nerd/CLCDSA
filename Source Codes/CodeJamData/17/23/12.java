...
public class C implements Runnable{
    ...
    private final static String fileName = "C-large";
    ...
    final static long INF = 1000L * 1000L * 1000 * 1000 * 1000;
    ...
    private void solveTest() {
        ...
        for (int i = 0; i < n; ++i) {
            maxDistances[i] = readInt();
            velocities[i] = readInt();
        }

        long[][] graph = new long[n][n];
        for (int i = 0; i < n; ++i) {
            graph[i] = readLongArray(n);
        }
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                if (maxDistances[i] >= graph[i][j]) {
                    times[i][j] = 1.0 * graph[i][j] / velocities[i];
                } else {
                    times[i][j] = INF;
                }
            }
        ...
        final long maxPossibleTime = n * 1L * 1000 * 1000 * 1000;

        while (q --> 0) {
            int from = readInt() - 1;
            int to = readInt() - 1;

            double time = times[from][to];
            if (time > maxPossibleTime) {
                throw new RuntimeException("Impossible way: " + from + " -> " + to);
            } else {
                out.print(time + " ");
            }
        }
