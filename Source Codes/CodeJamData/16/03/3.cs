        ...
        class Solver {
            public List<string> SolveAll(int threadId, StreamReader sr) {
                ...
                for (int testCaseIx = 0; testCaseIx < TestCaseCount; testCaseIx++) {
                    ...
                    #endregion
                    // ------------ 呼び出し ------------
                    string output = Solve(testCaseIx, N, J);
                    // ------------ 呼び出し ------------
                    outputs.Add(output);
                }
                return outputs;
            }
            string Solve(int testCaseIx, int N, int J) {
                string ans = "";
                int count = 0;
                for (long i = 0; i < 1 << (N - 2); i++) {
                    long a = (1L << (N - 1)) + (i << 1) + 1L;
                    string s = Check(a, 10000);
                    if (s != null) {
                        ans += Environment.NewLine + Convert.ToString(a, 2) + " " + s;
                        count++;
                        if (count >= J) {
                            break;
                        }
            ...
            static string Check(long a, BigInteger limit) {
                List<BigInteger> dvs = new List<BigInteger>();
                for (int j = 2; j <= 10; j++) {
                    BigInteger c = ConvertBase(a, j);
                    BigInteger dv = Divisor(c, limit);
                    if (dv == c) {
                        return null;
                    }
                    dvs.Add(dv);
                }
                return string.Join(" ", dvs);
            }
            static BigInteger ConvertBase(long base2, int newBase) {
                BigInteger b = 1;
                BigInteger ans = 0;
                for (long i = 1; i <= base2; i *= 2, b *= newBase) {
                    if ((base2 & i) != 0) {
                        ans += b;
                    }
                }
            ...
            static BigInteger Divisor(BigInteger a, BigInteger limit) {
                if (a % 2 == 0) {
                    return 2;
                }
                for (BigInteger p = 3; p <= limit; p += 2) {
                    if (a % p == 0) {
                        return p;
                    }
                }
