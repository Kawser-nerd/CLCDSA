...
public class ProblemC {
	...
	public class Case {
		...
		public void solve(int caseIndex) {
			...
			if (height == 1) {
				println("Case #" + (caseIndex + 1) + ": " + areaRequired);
				return;
			}

			int bestStones = Integer.MAX_VALUE;
			for (int y = 0; y <= height - 2; y++) {
				for (int x = 0; x <= width - 2; x++) {
					for (int k = 0; k <= 4; k++) {
						for (int i = 0; i <= 4; i++) {
							int area = x * y + 2 * x + 2 * y + i - 2 * k;
							if (area >= areaRequired) {
								int stones = 2 * x + 2 * y + i - k;
								bestStones = Math.min(stones, bestStones);
							}
						}
					}
				}
...
//			if (bestStones == Integer.MAX_VALUE) {
//				System.out.println(""+(caseIndex)+" "+width+"x"+height+", "+areaRequired);
//			}

			println("Case #" + (caseIndex + 1) + ": " + bestStones);
		}
