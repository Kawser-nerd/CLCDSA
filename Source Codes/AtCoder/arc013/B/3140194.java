            ...
            for (int j = 0; j < 101; j++) {
                for (int k = 0; k < 101; k++) {
                    ...
                    for (int ni = 0; ni < N; ni++) {
                        int a1 = a[ni], b1 = b[ni], c1 = c[ni];
                        int a2 = a[ni], b2 = c[ni], c2 = b[ni];
                        int a3 = b[ni], b3 = a[ni], c3 = c[ni];
                        int a4 = b[ni], b4 = c[ni], c4 = a[ni];
                        int a5 = c[ni], b5 = a[ni], c5 = b[ni];
                        int a6 = c[ni], b6 = b[ni], c6 = a[ni];
                        if (a1 <= i && b1 <= j && c1 <= k) continue;
                        if (a2 <= i && b2 <= j && c2 <= k) continue;
                        if (a3 <= i && b3 <= j && c3 <= k) continue;
                        if (a4 <= i && b4 <= j && c4 <= k) continue;
                        if (a5 <= i && b5 <= j && c5 <= k) continue;
                        if (a6 <= i && b6 <= j && c6 <= k) continue;
                        ok = false;
                        break;
                    }
                    if (ok) chMin(i * j * k);
