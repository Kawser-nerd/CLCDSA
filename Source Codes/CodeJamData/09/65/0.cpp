...
int n;
int A[maxn],B[maxn],C[maxn],H[maxn],Q[maxn];
bool G[maxn][maxn],S[maxn][maxn];

bool isIntersect(int a,int b,int c,int d)
{
	if (a>c) return isIntersect(c,d,a,b);
	return a<c && c<b && b<d;
}
bool isCover(int a,int b,int c,int d)
{
	return a<c && b>d;
}
int solve()
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int i=0;i<n;i++)
			for (int j=0;j<n;j++)
				if (i!=j)
				{
					G[i][j]=isIntersect(A[i],B[i],A[j],B[j]);
					S[i][j]=isCover(A[i],B[i],A[j],B[j]);
				}
