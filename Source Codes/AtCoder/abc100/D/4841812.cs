...
namespace AtCoder
{
    public class Program
    {
        ...
        private void Solve()
        {
            ...
            var tuple = x.Zip(y, Tuple.Create).Zip(z, (a, b) => Tuple.Create(a.Item1, a.Item2, b)).ToArray();
            ...
            for (int i = -1; i <= 1; i += 2)
            {
                for (int j = -1; j <= 1; j += 2)
                {
                    for (int k = -1; k <= 1; k += 2)
                    {
                        var sum = tuple.OrderByDescending(a => (a.Item1 * i) + (a.Item2 * j) + (a.Item3 * k)).ToArray();
                        var t = sum.Take(M);
                        long A = 0, B = 0, C = 0;
                        ...
                        chmax(ref max, Abs(A) + Abs(B) + Abs(C));
                    }
                }
            }
