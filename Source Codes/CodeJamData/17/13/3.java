...
public class Main {
    ...
    private String solve(Scanner scanner) {
        ...
        origin_hd=hd;
        int v=dfs(hd, ad, hk, ak, b, d, new HashMap<>());
        return v>1999999998?"IMPOSSIBLE":String.valueOf(v);
    }

    private int dfs(int hd, int ad, int hk, int ak, int b, int d, HashMap<Integer, Integer> map) {
        ...
        // attack
        num=Math.min(num, 1+dfs(hd-ak, ad, hk-ad, ak, b, d, map));
        // buff
        num=Math.min(num, 1+dfs(hd-ak, ad+b, hk, ak, b, d, map));
        // cure
        num=Math.min(num, 1+dfs(origin_hd-ak, ad, hk, ak, b, d, map));
        // debuff
        num=Math.min(num, 1+dfs(hd-decrease_ak, ad, hk, decrease_ak, b, d, map));
        map.put(state, num);
        return num;
    }
