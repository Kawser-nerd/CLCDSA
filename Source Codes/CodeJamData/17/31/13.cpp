...
double rectArea(double r, double h) {
	double w = 2 * pi * r;
	return w * h;
}
...
double f(int pos, int kleft) {
	double& res = dp[pos][kleft];
	if (res == res) return res;
	res = 0;
	double tmp;
	if (pos >= kleft) {
		tmp = rectArea(p[pos].r, p[pos].h);
		if (kleft == 1) {
			tmp += circleArea(p[pos].r);
			res = max(res, tmp);
		}
		else {
			res = max(res, f(pos - 1, kleft - 1) + tmp);
		}
	}

	if (pos > kleft) {
		res = max(res, f(pos - 1, kleft));
	}
