...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			sc.Read(out N);
			sc.Read(out A);
			a = sc.ReadManyLines<int, int>(A);
			sc.Read(out B);
			b = sc.ReadManyLines<int, int>(B);
		}
		void SolveOne()
		{
			var max = 0;
		...
		int Draw(bool[] bit)
		{
			var g = new DirectedGraph<bool, object>(N);
			for (var i = 0; i < A; i++) g.AddEdge(a[i].Item2 - 1, a[i].Item1 - 1, null);
			for (var j = 0; j < B; j++)
			{
				if (bit[j]) g[b[j].Item1 - 1] = true;
				else g.AddEdge(b[j].Item1 - 1, b[j].Item2 - 1, null);
			}
			var count = 0;
			var charge = new int[N];
			var used = new bool[N];
			var queue = new Queue<int>();
			for (var i = 0; i < N; i++) if (g.InDegree(i) == 0 && !g[i]) queue.Enqueue(i);
			while (queue.Count > 0)
			{
				var u = queue.Dequeue();
