s;n,b,c;main(a){for(scanf("%*d%d%d",&a,&b);~scanf("%s%d",&s,&c);n-=c*(s%4-2))c=c>b?b:c<a?a:c;printf("%s%d\n",n?n>0?"East ":"West ":"",abs(n));} ./Main.c:1:1: warning: data definition has no type or storage class
 s;n,b,c;main(a){for(scanf("%*d%d%d",&a,&b);~scanf("%s%d",&s,&c);n-=c*(s%4-2))c=c>b?b:c<a?a:c;printf("%s%d\n",n?n>0?"East ":"West ":"",abs(n));}
 ^
...
./Main.c:1:7: warning: type defaults to ‘int’ in declaration of ‘c’ [-Wimplicit-int]
 s;n,b,c;main(a){for(scanf("%*d%d%d",&a,&b);~scanf("%s%d",&s,&c);n-=c*(s%4-2))c=c>b?b:c<a?a:c;prin...