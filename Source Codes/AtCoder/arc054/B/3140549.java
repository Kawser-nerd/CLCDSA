...
public class Main {
    ...
    public static void solve() throws Exception {
        //long???????????????
        p = nd();
        double time = goldSectionSearchDeko(0, 1e18);
        System.out.println(calcg(time));
    }

    public static double calcg(double v) {
        return v + p * 1.0 / pow(2, v / 1.5);
    }

    public static double goldSectionSearchDeko(double left, double right) {
        double GRATIO = 1.6180339887498948482045868343656;
        double lm = left + ((right - left) / (GRATIO + 1.0));
        double rm = lm + ((right - lm) / (GRATIO + 1.0));
        double fl = calcg(lm);
        double fr = calcg(rm);
        for (int i = 0; i < 100000; i++) {
            if (left > right || lm > rm) break;
            if (fl < fr) {
                right = rm;
                rm = lm;
                fr = fl;
