...
namespace Contest
{
    ...
    class Program
    {
        ...
        private void Scan()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                sc.GetLongRow(out S[i].T, out S[i].D);
            }
        }

        PriorityQueue<Sushi> pq = new PriorityQueue<Sushi>((a, b) => -a.D.CompareTo(b.D));
        PriorityQueue<Sushi> pq2 = new PriorityQueue<Sushi>((a, b) => a.D.CompareTo(b.D));
        private Dictionary<long, int> map = new Dictionary<long, int>();
        public void Solve()
        {
            Scan();
            foreach (var sushi in S)
            {
                pq.Enqueue(sushi);
            }
            ...
            for (int i = 0; i < K; i++)
            {
                ...
                pq2.Enqueue(s);
                int o;
                map.TryGetValue(s.T, out o);
                map[s.T] = o + 1;
            }

            TCnt = map.Count;
            long ans = DSum + TCnt * TCnt;

            while (pq.Count > 0 && pq2.Count > 0)
            {
                ...
                while (pq2.Count > 0)
                {
                    ...
                    if (map[s.T] > 1)
                    {
                        while (pq.Count > 0)
                        {
                            var ss = pq.Dequeue();
                            int o;
                            map.TryGetValue(ss.T, out o);
                            if (o == 0)
                            {
                                TCnt++;
                                DSum += ss.D;
                                DSum -= s.D;
                                ans = Math.Max(ans, DSum + TCnt * TCnt);
                                map[s.T]--;
                                map[ss.T] = 1;
                                break;
                            }
