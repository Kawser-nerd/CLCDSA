...
static class MainClass
{
    public static void Main(string[] args)
    {
        ...
        var s = Console.ReadLine().ToCountedDictionary();
        BigInteger count = 1;
        foreach (var item in s)
        {
            count *= (item.Value + 1) % Mod1e9;
        }
        Console.WriteLine((count - 1) % Mod1e9);

        Console.ReadLine();

    }
    
    public static int sumofstr(string num)
    {
        var sum = 0;
        foreach (var item in num)
        {
            sum += item.ToString().ToInt32();
        }
        return sum;
    }
