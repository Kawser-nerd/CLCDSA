...
namespace AtCoder {
    public class Program {
        ...
        static T[,] getUniform_Diag<T>(int row, int col, T value, T defaultValue = default(T)) {
            T[,] Ts = new T[row, col];
            for (int i = 0; i < row; i++) {
                for (int j = 0; j < col; j++) {
        ...
        static void setUniform_Diag<T>(T[,] ts, T value, T defaultValue = default(T)) {
            int N = ts.GetLength(0);
            int M = ts.GetLength(1);
            for (int i = 0; i < M; i++) {
                for (int j = 0; j < N; j++) {
        ...
        static void Main() {
            ...
            Console.WriteLine(Math.Min(keys[1] / keys[0], keys[2]));
        }
    }
}