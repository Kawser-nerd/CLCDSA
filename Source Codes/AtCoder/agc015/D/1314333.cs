...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			pr.WriteLine(Func(A, B));
			/*for (var a = 1; a <= A; a++)
				for (var b = a; b <= B; b++)
				{
					var f = Func(a, b);
					var g = Get(a, b).Count;
					if (f != g) WriteLine("({0}, {1}) => {2} != {3}", a, b, f, g);
				}*/
		}
		long Func(long A, long B)
		{
			...
			var g = (1L << e) - 1;
			if (a >= A) ans += 1L << (d - 1);
			else ans += (a + 1 + (g - A + 1)) + (1L << (d - 1)) - (1L << e);
			return ans;
		}
		// 2^(e-1) <= A < 2^e
		int Log(long A)
		...
		void Hoge(long A, long B)
		{
			...
			WriteLine();
			var x = Get(A, B);
			foreach (var a in x) WriteLine(ToBits(a));
			WriteLine(x.Count);
		}
		SortedSet<long> Get(long A, long B)
		{
			var x = new SortedSet<long>();
			for (var i = A; i <= B; i++) x.Add(i);
			while (true)
			{
				var y = new SortedSet<long>(x);
				foreach (var a in x) foreach (var b in x) y.Add(a | b);
				if (y.Count == x.Count) return x;
				x = y;
			}
		}
		string ToBits(long x)
		{
			var sb = new StringBuilder(d);
			for (var i = d - 1; i >= 0; i--) sb.Append((x & (1L << i)) > 0 ? '1' : '0');
			return sb.ToString();
		}
