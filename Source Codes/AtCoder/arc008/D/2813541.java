...
class Tako implements Comparable<Tako>
{
    double a, b;
    int when;
    long index;

    Tako(double a, double b, int when, long index)
    {
        this.a = a;
        this.b = b;
        this.when = when;
    ...
    public boolean equals(Object o)
    {
        ...
        if (!(o instanceof Tako)) return false;
        Tako p = (Tako) o;
        return a == p.a && b == p.b && when == p.when;
    }

    @Override
    public int hashCode()
    {
        return Objects.hash(a, b, when);
    }

    public int compareTo(Tako p)
    {
        if (index > p.index) return 1;
        else if (index == p.index) return 0;
        else return -1;
    }
}
...
public class Main
{
    ...
    public static void main(String[] args)
    {
        ...
        ArrayList<Tako> list = new ArrayList<>();
        for (int m = 0; m < M; m++)
        {
            long p = sc.nextLong() - 1;
            double a = sc.nextDouble();
            double b = sc.nextDouble();
            list.add(new Tako(a, b, m, p));
        }
        Collections.sort(list);

        Tako[] ques = new Tako[M];
        Tako first = list.get(0);
        ques[first.when] = new Tako(first.a, first.b, first.when, 0);

        for (int ite = 1, index = 1; ite < list.size(); ite++, index++)
        {
            Tako now = list.get(ite);
            long pastIndex = list.get(ite - 1).index;
            long nowIndex = now.index;
            if (pastIndex == nowIndex) index--; //???????????
            ques[now.when] = new Tako(now.a, now.b, now.when, index);
        }
