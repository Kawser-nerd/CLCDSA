...
void search() {
  ...
  for (int p = 1; p <= D; p *= 2) {
    if (timesec() > time_s + 1.5 * 60) return;

    rep (d, p * 2) {
      int i;
      for (i = 0; i < N; ++i) {
        if (isin(p, d, i)) if (--A[i] < 0) goto dmp;
      }
   ...
   dmp:
      ...
      /*
      rep (i, N) if (isin(p, d, i)) --A[i];

      rep (i, N) if (A[i] < 0) goto dmp;
      search();
   dmp:
      rep (i, N) if (isin(p, d, i)) ++A[i];
      */
