...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		{
			sc.Read(out Y, out X);
			map = sc.Board<bool>(X, Y, (c, x, y) => c == '#');
		}
		void SolveOne()
		{
			var left = new bool[X, Y];
			for (var y = 0; y < Y; y++) left[0, y] = true;
			for (var x = 1; x < X - 1; x++) for (var y = 0; y < Y; y += 2) left[x, y] = true;
			for (var y = 0; y < Y; y++)
			{
				for (var x = 0; x < X; x++) pr.Write(left[x, y] || map[x, y] ? '#' : '.');
				pr.WriteLine();
			}
			pr.WriteLine();
			for (var y = 0; y < Y; y++)
