...
namespace Program {
    class MainClass {
        ...
        int Dfs(int n, int k) {
            if (k == 0) return 0;
            int move;
            if (k > G + B) move = Math.Abs(900 - n);
            else if (k > B) move = Math.Abs(1000 - n);
            else move = Math.Abs(1100 - n);
            if (n + k == 2000) return memo[n, k] = move + Dfs(n+1,k - 1);
            // ?????
            int np = (memo[n+1,k] != -1) ? memo[n+1, k] : Dfs( n + 1,k);
            // ???
            int p = (memo[n+1,k - 1] != -1) ? memo[n+1,k - 1] + move : Dfs(n+1,k - 1) + move;
            return memo[ n,k] = Math.Min(np, p);
        }
            ////////////////////////////////////////////////////////////
