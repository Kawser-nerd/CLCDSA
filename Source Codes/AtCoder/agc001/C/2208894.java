...
public class Main {
    ...
    public static void main(String[] args){
        ...
        int d=k/2;
        int[][] neis=new int[n+1][];
        for(int i=1;i<=n;i++){
            int[] dist=new int[n+1];
            int[] stack=new int[n*4+10];
            int[] nei=new int[n];
            int neisize=0;
            int size=0;
            stack[size++]=i;
            while(size>0){
                ...
                for(int ch:ad[cur]){
                    if(ch==i || dist[ch]>0) continue;
                    dist[ch]=dist[cur]+1;
                    stack[size++]=ch;
                }
            }
            nei=Arrays.copyOf(nei,neisize);
            neis[i]=nei;
        }
        ...
        if(k%2==0){
            int res=0;
            for(int i=1;i<=n;i++){
                res=Math.max(res,neis[i].length);
            }
            pl(n-res);
        }else{
            int res=0;
            for(int i=1;i<n;i++){
                for(int ch:ad[i]){
                    ...
                    for(int ne:neis[i]) attend[ne]=true;
                    for(int ne:neis[ch]) attend[ne]=true;
                    int count=0;
                    for(boolean bo:attend) count+=(bo?1:0);
 ...
 public static int[][] tree(int[][] ad,int from){// root is from
     int n=ad.length;
     int[][] res=new int[n][];
