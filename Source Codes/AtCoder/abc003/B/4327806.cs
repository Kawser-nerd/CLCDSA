	...
	class Program
	{
		...
		static void Solve()
		{
			...
			for(int i=0;i<s.Length;++i)
			{
				...
				var ti = t[i];
				if( si == ti) { continue; }
				if( si == '@' && IsCharAtCoder(ti)) { continue; }
				if( ti == '@' && IsCharAtCoder(si)) { continue; }

				b = false;
				break;
			}
			var ans = b ? "You can win" : "You will lose";
			Console.WriteLine(ans);
		}
		private static bool IsCharAtCoder(char c)
		{
			const string sz = "atcoder";
			return sz.Contains(c);
		}
		#region ????????a
