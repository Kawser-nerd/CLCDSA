...
int N;
ALIGNED(16) int A[1000100];
ALIGNED(16) int B[1000100];
...
int calc(int *A, int *B, int N){
    ...
    for(int i = 0; i < N / 4; i++){
        __m128i a1 = _mm_load_si128(reinterpret_cast<__m128i*>(&A[i*4]));
    ...
    ALIGNED(16) int C[4];
    _mm_store_si128(reinterpret_cast<__m128i*>(C), d);
