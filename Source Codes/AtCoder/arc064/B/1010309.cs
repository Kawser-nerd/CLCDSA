...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			pr.WriteLine(Calc() ? "First" : "Second");
		}
		bool Calc()
		{
			var N = S.Length;
			...
			{
				var f = true;
				for (var i = 0; f && i < N - 2; i++) f &= S[i] == S[i + 2];
				return S[0] == S[N - 1] & !f;
			}
			else if (N == 3) return S[0] != S[2];
			else
			{
				var b = new bool[N - 2];
				for (var i = 0; i < N - 2; i++) b[i] = S[i] == S[i + 2];
				var c = new bool[N - 1];
				c[0] = true;
				// c[i] = b[0] | ... | b[i - 1]
				for (var i = 0; i < N - 2; i++) c[i + 1] = c[i] & b[i];
				var d = new bool[N - 1];
				d[N - 2] = true;
				// d[i] = b[i] | ... | b[N - 3]
				for (var i = N - 3; i >= 0; i--) d[i] = d[i + 1] & b[i];
				var win = false;
				if (S[0] != S[N - 1]) for (var i = 1; !win && i < N - 1; i++) win |= !b[i - 1];
				else
				{
					win |= !b[0] & S[0] == S[2] & d[2];
					win |= !b[N - 3] & S[N - 4] == S[N - 1] & c[N - 4];
					for (var i = 2; !win && i < N - 2; i++)
					{
						var f = c[i - 2] & d[i + 1] & S[i - 2] == S[i + 1] & S[i - 1] == S[i + 2];
						win |= !b[i - 1] & f;
					}
				}
				return win;
		...
		{
			var flag = false;
			for (var i = 1; !flag && i < S.Length - 1; i++) if (S[i - 1] != S[i + 1]) flag |= !Win(S.Remove(i, 1));
			return flag;
		}
