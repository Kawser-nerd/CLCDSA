...
public class Main {
    ...
    public static void main(String[] args){
    	...
    	for(int i=1;i<k;i++){
    		for(int j=1;j<2*k;j++){
    			rui[i][j]=t[i][j]+rui[i-1][j]+rui[i][j-1]-rui[i-1][j-1];
    		}
    	}
    	int res=0;
    	for(int i=0;i<k;i++){
    		for(int j=0;j<k;j++){
    			int v1=rui[i][j];
    			int v2=rui[k-1][j+k]-rui[i][j+k]-rui[k-1][j]+v1;
    			int v3=rui[i][2*k-1]-rui[i][j+k];
    			int score=v1+v2+v3;
