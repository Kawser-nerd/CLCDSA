...
bool win[4][4][SHIFT(16)];
bool visited[4][4][SHIFT(16)];
vector<string> board;
int R, C;
...
bool canwin(int r, int c, int mask){
   if(visited[r][c][mask]) return win[r][c][mask];
   visited[r][c][mask] = true;

   bool theyWin = true;
   FOR(i,0,8){
	  int nr = r+dr[i], nc=c+dc[i];
          if(nr < 0 || nc < 0 || nr >= R || nc >= C || (mask & SHIFT(nr*4+nc)) || board[nr][nc]=='#') continue;
//	  cout << "Trying: " << nr << " " << nc << " " << (mask | SHIFT(nr*4+nc)) << endl;
	  theyWin = theyWin && canwin(nr,nc,mask | SHIFT(nr*4+nc));
	}
//  cout << "THEY WIN: " << theyWin << endl;
  win[r][c][mask] = !theyWin;
//  cout << r << " " << c << " " << mask << " -> " << win[r][c][mask] << endl;
  return win[r][c][mask];
}
...
int main(){
  ...
  FOR(_t,1,_times+1){
    cout << "Case #" << _t << ": ";
    MEMSET(win,false); MEMSET(visited,false);
    cin >> R >> C;
    board.resize(R);
    int kr=0, kc=0;
    FOR(i,0,R) { cin >> board[i];  unsigned pos = board[i].find("K"); if(pos!=string::npos) kr=i,kc=pos;}
//    cout << kr << " " << kc << endl;
    cout << (canwin(kr,kc,SHIFT(kr*4+kc))?"A":"B") << endl;
  }
  return 0;
}
