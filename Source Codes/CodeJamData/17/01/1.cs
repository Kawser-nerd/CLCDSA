        ...
        class Solver {
            public List<string> SolveAll(int threadId, StreamReader sr) {
                ...
                for (int testCaseIx = 0; testCaseIx < TestCaseCount; testCaseIx++) {
                    // ------------ 読み込み ------------
                    List<string> cs = SplitToStrings(sr.ReadLine());
                    int K = int.Parse(cs[1]);
                    // ------------ 読み込み ------------
                    #region
                    ...
                    #endregion
                    // ------------ 呼び出し ------------
                    string output = Solve(testCaseIx, cs[0], K);
                    // ------------ 呼び出し ------------
                    outputs.Add(output);
                }
                return outputs;
            }
            string Solve(int testCaseIx, string S, int k) {
                string s = S;
                int ans = 0;
                while (s != "") {
                    if (s[0] == '-') {
