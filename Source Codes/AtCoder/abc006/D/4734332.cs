...
class Program
{
    ...
    {
        var num = Input.num;
        var cc = Enumerable.Repeat(0, num).Select(v => Input.num).ToArray();
        var list = new List<int> { cc[0] };
        for(var i = 1; i < num; i++)
        {
            if (cc[i] > list[list.Count - 1]) list.Add(cc[i]);
            else list[Search.LowerBound(list.ToArray(), cc[i])] = cc[i];
        }
