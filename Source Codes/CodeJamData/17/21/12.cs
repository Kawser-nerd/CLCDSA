...
partial class Solver
{
    public void Run()
    {
        ...
        while (t-- > 0)
        {
            ...
            double lastReachTime = 0;

            for (int i = 0; i < N; i++)
            {
                var remainDistance = (double)(D - K[i]);
                var reachTime = remainDistance / S[i];
                lastReachTime = Math.Max(reachTime, lastReachTime);
            }

            ans = D / lastReachTime;

            cout.WriteLine("Case #{0}: {1:0.0000000000}", caseNo++, ans);
        }
    }
}
