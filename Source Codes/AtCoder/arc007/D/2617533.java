...
public class Main {
	private static void solve() {
		...
		int firstLen = s.length() - s.replaceFirst("^.0*", "").length();
		String fStr = s.substring(0, firstLen);
		if (fStr.replace("0", "").isEmpty())
			fStr = "1" + fStr;
		String rStr = s.substring(firstLen);
		BigInteger first = new BigInteger(fStr);
		if (rStr.isEmpty())
			kil(first + " 1");
		if (fStr.length() < rStr.length()) {
			for (int i = 1; i <= rStr.length(); i++) {
				String nextStr = rStr.substring(0, i);
				BigInteger next = new BigInteger(nextStr);
				BigInteger diff = next.subtract(first);
				...
				while (sb.length() < rStr.length()) {
					aa = aa.add(diff);
					sb.append(aa.toString());
				}
				if (sb.toString().startsWith(rStr)) {
					kil(first + " " + diff);
				}
			}
		} else if (fStr.length() == rStr.length()) {
			BigInteger rest = new BigInteger(rStr);
			if (rest.compareTo(first) <= 0)
				kil(first + " " + rest.multiply(BigInteger.TEN).subtract(first));
			kil(first + " " + rest.subtract(first));
		} else {
			BigInteger rest = new BigInteger(rStr);
			while (true) {
				BigInteger next = rest.multiply(BigInteger.TEN);
				if (next.compareTo(first) > 0)
					kil(first + " " + next.subtract(first));
