...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		bool[,] used;
		long[,] pows;
		long[,] powsum;  // powsum[i,s] = (pows[0,s]+...+pows[i-1,s]) % Mod
		void SolveOne()
		{
			memo = new long[N, C + 1];
			used = new bool[N, C + 1];
		...
		long Calc(int n, int s)
		{
			if (!used[n, s])
			{
				...
				else
				{
					for (var t = 0; t <= s; t++)
					{
						var sum = (powsum[B[n] + 1, t] - powsum[A[n], t]) % Mod;
						ans = (ans + sum * Calc(n + 1, s - t) % Mod) % Mod;
					}
				}
				ans %= Mod;
				if (ans < 0) ans += Mod;
...
class Eq2 : IEqualityComparer<Tuple<int, int>>
{
