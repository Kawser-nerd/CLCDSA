...
int n;
int64 a[50];
set<int64> s1[50],s2[50];
int64 L;

void expand(set<int64> s[50],int64 key)
{
	set<int64> a[50];
	for (int i=0;i<50;i++)
	{
	...
	for (int i=0;i<50;i++)
		for (set<int64>::iterator it=a[i].begin();it!=a[i].end();++it)
			for (int d=-1;d<=1;d++)
			{
				int k=i+d;
				if (k>0)
				{
					int64 other=*it+key*(int64)k;
					if (other<=L)
						s[k].insert(other);
				}
			}
...
//	freopen("D-small-attempt2.in","r",stdin);freopen("D-small-attempt2.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int i=0;i<50;i++)
			for (int j=0;j<50;j++)
				if (abs(i-j)<=1)
					for (set<int64>::iterator it=s1[i].begin();it!=s1[i].end();++it)
					{
						int64 current=*it;
						if (current>L)
							continue;
						set<int64>::iterator it2=s2[j].upper_bound(L-current);
						if (it2==s2[j].begin())
							continue;
						--it2;
						checkmax(R,current+*it2);
					}
		if (R<0)
			printf("NO SOLUTION\n");
		else
