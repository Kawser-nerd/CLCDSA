...
namespace ProgrammingContest
{
    ...
    class MainClass : IDisposable
    {
        ...
        void Solve()
        {
            var points = Enumerable.Range(0, Sc.NextInt()).Select(e =>
            {
                return new
                {
                    x = Sc.NextInt(),
            ...
            var key = points.Where(p => p.h > 0).First();
            for (int cx = 0; cx < 101; cx++)
            {
                for (int cy = 0; cy < 101; cy++)
                {
                    bool ok = true;
                    var H = key.h + Math.Abs(key.x - cx) + Math.Abs(key.y - cy);
                    foreach (var p in points)
                    {
                        if (Math.Max(H - Math.Abs(p.x - cx) - Math.Abs(p.y - cy), 0) != p.h)
                        {
                            ok = false;
                        }
                    }
