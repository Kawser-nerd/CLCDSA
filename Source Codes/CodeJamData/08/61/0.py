...
def solve(input_set):
    ...
    for sH, sW, sX in S:
        if not sX:
            if blH is None or sH < blH:
                nuH = max_none(nuH, sH)
            if buH is None or sH > buH:
                nlH = min_none(nlH, sH)
            if blW is None or sW < blW:
                nuW = max_none(nuW, sW)
            if buW is None or sW > buW:
                nlW = min_none(nlW, sW)
    log('%s %s %s %s\n', blH, buH, blW, buW)
    log('%s %s %s %s\n', nlH, nuH, nlW, nuW)
    answers = []
    for uH, uW in U:
        if blH is not None and buH is not None and blW is not None and buW is not None and blH <= uH <= buH and blW <= uW <= buW:
            answers.append('BIRD')
        elif buH is not None and nlH is not None and uH >= nlH:
            answers.append('NOT BIRD')
        elif blH is not None and nuH is not None and uH <= nuH:
            answers.append('NOT BIRD')
        elif buW is not None and nlW is not None and uW >= nlW:
            answers.append('NOT BIRD')
        elif blW is not None and nuW is not None and uW <= nuW:
            answers.append('NOT BIRD')
        else:
