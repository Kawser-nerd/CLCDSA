...
namespace Program {
    ...
    class IO {
        ...
        public void ini<T, U>(out T[] a, out U[] b, int n) { a = new T[n]; b = new U[n]; for (int i = 0; i < n; ++i) { a[i] = i<T>(); b[i] = i<U>(); } }
        public void ini<T, U, V>(out T[] a, out U[] b, out V[] c, int n) { a = new T[n]; b = new U[n]; c = new V[n]; for (int i = 0; i < n; ++i) { a[i] = i<T>(); b[i] = i<U>(); c[i] = i<V>(); } }
        ...
        public void o<T>(T[,] a) {
            ...
            for (int i = 0; i < a0; ++i) { for (int j = 0; j < a1 - 1; ++j) Console.Write(a[i, j] + " "); Console.WriteLine(a[i, a1 - 1]); }
        }
    ...
    class BellmanFord : ShortestPath {
        ...
        public void Run(int f) {
            ...
            for (int i = 0; i < I - 1; i++) for (int j = 0; j < I; ++j) for (int k = 0; k < Adj[j].Count; ++k) {
                        var cur = Adj[j][k];
                        if (cost[cur.to] > cost[j] + cur.cost) cost[cur.to] = cost[j] + cur.cost;
                    }
            for (int i = 0; i < I; i++) for (int j = 0; j < I; ++j) for (int k = 0; k < Adj[j].Count; ++k) {
                        var cur = Adj[j][k];
                        if (cost[cur.to] > cost[j] + cur.cost) { cost[cur.to] = cost[j] + cur.cost; neg[cur.to] = true; }
                        if (neg[j]) neg[cur.to] = true;
                    }
        }
    ...
    class WarshallFloyd {
        ...
        public void AddPath(int f, int t, long c = 1) { if (isNonDir) { G[f, t] = c; G[t, f] = c; BefG[f, t] = c; BefG[t, f] = c; } else { G[f, t] = c; BefG[f, t] = c; } }
    ...
    class UnionFind {
        int[] dat;
        public void Init(int n) { dat = new int[n + 1]; for (int i = 0; i <= n; ++i) dat[i] = -1; }
    ...
    class Kruskal : ShortestPath {
        ...
        public long Run() {
            ...
            for (int i = 0; i < I; ++i) for (int j = 0; j < Adj[i].Count; ++i) L.Add(Tuple.Create(i, Adj[i][j].to, Adj[i][j].cost));
