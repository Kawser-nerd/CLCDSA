...
public class Heap<T>{
    ...
    public Heap(int capacity, Comparison<T> cmp){
    ...
    public void Heapify(int i){
        ...
        int r = Right(i);
        int largest = i;
        if(l<size && cmp(array[l], array[i]) > 0) largest = l;
        if(r<size && cmp(array[r], array[largest])>0) largest = r;
        if(largest != i){
            T tmp = array[i];
    ...
    public void Insert(T key){
        ...
        int i = size-1;
        while(i>0 && cmp(array[Parent(i)],array[i])<0){
            T tmp = array[i];
...
public class FDijkstra {
    ...
    public FDijkstra(int maxV){
        d = new int[maxV];
        hp = new Heap<Z2>(maxV, (a,b)=>b.x-a.x);
    ...
    public void Dijkstra(int s){
        ...
        while(!hp.Empty()){
            ...
            foreach(var e in G[v]){
                if(d[e.to] == INF || d[e.to]>d[v]+e.cost){
                    d[e.to] = d[v]+e.cost;
