...
namespace CodeJam2010
{
    ...
    public class B
    {
        System.IO.StreamWriter writer = new System.IO.StreamWriter("D:\\BackupPlace\\project\\MiscHelper\\MiscHelper\\out.txt");
        ...
        private string Solve(string s)
        {
            string[] strAll = s.Split(
                new char[] { ' ' },
                StringSplitOptions.RemoveEmptyEntries);
            int n = int.Parse(strAll[0]);
            System.Numerics.BigInteger[] num = new BigInteger[n];
            for (int i = 0; i < n; i++)
            {
                num[i] = BigInteger.Parse(strAll[i + 1]);
            }
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = i + 1; j < n; j++)
                {
                    BigInteger delta = num[i] - num[j];
                    gcd = BigInteger.GreatestCommonDivisor(delta, gcd);
                }
            }
