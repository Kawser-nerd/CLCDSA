...
int count_way(int x, int y){
  int res = INFi;
  int res1 = (y >= x) ? (y - x) : INFi; chmin(res, res1);
  int res2 = (-x <= y) ? (1 + y + x) : INFi; chmin(res, res2);
  int res3 = (y <= -x) ? (1 - x - y) : INFi; chmin(res, res3);
  int res4 = (x >= y) ? (2 + x - y) : INFi; chmin(res, res4);
  return res;
}
