...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        int[] f = niah(N, 2);
        int[] t = niahd(N, 2);
        int[][] cou = new int[(int) 1e5 + 10][4];
        for (int i = 0; i < N; i++) {
            cou[f[i]][t[i]]++;
        }
        RectangleSum r = new RectangleSum(cou);
        for (int i = 0; i < N; i++) {
            long win = r.getSum(0, 4, 0, f[i]);
            win += cou[f[i]][(t[i] + 1) % 3];
            long hiki = cou[f[i]][t[i]] - 1;
            long lose = N - 1 - win - hiki;
            System.out.println(win + " " + lose + " " + hiki);
        }
    }
