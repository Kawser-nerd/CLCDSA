	...
	internal class Program {
		...
		public class Calc
		{
			public void Solve() {
                ...
                var w = Generate.Seq(1, 2 * N).ToList();
                var k = x == 2 ? new[] { 4, 1, 2, 3 } : new[] { x - 2, x + 1, x, x - 1 };
                k.Foreach(item => w.Remove(item));

                var q = w.ToQueue();
                (N - 2).REP(i => { q.Dequeue().WL(); });
                k.WL();
                q.WL();
...
namespace CS_Contest.Utils {
	...
	public class Map<TKey, TValue> : Dictionary<TKey, TValue> {
		...
		public Map(TValue def=default(TValue)) : base() { _default = def; }

		public new TValue this[TKey index] {
			get {
				TValue v;
				return this.TryGetValue(index, out v) ? v : base[index] = _default;
			}
