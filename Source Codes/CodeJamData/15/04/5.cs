        ...
        class Solver {
            public List<string> SolveAll(int threadId, StreamReader sr) {
                ...
                for (int testCase = 0; testCase < TestCase; testCase++) {
                    // ------------ 読み込み ------------
                    int[] cs = sr.ReadLine().Split(' ').Select(c => int.Parse(c)).ToArray();
                    int X = cs[0];
                    int R = cs[1];
                    int C = cs[2];
                    // ------------ 読み込み ------------
                    ...
                    #endregion
                    // ------------ 呼び出し ------------
                    string output = Solve(testCase, X, R, C);
                    // ------------ 呼び出し ------------
                    outputs.Add(output);
                }
                return outputs;
            ...
            string Solve(int testCase, int X, int R, int C) {
                string win = "GABRIEL";
                string lose = "RICHARD";
                int w = Math.Max(R, C);
                int h = Math.Min(R, C);
