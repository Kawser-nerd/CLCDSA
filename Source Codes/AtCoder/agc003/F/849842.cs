...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			sc.Read(out Y, out X, out K);
			map = sc.Board(X, Y, (c, x, y) => c == '#');
		}
		void SolveOne()
		{
			if (K <= 1) { pr.WriteLine(1); return; }
			var jx = new List<int>();
			var jy = new List<int>();
			for (var x = 0; x < X; x++) if (map[x, 0] && map[x, Y - 1]) jx.Add(x);
			for (var y = 0; y < Y; y++) if (map[0, y] && map[X - 1, y]) jy.Add(y);
			var sum = 0;
			for (var x = 0; x < X; x++) for (var y = 0; y < Y; y++) if (map[x, y]) sum++;
			if (jx.Count > 0 && jy.Count > 0) pr.WriteLine(1);
			else if (jx.Count == 0 && jy.Count == 0) pr.WriteLine(Func.Pow(sum, K - 1, Mod));
			else
			{
				if (jy.Count == 0)
				{
					var tmp = new bool[Y, X];
					for (var x = 0; x < X; x++) for (var y = 0; y < Y; y++) tmp[y, x] = map[x, y];
					map = tmp;
					Func.Swap(ref X, ref Y);
					Func.Swap(ref jx, ref jy);
				}
				var cont = 0;
				for (var x = 0; x < X - 1; x++) for (var y = 0; y < Y; y++) if (map[x, y] && map[x + 1, y]) cont++;
				var mat = new Matrix(2, 2);
				mat[0, 0] = sum;
				mat[1, 0] = cont;
				mat[1, 1] = jy.Count;
				mat = mat.Pow((ulong)(K - 1), Mod);
				var ans = (mat[0, 0] - mat[1, 0]) % Mod;
				if (ans < 0) ans += Mod;
				pr.WriteLine(ans);
			}
		}
...
namespace GeometryLong
{
	class Vector3D : IEnumerable<long>, IEquatable<Vector3D>, IComparable<Vector3D>, IComparable
	{
		public long X;
		public long Y;
		public long Z;
		public Vector3D() : this(0, 0, 0) { }
		public Vector3D(long x, long y, long z) { X = x; Y = y; Z = z; }
		public Vector3D(long[] components) : this(components[0], components[1], components[2]) { }
		public Vector3D(Func<int, long> components) : this(components(0), components(1), components(2)) { }
		public long this[int n] { get { return n == 0 ? X : n == 1 ? Y : Z; } set { if (n == 0) X = value; else if (n == 1) Y = value; else Z = value; } }
		public double Abs { get { return Math.Sqrt(this * this); } }
		public static Vector3D operator +(Vector3D p) { return new Vector3D(p.X, p.Y, p.Z); }
		public static Vector3D operator -(Vector3D p) { return new Vector3D(-p.X, -p.Y, -p.Z); }
		public static Vector3D operator -(Vector3D p, Vector3D q) { return new Vector3D(p.X - q.X, p.Y - q.Y, p.Z - q.Z); }
		public static Vector3D operator +(Vector3D p, Vector3D q) { return new Vector3D(p.X + q.X, p.Y + q.Y, p.Z + q.Z); }
		public static Vector3D operator *(Vector3D p, long r) { return new Vector3D(p.X * r, p.Y * r, p.Z * r); }
		public static Vector3D operator /(Vector3D p, long r) { return new Vector3D(p.X / r, p.Y / r, p.Z / r); }
		public static Vector3D operator *(long r, Vector3D p) { return p * r; }
		public static long operator *(Vector3D p, Vector3D q) { return p.X * q.X + p.Y * q.Y + p.Z * q.Z; }
		public static bool operator ==(Vector3D p, Vector3D q) { return ReferenceEquals(p, q) || ((object)p != null && (object)q != null && p.X == q.X && p.Y == q.Y && p.Z == q.Z); }
		public static bool operator !=(Vector3D p, Vector3D q) { return !ReferenceEquals(p, q) && ((object)p == null || (object)q == null || p.X != q.X || p.Y != q.Y || p.Z != q.Z); }
		public static bool operator <(Vector3D p, Vector3D q) { return p.X == q.X ? p.Y == q.Y ? p.Z < q.Z : p.Y < q.Y : p.X < q.X; }
		public static bool operator <=(Vector3D p, Vector3D q) { return p.X == q.X ? p.Y == q.Y ? p.Z <= q.Z : p.Y < q.Y : p.X < q.X; }
		public static bool operator >(Vector3D p, Vector3D q) { return p.X == q.X ? p.Y == q.Y ? p.Z > q.Z : p.Y > q.Y : p.X > q.X; }
		public static bool operator >=(Vector3D p, Vector3D q) { return p.X == q.X ? p.Y == q.Y ? p.Z >= q.Z : p.Y > q.Y : p.X > q.X; }
		public static Vector3D operator %(Vector3D p, Vector3D q) { return new Vector3D(p.Y * q.Z - p.Z * q.Y, p.Z * q.X - p.X * q.Z, p.X * q.Y - p.Y * q.X); }
		public bool OnLine(Line3D line) { return ((this - line.P) % (this - line.Q)).Abs <= Geometry.EPS; }
		public bool OnSegment(Line3D line) { return OnLine(line) && (this - line.P) * (line.Q - this) >= Geometry.EPS; }
		public static explicit operator Vector(Vector3D p) { return new Vector(3, i => p[i]); }
		public static explicit operator Vector3D(Vector p) { return new Vector3D(p[0], p[1], p[2]); }
		public IEnumerator<long> GetEnumerator() { yield return X; yield return Y; yield return Z; }
		IEnumerator IEnumerable.GetEnumerator() { return GetEnumerator(); }
		public Vector3D ProjectTo(Vector3D direction) { return ((this * direction) / (direction * direction)) * direction; }
		public override bool Equals(object obj)
		{
			if (obj == null) return false;
			var p = obj as Vector3D;
			return (object)p != null && (X == p.X) && (Y == p.Y) && (Z == p.Z);
		}
		public override int GetHashCode() { return X.GetHashCode() ^ Y.GetHashCode() ^ Z.GetHashCode(); }
		public bool Equals(Vector3D other) { return this == other; }
		public int CompareTo(Vector3D other) { return other == null || this > other ? 1 : this < other ? -1 : 0; }
		public int CompareTo(object obj) { return obj == null ? 1 : CompareTo((Vector3D)obj); }
	}
	class Line3D : IEnumerable<Vector3D>
	{
		public Vector3D P;
		public Vector3D Q;
		public Vector3D this[int n] { get { return n == 0 ? P : Q; } set { if (n == 0) P = value; else Q = value; } }
		public Vector3D D { get { return Q - P; } }
		public Line3D(Vector3D p, Vector3D q) { P = p; Q = q; }
		public IEnumerator<Vector3D> GetEnumerator() { yield return P; yield return Q; }
		IEnumerator IEnumerable.GetEnumerator() { return GetEnumerator(); }
	}
	class Line2D : IEnumerable<Vector2D>
	{
		public Vector2D P;
		public Vector2D Q;
		public Vector2D this[int n] { get { return n == 0 ? P : Q; } set { if (n == 0) P = value; else Q = value; } }
		public Vector2D D { get { return Q - P; } }
		public Line2D(Vector2D p, Vector2D q) { P = p; Q = q; }
		public IEnumerator<Vector2D> GetEnumerator() { yield return P; yield return Q; }
		IEnumerator IEnumerable.GetEnumerator() { return GetEnumerator(); }
	}
	// points assumed to be counter clockwise
	class Polygon : List<Vector2D>
	{
		public Polygon() : base() { }
		public Vector2D Next(int i) { return this[(i + 1) % Count]; }
		public Vector2D Prev(int i) { return this[(i + Count - 1) % Count]; }
		private Vector2D Diff(int i) { return Next(i) - this[i]; }
		public long Area2 { get { var area = 0L; for (var i = 0; i < Count; i++) area += this[i] % Next(i); return area; } }
		public long Area { get { return Area2 / 2; } }
		public enum State { Out = -1, On, In };
		public State Includes(Vector2D p)
		{
			var isIn = false;
			for (var i = 0; i < Count; i++)
			{
				...
				var cross = a % b;
				if (a.Y <= 0 && 0 < b.Y && cross < 0) isIn = !isIn;
				if (cross == 0 && a * b <= 0) return State.On;
			}
			return isIn ? State.In : State.Out;
		}
		public bool IsConvex() { for (var i = 0; i < Count; i++) if (Geometry.CCW(Prev(i), this[i], Next(i)) > 0) return false; return true; }// only applicable to a convex polygon
		public long Diameter2()
		{
			int i0 = 0, j0 = 0;
			for (int k = 1; k < Count; ++k) { if (this[k].Y > this[i0].Y) i0 = k; if (this[k].Y < this[j0].Y) j0 = k; }
			var max = (this[i0] - this[j0]).Norm2;
			int i, maxi, j, maxj;
			i = maxi = i0;
			j = maxj = j0;
			do
			{
				if (Diff(i) % Diff(j) >= 0) j = (j + 1) % Count;
				else i = (i + 1) % Count;
				var tmp = (this[i] - this[j]).Norm2;
				if (tmp > max) { max = tmp; maxi = i; maxj = j; }
			} while (i != i0 || j != j0);
			return max;
		}
	}
	class Circle2D
	{
		public Vector2D C;
		public long R;
		public Circle2D() : this(new Vector2D(), 0) { }
		public Circle2D(Vector2D c, long r) { C = c; R = r; }
		public double Area { get { return R * R * Math.PI; } }
	}
	class Vector2D : IEnumerable<long>, IEquatable<Vector2D>, IComparable<Vector2D>, IComparable
	{
		public long X;
		public long Y;
		public Vector2D() : this(0, 0) { }
		public Vector2D(long x, long y) { X = x; Y = y; }
		public Vector2D(long[] components) : this(components[0], components[1]) { }
		public Vector2D(Func<int, long> components) : this(components(0), components(1)) { }
		public long this[int n] { get { return n == 0 ? X : Y; } set { if (n == 0) X = value; else Y = value; } }
		public double Abs { get { return Math.Sqrt(this * this); } }
		public double Norm { get { return Math.Sqrt(this * this); } }
		public long Norm2 { get { return this * this; } }
		public static Vector2D operator +(Vector2D p) { return new Vector2D(p.X, p.Y); }
		public static Vector2D operator -(Vector2D p) { return new Vector2D(-p.X, -p.Y); }
		public static Vector2D operator -(Vector2D p, Vector2D q) { return new Vector2D(p.X - q.X, p.Y - q.Y); }
		public static Vector2D operator +(Vector2D p, Vector2D q) { return new Vector2D(p.X + q.X, p.Y + q.Y); }
		public static Vector2D operator *(Vector2D p, long r) { return new Vector2D(p.X * r, p.Y * r); }
		public static Vector2D operator /(Vector2D p, long r) { return new Vector2D(p.X / r, p.Y / r); }
		public static Vector2D operator *(long r, Vector2D p) { return p * r; }
		public static long operator *(Vector2D p, Vector2D q) { return p.X * q.X + p.Y * q.Y; }
		public static explicit operator Vector(Vector2D p) { return new Vector(2, i => p[i]); }
		public static explicit operator Vector2D(Vector p) { return new Vector2D(p[0], p[1]); }
		public static explicit operator Vector3D(Vector2D p) { return new Vector3D(p.X, p.Y, 0); }
		public static explicit operator Vector2D(Vector3D p) { return new Vector2D(p.X, p.Y); }
		public static bool operator ==(Vector2D p, Vector2D q) { return ReferenceEquals(p, q) || ((object)p != null && (object)q != null && p.X == q.X && p.Y == q.Y); }
		public static bool operator !=(Vector2D p, Vector2D q) { return !ReferenceEquals(p, q) && ((object)p == null || (object)q == null || p.X != q.X || p.Y != q.Y); }
		public static bool operator <(Vector2D p, Vector2D q) { return p.X == q.X ? p.Y < q.Y : p.X < q.X; }
		public static bool operator <=(Vector2D p, Vector2D q) { return p.X == q.X ? p.Y <= q.Y : p.X < q.X; }
		public static bool operator >(Vector2D p, Vector2D q) { return p.X == q.X ? p.Y > q.Y : p.X > q.X; }
		public static bool operator >=(Vector2D p, Vector2D q) { return p.X == q.X ? p.Y >= q.Y : p.X > q.X; }
		// cross product ('%' was chosen because it has a good priority as operator)
		public static long operator %(Vector2D p, Vector2D q) { return p.X * q.Y - p.Y * q.X; }
		public bool OnLine(Line2D line) { return (this - line.P) % (this - line.Q) == 0; }
		public bool OnSegment(Line2D line) { return OnLine(line) && (this - line.P) * (line.Q - this) >= 0; }
		public IEnumerator<long> GetEnumerator() { yield return X; yield return Y; }
		IEnumerator IEnumerable.GetEnumerator() { return GetEnumerator(); }
		public Vector2D ProjectTo(Vector2D direction) { return ((this * direction) / (direction * direction)) * direction; }
		public override bool Equals(object obj)
		{
			if (obj == null) return false;
		...
		public int CompareTo(Vector2D other) { return other == null || this > other ? 1 : this < other ? -1 : 0; }
		public int CompareTo(object obj) { return obj == null ? 1 : CompareTo((Vector2D)obj); }
	}
	static class Geometry
	{
		...
		public static bool IntersectsLL(Line2D l, Line2D m) { var dl = l.D; return dl % m.D != 0 || dl % (m.P - l.P) == 0; }
		public static bool IntersectsLS(Line2D l, Line2D s) { var dl = l.D; return (dl % (s.P - l.P)) * (dl % (s.Q - l.P)) <= 0; }
		public static bool IntersectsLP(Line2D l, Vector2D p) { return (l.Q - p) % (l.P - p) == 0; }
		public static bool IntersectsSS(Line2D s, Line2D t) { return CCW(s.P, s.Q, t.P) * CCW(s.P, s.Q, t.Q) <= 0 && CCW(t.P, t.Q, s.P) * CCW(t.P, t.Q, s.Q) <= 0; }
		public static bool IntersectsSP(Line2D s, Vector2D p) { return (s.P - p).Abs + (s.Q - p).Abs - s.D.Abs < EPS; }
		public static bool IntersectsPP(Vector2D p, Vector2D q) { return p.X == q.X && p.Y == q.Y; }
		public static Vector2D Project(Vector2D p, Line2D l) { return l.P + p.ProjectTo(l.D); }
		public static Vector2D Reflect(Vector2D p, Line2D l) { return 2 * Project(p, l) - p; }
		public static double DistanceLP(Line2D l, Vector2D p) { return (p - Project(p, l)).Abs; }
		public static double DistanceLL(Line2D l, Line2D m) { return IntersectsLL(l, m) ? 0 : DistanceLP(l, m.P); }
		public static double DistanceLS(Line2D l, Line2D s) { return IntersectsLS(l, s) ? 0 : Math.Min(DistanceLP(l, s.P), DistanceLP(l, s.Q)); }
		public static double DistanceSP(Line2D s, Vector2D p)
		{
			var r = Project(p, s);
			return IntersectsSP(s, r) ? (r - p).Abs : Math.Min((s.P - p).Abs, (s.Q - p).Abs);
		}
		public static double DistanceSS(Line2D s, Line2D t)
		{
			return IntersectsSS(s, t) ? 0 : Func.Min(DistanceSP(s, t.P), DistanceSP(s, t.Q), DistanceSP(t, s.P), DistanceSP(t, s.Q));
		}
		public static Vector2D CrossPoint(Line2D l, Line2D m)
		{
			var dl = l.D;
			var A = dl % m.D;
			var B = dl % (l.Q - m.P);
			if (A == 0 && B == 0) return m.P;
			return m.P + B / A * m.D;
		}
		public static List<Vector2D> ConvexHull(List<Vector2D> points)
		{
			...
			var convex = new List<Vector2D>(2 * n);
	...
	class Vector : IEnumerable<long>
	{
		public int Dimension { get; protected set; }
		protected long[] a;
		public IEnumerable<long> Components { get { return a; } }
		public long this[int n] { get { return a[n]; } set { a[n] = value; } }
		public Vector(int dim) { Dimension = dim; a = new long[Dimension]; }
		public Vector(long[] components) : this(components.Length, i => components[i]) { }
		public Vector(int dim, Func<int, long> components) : this(dim) { for (var i = 0; i < dim; i++) this.a[i] = components(i); }
		public static Vector operator +(Vector p) { return new Vector(p.a); }
		public static Vector operator -(Vector p) { return new Vector(p.Dimension, i => -p.a[i]); }
		public static Vector operator -(Vector p, Vector q) { return new Vector(p.Dimension, i => p.a[i] - q.a[i]); }
		public static Vector operator +(Vector p, Vector q) { return new Vector(p.Dimension, i => p.a[i] + q.a[i]); }
		public static Vector operator *(Vector p, long r) { return new Vector(p.Dimension, i => p.a[i] * r); }
		public static Vector operator /(Vector p, long r) { return new Vector(p.Dimension, i => p.a[i] / r); }
		public static Vector operator %(Vector p, long mod) { return new Vector(p.Dimension, i => p.a[i] % mod); }
		public static Vector operator *(long r, Vector p) { return new Vector(p.Dimension, i => p.a[i] * r); }
		public static long operator *(Vector p, Vector q) { var sum = 0L; for (var i = 0; i < p.Dimension; i++) sum += p.a[i] * q.a[i]; return sum; }
		public override string ToString()
		{
			var sb = new StringBuilder();
			sb.Append("(");
			for (var i = 0; i < Dimension; i++) sb.Append(a[i] + (i == Dimension - 1 ? ")" : ", "));
			return sb.ToString();
		}
		public Vector ProjectTo(Vector direction) { return ((this * direction) / (direction * direction)) * direction; }
		public IEnumerator<long> GetEnumerator() { return (IEnumerator<long>)a.GetEnumerator(); }
		IEnumerator IEnumerable.GetEnumerator() { return GetEnumerator(); }
		public Vector SolveLinearEquation(Matrix A, long mod)
		{
			var inv = new Vector(Dimension, i => { var x = a[i] % mod; return x >= 0 ? x : x + mod; });
			var M = new Matrix(Dimension, Dimension, (r, c) => { var x = A[r, c] % mod; return x >= 0 ? x : x + mod; });
			for (var j = 0; j < Dimension; j++)
			{
				var pos = j;
				while (pos < Dimension && M[pos, j] == 0) pos++;
				if (pos == Dimension) return null;
				M.SwitchRow(j, pos, j);
				inv.a.Swap(j, pos);
				var inv2 = Func.Inverse(M[j, j], mod);
				inv.a[j] *= inv2;
				M.MultiplicateRow(j, inv2, j + 1);
				M.ModRow(j, mod, j + 1);
				M[j, j] = 1;
				for (var i = j + 1; i < Dimension; i++) { inv.a[i] -= (inv.a[j] * M[i, j]) % mod; inv.a[i] %= mod; if (inv.a[i] < 0) inv.a[i] += mod; M.AddRow(j, i, -M[i, j], j + 1); M.ModRow(i, mod, j + 1); }
			}
			for (var j = Dimension - 1; j > 0; j--) for (var i = j - 1; i >= 0; i--) { inv.a[i] -= (inv.a[j] * M[i, j]) % mod; inv.a[i] %= mod; if (inv.a[i] < 0) inv.a[i] += mod; }
			return inv;
		}
	}
	class Matrix
	{
		...
		private long[,] a;
		public long this[int r, int c] { get { return a[r, c]; } set { a[r, c] = value; } }
		public Matrix(int r, int c) { Row = r; Column = c; a = new long[r, c]; }
		public Matrix(long[,] components) : this(components.GetLength(0), components.GetLength(1), (i, j) => components[i, j]) { }
		public Matrix(int r, int c, Func<int, int, long> components) : this(r, c) { for (var i = 0; i < r; i++) for (var j = 0; j < c; j++) this.a[i, j] = components(i, j); }
		public long Trace { get { var sum = 0L; for (var i = 0; i < Row; i++) sum += a[i, i]; return sum; } }
		public static Matrix operator +(Matrix p) { return new Matrix(p.a); }
		public static Matrix operator -(Matrix p) { return new Matrix(p.Row, p.Column, (i, j) => -p.a[i, j]); }
		public static Matrix operator -(Matrix p, Matrix q) { return new Matrix(p.Row, p.Column, (i, j) => p.a[i, j] - q.a[i, j]); }
		public static Matrix operator +(Matrix p, Matrix q) { return new Matrix(p.Row, p.Column, (i, j) => p.a[i, j] + q.a[i, j]); }
		public static Matrix operator *(Matrix p, long r) { return new Matrix(p.Row, p.Column, (i, j) => p.a[i, j] * r); }
		public static Matrix operator /(Matrix p, long r) { return new Matrix(p.Row, p.Column, (i, j) => p.a[i, j] / r); }
		public static Matrix operator %(Matrix p, long mod) { return new Matrix(p.Row, p.Column, (i, j) => p.a[i, j] % mod); }
		public static Matrix operator *(long r, Matrix p) { return new Matrix(p.Row, p.Column, (i, j) => p.a[i, j] * r); }
		public static Matrix operator *(Matrix p, Matrix q)
		{
			return new Matrix(p.Row, q.Column, (i, j) => { var sum = 0L; for (var k = 0; k < p.Column; k++) sum = (sum + p.a[i, k] * q.a[k, j]); return sum; });
		}
		public static Vector operator *(Matrix p, Vector v)
		{
			return new Vector(p.Row, i => { var sum = 0L; for (var j = 0; j < p.Column; j++) sum = (sum + p.a[i, j] * v[j]); return sum; });
		}
		public static Vector operator *(Vector v, Matrix p)
		{
			return new Vector(p.Column, j => { var sum = 0L; for (var i = 0; i < p.Row; i++) sum = (sum + p.a[i, j] * v[i]); return sum; });
		}
		public Matrix Transpose() { return new Matrix(Column, Row, (i, j) => a[j, i]); }
		public override string ToString()
		{
			var sb = new StringBuilder();
			for (var i = 0; i < Row; i++) for (var j = 0; j < Column; j++) sb.Append(a[i, j] + (j == Column - 1 ? "\n" : " "));
			sb.Remove(sb.Length - 1, 1);
			return sb.ToString();
		}
		public static Matrix One(int size) { return new Matrix(size, size, (i, j) => i == j ? 1 : 0); }
		public static Matrix Zero(int size) { return new Matrix(size, size); }
		public static Matrix Diagonal(long[] diag) { return new Matrix(diag.Length, diag.Length, (i, j) => i == j ? diag[i] : 0); }
		public Matrix Pow(ulong n, long mod)
		{
			var p = One(Row);
			var x = this % mod;
			while (n > 0)
		...
		public static Matrix Pow(long[] coeff, long N, long mod)
		{
			var dim = coeff.Length;
			var vec = new Vector(dim, i => i == 0 ? 1 : 0);
			var mat = new Matrix(dim, dim, (i, j) => j == i + 1 ? 1 : i == dim - 1 ? coeff[j] % mod : 0);
			for (var m = N; m > 0; m >>= 1)
			{
				if ((m & 1) == 1) vec = vec * mat;
				vec %= mod;
				var a = new Vector(dim, i => mat.a[0, i]);
				a = a * mat;
				a %= mod;
				var tmp = new Vector[dim];
				tmp[0] = a;
				for (var k = 1; k < dim; k++)
				{
					var w = tmp[k - 1][dim - 1];
					tmp[k] = new Vector(dim, i => i == 0 ? coeff[0] * w : tmp[k - 1][i - 1] + w * coeff[i]);
					tmp[k] %= mod;
				}
				mat = new Matrix(dim, dim, (i, j) => tmp[i][j] % mod);
			}
			var hoge = new Vector[dim];
			hoge[0] = vec % mod;
			for (var k = 1; k < dim; k++)
			{
				var w = hoge[k - 1][dim - 1];
				hoge[k] = new Vector(dim, i => i == 0 ? coeff[0] * w : hoge[k - 1][i - 1] + w * coeff[i]);
				hoge[k] %= mod;
			}
			return new Matrix(dim, dim, (i, j) => hoge[i][j] % mod);
		}
		public static Matrix Pow(long[] coeff, long N)
		{
			var dim = coeff.Length;
			var vec = new Vector(dim, i => i == 0 ? 1 : 0);
			var mat = new Matrix(dim, dim, (i, j) => j == i + 1 ? 1 : i == dim - 1 ? coeff[j] : 0);
			for (var m = N; m > 0; m >>= 1)
			{
				if ((m & 1) == 1) vec = vec * mat;
				var a = new Vector(dim, i => mat.a[0, i]);
				...
				mat = new Matrix(dim, dim, (i, j) => tmp[i][j]);
			}
			var hoge = new Vector[dim];
			hoge[0] = vec;
			for (var k = 1; k < dim; k++)
			...
			return new Matrix(dim, dim, (i, j) => hoge[i][j]);
		}
		public long Determinant(long mod)
		{
			if (Row == 0) return 1;
			if (Row == 1) { var tmp = a[0, 0] % mod; return tmp >= 0 ? tmp : tmp + mod; }
			if (Row == 2) { var tmp = ((a[0, 0] % mod) * (a[1, 1] % mod) - (a[0, 1] % mod) * (a[1, 0] % mod)) % mod; return tmp >= 0 ? tmp : tmp + mod; }
			var mat = new Matrix(Row, Column, (r, c) => { var tmp = a[r, c] % mod; return tmp >= 0 ? tmp : tmp + mod; });
			var prod = 1L;
			for (var j = 0; j < Row; j++)
			{
				var pos = j;
				while (pos < Row && mat.a[pos, j] == 0) pos++;
				if (pos == Row) return 0;
				if (j != pos) { mat.SwitchRow(j, pos, j); prod = mod - prod; }
				prod = (prod * mat.a[j, j]) % mod;
				var inv = Func.Inverse(mat.a[j, j], mod);
				mat.MultiplicateRow(j, inv, j);
				mat.ModRow(j, mod, j);
				for (var i = j + 1; i < Row; i++) { mat.AddRow(j, i, -mat.a[i, j], j); mat.ModRow(i, mod, j); }
			}
			return prod;
		}
		public Matrix Invert(long mod)
		{
			var inv = One(Row);
			var tmp = new Matrix(Row, Column, (r, c) => { var x = a[r, c] % mod; return x >= 0 ? x : x + mod; });
			for (var j = 0; j < Row; j++)
			{
				var pos = j;
				while (pos < Row && tmp.a[pos, j] == 0) pos++;
				if (pos == Row) return null;
				tmp.SwitchRow(j, pos);
				inv.SwitchRow(j, pos);
				var inv2 = Func.Inverse(tmp.a[j, j], mod);
				inv.MultiplicateRow(j, inv2);
				tmp.MultiplicateRow(j, inv2, j);
				inv.ModRow(j, mod);
				tmp.ModRow(j, mod, j);
				for (var i = j + 1; i < Row; i++) { inv.AddRow(j, i, -tmp.a[i, j]); inv.ModRow(i, mod); tmp.AddRow(j, i, -tmp.a[i, j], j); inv.ModRow(i, mod, j); }
			}
			for (int j = Row - 1; j > 0; j--) for (int i = j - 1; i >= 0; i--)
				{
					inv.AddRow(j, i, -tmp.a[i, j]); inv.ModRow(i, mod);
					tmp.AddRow(j, i, -tmp.a[i, j]); tmp.ModRow(i, mod);
				}
			return inv;
		}
		public void ModRow(int i, long mod, int start = 0) { for (var y = start; y < Column; y++) { a[i, y] %= mod; if (a[i, y] < 0) a[i, y] += mod; } }
		public void SwitchRow(int i, int j, int start = 0) { for (var y = start; y < Column; y++) { var tmp = a[i, y]; a[i, y] = a[j, y]; a[j, y] = tmp; } }
		public void MultiplicateRow(int i, long r, int start = 0) { for (var y = start; y < Column; y++) a[i, y] *= r; }
		public void AddRow(int from, int to, long r, int start = 0) { for (var y = start; y < Column; y++) a[to, y] += a[from, y] * r; }
	}
	class Fraction : IComparable, IComparable<Fraction>, IEquatable<Fraction>
	{
		public long Num { get; set; }
		public long Denom { get; set; }
		public bool IsInteger { get { return Denom == 1; } }
		public Fraction Inverse { get { return Num > 0 ? Init(Denom, Num) : Init(-Denom, -Num); } }
		public Fraction Abs { get { return Num > 0 ? this : -this; } }
		public Fraction() : this(0) { }
		public Fraction(long n) { Num = n; Denom = 1; }
		public Fraction(long numerator, long denominator) { Num = numerator; Denom = denominator; Reduce(); }
		private static Fraction Init(long num, long denom) { var r = new Fraction(); r.Num = num; r.Denom = denom; return r; }
		protected void Reduce() { var tmp = Func.GCD(Num, Denom) * (Denom < 0 ? -1 : 1); Num /= tmp; Denom /= tmp; }
		public static Fraction operator +(Fraction a, Fraction b) { var gcd = Func.GCD(a.Denom, b.Denom); return new Fraction(a.Num * (b.Denom / gcd) + (a.Denom / gcd) * b.Num, (a.Denom / gcd) * b.Denom); }
		public static Fraction operator -(Fraction a, Fraction b) { var gcd = Func.GCD(a.Denom, b.Denom); return new Fraction(a.Num * (b.Denom / gcd) - (a.Denom / gcd) * b.Num, (a.Denom / gcd) * b.Denom); }
		public static Fraction operator *(Fraction a, Fraction b)
		{
			var g = Func.GCD(a.Num, b.Denom);
			var h = Func.GCD(a.Denom, b.Num);
			return Init((a.Num / g) * (b.Num / h), (a.Denom / h) * (b.Denom / g));
		}
		public static Fraction operator /(Fraction a, Fraction b) { return a * b.Inverse; }
		public static Fraction operator +(Fraction a) { return Init(a.Num, a.Denom); }
		public static Fraction operator -(Fraction a) { return Init(-a.Num, a.Denom); }
		public static bool operator ==(Fraction a, Fraction b)
		{
			return ReferenceEquals(a, b) || ((object)a != null && (object)b != null && a.Num == b.Num && a.Denom == b.Denom);
		}
		public static bool operator !=(Fraction a, Fraction b)
		{
			return !ReferenceEquals(a, b) && ((object)a == null || (object)b == null || a.Num != b.Num || a.Denom != b.Denom);
		}
		public static bool operator >(Fraction a, Fraction b) { if (b.Num == 0) return a.Num > 0; var t = a / b; return b.Num > 0 ? t.Num > t.Denom : t.Num < t.Denom; }
		public static bool operator >=(Fraction a, Fraction b) { if (b.Num == 0) return a.Num >= 0; var t = a / b; return b.Num > 0 ? t.Num >= t.Denom : t.Num <= t.Denom; }
		public static bool operator <(Fraction a, Fraction b) { if (b.Num == 0) return a.Num < 0; var t = a / b; return b.Num > 0 ? t.Num < t.Denom : t.Num > t.Denom; }
		public static bool operator <=(Fraction a, Fraction b) { if (b.Num == 0) return a.Num <= 0; var t = a / b; return b.Num > 0 ? t.Num <= t.Denom : t.Num >= t.Denom; }
		public static explicit operator int(Fraction a) { return (int)(a.Num / a.Denom); }
		public static explicit operator long(Fraction a) { return (long)(a.Num / a.Denom); }
		public static explicit operator double(Fraction a) { return (double)a.Num / a.Denom; }
		public static explicit operator decimal(Fraction a) { return (decimal)a.Num / a.Denom; }
		public static implicit operator Fraction(int a) { return new Fraction(a); }
		public static implicit operator Fraction(long a) { return new Fraction(a); }
		public override string ToString() { return IsInteger ? Num.ToString() : Num + "/" + Denom; }
		public override int GetHashCode() { return Num.GetHashCode() ^ Denom.GetHashCode(); }
		public override bool Equals(object x) { if (x == null) return false; var p = x as Fraction; return (object)p != null && Num * p.Denom == p.Num * Denom; }
		public int CompareTo(object x) { return x == null ? 1 : CompareTo((Fraction)x); }
		public int CompareTo(Fraction r) { if (r.Num == 0) return Num.CompareTo(0); var t = this / r; return (r.Num > 0 ? -1 : 1) * t.Num.CompareTo(t.Denom); }
		public bool Equals(Fraction r) { return this == r; }
		public Fraction Pow(ulong n)
		{
			var np = 1L;
			var dp = 1L;
			var nx = Num;
			...
			while (n > 0)
			{
				if ((n & 1) == 1) { np *= nx; dp *= dx; }
				n >>= 1;
				nx *= nx;
				dx *= dx;
			}
			return new Fraction(np, dp);
		}
		// when parse failed, return null
		public static Fraction Parse(string s)
		{
			long tmp;
			if (long.TryParse(s, out tmp)) return tmp;
			...
			else
			{
				...
				var split = 0;
				while (split < s.Length && s[split] != '/' && s[split] != '÷' && s[split] != '?') split++;
				if (split == s.Length || !char.IsDigit(s[split + 1])) return null;
				string num_s = s.Substring(0, split), denom_s = s.Substring(split + 1, s.Length - split - 1);
				long num, denom;
				if (!long.TryParse(num_s, out num) || !long.TryParse(denom_s, out denom) || denom == 0) return null;
				result.Num = num * flag;
				result.Denom = denom;
				result.Reduce();
				return result;
	...
	class BigFraction : IComparable, IComparable<BigFraction>, IEquatable<BigFraction>
	{
		public BigInteger Num { get; set; }
		public BigInteger Denom { get; set; }
		public bool IsInteger { get { return Denom == 1; } }
		public BigFraction Inverse { get { return Num > 0 ? Init(Denom, Num) : Init(-Denom, -Num); } }
		public BigFraction Abs { get { return Num > 0 ? this : -this; } }
		public BigFraction() : this(0) { }
		public BigFraction(BigInteger n) { Num = n; Denom = 1; }
		public BigFraction(BigInteger numerator, BigInteger denominator) { Num = numerator; Denom = denominator; Reduce(); }
		private static BigFraction Init(BigInteger num, BigInteger denom) { var r = new BigFraction(); r.Num = num; r.Denom = denom; return r; }
		protected void Reduce() { var tmp = Func.GCD(Num, Denom) * (Denom < 0 ? -1 : 1); Num /= tmp; Denom /= tmp; }
		public static BigFraction operator +(BigFraction a, BigFraction b) { var gcd = Func.GCD(a.Denom, b.Denom); return new BigFraction(a.Num * (b.Denom / gcd) + (a.Denom / gcd) * b.Num, (a.Denom / gcd) * b.Denom); }
		public static BigFraction operator -(BigFraction a, BigFraction b) { var gcd = Func.GCD(a.Denom, b.Denom); return new BigFraction(a.Num * (b.Denom / gcd) - (a.Denom / gcd) * b.Num, (a.Denom / gcd) * b.Denom); }
		...
		public static BigFraction operator /(BigFraction a, BigFraction b) { return a * b.Inverse; }
		public static BigFraction operator +(BigFraction a) { return Init(a.Num, a.Denom); }
		public static BigFraction operator -(BigFraction a) { return Init(-a.Num, a.Denom); }
		...
		public static bool operator >(BigFraction a, BigFraction b) { if (b.Num == 0) return a.Num > 0; var t = a / b; return b.Num > 0 ? t.Num > t.Denom : t.Num < t.Denom; }
		public static bool operator >=(BigFraction a, BigFraction b) { if (b.Num == 0) return a.Num >= 0; var t = a / b; return b.Num > 0 ? t.Num >= t.Denom : t.Num <= t.Denom; }
		public static bool operator <(BigFraction a, BigFraction b) { if (b.Num == 0) return a.Num < 0; var t = a / b; return b.Num > 0 ? t.Num < t.Denom : t.Num > t.Denom; }
		public static bool operator <=(BigFraction a, BigFraction b) { if (b.Num == 0) return a.Num <= 0; var t = a / b; return b.Num > 0 ? t.Num <= t.Denom : t.Num >= t.Denom; }
		public static explicit operator int(BigFraction a) { return (int)(a.Num / a.Denom); }
		public static explicit operator long(BigFraction a) { return (long)(a.Num / a.Denom); }
		public static explicit operator BigInteger(BigFraction a) { return a.Num / a.Denom; }
		public static explicit operator double(BigFraction a) { return (double)a.Num / (double)a.Denom; }
		public static explicit operator decimal(BigFraction a) { return (decimal)a.Num / (decimal)a.Denom; }
		public static implicit operator BigFraction(int a) { return new BigFraction(a); }
		public static implicit operator BigFraction(long a) { return new BigFraction(a); }
		public static implicit operator BigFraction(BigInteger a) { return new BigFraction(a); }
		public override string ToString() { return IsInteger ? Num.ToString() : Num + "/" + Denom; }
		public override int GetHashCode() { return Num.GetHashCode() ^ Denom.GetHashCode(); }
		public override bool Equals(object x) { if (x == null) return false; var p = x as BigFraction; return (object)p != null && Num * p.Denom == p.Num * Denom; }
		public int CompareTo(object x) { return x == null ? 1 : CompareTo((BigFraction)x); }
		public int CompareTo(BigFraction r) { if (r.Num == 0) return Num.CompareTo(0); var t = this / r; return (r.Num > 0 ? -1 : 1) * t.Num.CompareTo(t.Denom); }
		public bool Equals(BigFraction r) { return this == r; }
		public BigFraction Pow(ulong n)
		{
			var np = BigInteger.One;
		...
		public static BigFraction Parse(string s)
		{
			...
			{
				BigInteger tmp;
				if (BigInteger.TryParse(s, out tmp)) return tmp;
				return null;
			}
			if (s.Length < 3) return null;
			else if (!s.Contains("."))
			{
				...
				var split = 0;
				while (split < s.Length && s[split] != '/' && s[split] != '÷' && s[split] != '?') split++;
				if (split >= s.Length || (split + 1 < s.Length && !char.IsDigit(s[split + 1]))) return null;
				string num_s = s.Substring(0, split), denom_s = s.Substring(split + 1, s.Length - split - 1);
				BigInteger num, denom;
				if (!BigInteger.TryParse(num_s, out num) || !BigInteger.TryParse(denom_s, out denom) || denom == 0) return null;
			...
			else
			{
				...
				var split = 0;
				while (split < s.Length && s[split] != '.' && s[split] != '?') split++;
				if (split + 1 >= s.Length || !char.IsDigit(s[split + 1])) return null;
				var exp10 = s.Length - split - 1;
				s = s.Remove(split, 1);
				BigInteger num;
				if (!BigInteger.TryParse(s, out num)) return null;
				var result = new BigFraction();
				result.Num = num * flag;
				result.Denom = BigInteger.Pow(10, exp10);
				result.Reduce();
				return result;
			}
		}
	}
	struct ModInteger : IEquatable<ModInteger>, IFormattable
	{
		const long Mod = 1000000007;
		public long x;
		public ModInteger(long x) { this.x = x % Mod; }
		public static ModInteger operator +(ModInteger a) { return new ModInteger(a.x); }
		public static ModInteger operator -(ModInteger a) { return new ModInteger(-a.x); }
		public static ModInteger operator +(ModInteger a, ModInteger b) { return (a.x + b.x) % Mod; }
		public static ModInteger operator -(ModInteger a, ModInteger b) { return (a.x - b.x) % Mod; }
		public static ModInteger operator *(ModInteger a, ModInteger b) { return (a.x * b.x) % Mod; }
		public static ModInteger operator /(ModInteger a, ModInteger b) { return a * b.Inverse; }
		public static bool operator ==(ModInteger a, ModInteger b) { return (a.x - b.x) % Mod == 0; }
		public static bool operator !=(ModInteger a, ModInteger b) { return (a.x - b.x) % Mod != 0; }
		public static explicit operator long(ModInteger a) { return a.x < 0 ? a.x + Mod : a.x; }
		public static implicit operator ModInteger(long a) { return new ModInteger(a); }
		public override bool Equals(object x) { if (x == null) return false; var p = (ModInteger)x; return p == this; }
		public override int GetHashCode() { return x.GetHashCode(); }
		public override string ToString() { return x.ToString(); }
		public bool Equals(ModInteger other) { return this == other; }
		public string ToString(string format, IFormatProvider formatProvider) { return ((long)this).ToString(format, formatProvider); }
		public ModInteger Pow(long exponent) { return Func.Pow(x, exponent, Mod); }
		public ModInteger Inverse { get { return new ModInteger(Func.Inverse(x, Mod)); } }
	}
	class MyInteger : IEquatable<MyInteger>, IComparable<MyInteger>, IComparable
	{
		public const int Base = 1000000000;    // 2*Base < int.MaxValue
											   //public const int Base = 200;
		private const int MaxDigit = Base - 1;
		public const int LogBase = 9;
		// 0 -> isPositive = true, digits = { }
		// digits[digits.Length - 1] != 0
		// 0 <= digits[i] < Base
		// Number = (isPositive ? 1 : -1) * (digits[0] + Base * (digits[1] + Base * (digits[2] + ...)))
		public bool isPositive;
		public int[] digits;
		public MyInteger() { digits = new int[0]; isPositive = true; }
		public MyInteger(long x) { var y = (MyInteger)x; isPositive = y.isPositive; digits = y.digits; }
		public MyInteger(ulong x) { var y = (MyInteger)x; isPositive = y.isPositive; digits = y.digits; }
		public MyInteger(bool isPositive, int[] digits) { this.isPositive = isPositive; this.digits = new int[digits.Length]; digits.CopyTo(this.digits, 0); }
		public bool IsEven { get { return digits.Length == 0 || digits[0] % 2 == 0; } }
		public bool IsZero { get { return digits.Length == 0; } }
		public bool IsOne { get { return digits.Length == 1 && isPositive && digits[0] == 1; } }
		public int Sign { get { return isPositive ? digits.Length == 0 ? 0 : 1 : -1; } }
		public MyInteger Abs { get { return new MyInteger(true, digits); } }
		public void Negate() { if (digits.Length > 0) isPositive = !isPositive; }
		public static MyInteger Max(MyInteger a, MyInteger b) { return a >= b ? a : b; }
		public static MyInteger Min(MyInteger a, MyInteger b) { return a <= b ? a : b; }
		public static readonly MyInteger MinusOne = new MyInteger(false, new int[] { 1 });
		public static readonly MyInteger Zero = new MyInteger();
		public static readonly MyInteger One = new MyInteger(true, new int[] { 1 });
		public static readonly MyInteger Two = new MyInteger(true, new int[] { 2 });
		public bool Check
		{
			get
			{
				if (IsZero) return isPositive;
				if (digits[digits.Length - 1] == 0) return false;
				for (var i = 0; i < digits.Length; i++) if (digits[i] < 0 || digits[i] >= Base) return false;
				return true;
			}
		}
		public void Increment()
		{
			...
			if (isPositive)
			{
				if (N == 0) digits = new int[] { 1 };
				else
				{
					var i = 0;
					while (i < N && digits[i] == MaxDigit) i++;
					if (i == N) { digits = new int[N + 1]; digits[N] = 1; }
					else
					{
						digits[i--]++;
						for (; i >= 0; i--) digits[i] = 0;
					}
				}
			}
			else
			{
				if (N == 1 && digits[0] == 1) { isPositive = true; digits = new int[0]; }
				else
				{
					var i = 0;
					while (digits[i] == 0) i++;
					...
					{
						digits = new int[N - 1];
						for (i = 0; i < N - 1; i++) digits[i] = MaxDigit;
					}
					else
					{
						digits[i--]--;
						for (; i >= 0; i--) digits[i] = MaxDigit;
					}
				}
			}
		}
		public void Decrement() { Negate(); Increment(); Negate(); }
		public void Add(MyInteger a)
		{
			var N = digits.Length;
			var M = a.digits.Length;
			if (M == 0) return;
			else if (N == 0) { isPositive = a.isPositive; digits = new int[M]; a.digits.CopyTo(digits, 0); }
			else if (isPositive == a.isPositive)
			{
				if (N < M) digits = Add(a.digits, digits).ToArray();
				else
				{
					var k = 0;
					for (var i = 0; i < M; i++)
					{
						digits[i] += k + a.digits[i];
						if (digits[i] >= Base) { digits[i] -= Base; k = 1; }
						else k = 0;
					}
					for (var i = M; i < N; i++)
					{
					...
					if (k > 0) { var tmp = new int[N + 1]; for (var i = 0; i < N; i++) tmp[i] = digits[i]; tmp[N] = 1; digits = tmp; }
				}
			}
			else
			{
				if (N < M) { var c = this + a; isPositive = c.isPositive; digits = c.digits; }
				else if (N > M)
				{
					var k = 0;
					var z = -1;
					for (var i = 0; i < M; i++)
					{
						digits[i] -= (a.digits[i] + k);
						if (digits[i] < 0) { digits[i] += Base; k = 1; }
						else k = 0;
						if (digits[i] != 0) z = i;
					}
					for (var i = M; i < N; i++)
					...
					{
						var tmp = new int[z + 1];
						for (var i = 0; i <= z; i++) tmp[i] = digits[i];
						digits = tmp;
					}
				}
				else
				{
					var j = N - 1;
					while (j >= 0 && digits[j] == a.digits[j]) j--;
					if (j < 0) { isPositive = true; digits = new int[0]; }
					else
					{
						var flag = digits[j] >= a.digits[j];
						var p = flag ? digits : a.digits;
						var q = !flag ? digits : a.digits;
						var k = 0;
						var z = -1;
						for (var i = 0; i <= j; i++)
						{
		...
		public void Subtract(MyInteger a) { a.Negate(); Add(a); a.Negate(); }
		public static MyInteger Add(MyInteger a, MyInteger b)
		{
			...
			{
				var c = Add(a.digits, b.digits);
				return new MyInteger(a.isPositive, c.ToArray());
			}
			else
			{
				var c = Subtract(a.digits, b.digits);
				if (c.Item2.Count == 0) return new MyInteger();
				return new MyInteger(!a.isPositive ^ c.Item1, c.Item2.ToArray());
			}
		}
		public static MyInteger Subtract(MyInteger a, MyInteger b) { b.Negate(); var res = a + b; b.Negate(); return res; }
		...
		public static Tuple<bool, List<int>> Subtract(int[] a, int[] b)
		{
			var N = a.Length;
			var M = b.Length;
			if (N < M) { var tmp = Subtract(b, a); return new Tuple<bool, List<int>>(!tmp.Item1, tmp.Item2); }
			else if (N > M)
			{
				var c = new List<int>(N);
				var k = 0;
				...
				if (c[c.Count - 1] == 0) c.RemoveRange(z + 1, c.Count - z - 1);
				return new Tuple<bool, List<int>>(true, c);
			}
			else
			{
				var c = new List<int>(N);
				var j = N - 1;
				while (j >= 0 && a[j] == b[j]) j--;
				if (j < 0) return new Tuple<bool, List<int>>(true, new List<int>());
				var flag = a[j] >= b[j];
				if (!flag) { var tmp = a; a = b; b = tmp; }
				var k = 0;
				var z = -1;
				for (var i = 0; i <= j; i++)
				{
				...
				if (c[c.Count - 1] == 0) c.RemoveRange(z + 1, c.Count - z - 1);
				return new Tuple<bool, List<int>>(flag, c);
			}
		}
		public static int[] Multiply(int[] a, int[] b)
		{
			...
			for (var i = 0; i < N; i++)
				...
				{
					long q;
					var p = (int)Math.DivRem((long)a[i] * b[j], Base, out q);
					c[i + j] += (int)q;
					c[i + j + 1] += p;
				}
			var k = 0L;
			...
			{
				c[i] += k;
				if (c[i] >= Base) { k = c[i] / Base; c[i] %= Base; }
				else k = 0;
			}
			var z = c.Length - 1;
			while (c[z] == 0) z--;
			var res = new int[z + 1];
			for (var i = 0; i <= z; i++) res[i] = (int)c[i];
			return res;
		}
		public static MyInteger Divide(MyInteger a, MyInteger b)
		{
			if (b.IsZero) throw new DivideByZeroException();
			else if (a.IsZero) return new MyInteger();
			else if (a.isPositive) return new MyInteger(b.isPositive, Divide(a.digits, b.digits));
			else
			{
				var tmp = DivRem(a.digits, b.digits);
				var x = new MyInteger(true, tmp.Item1);
				if (tmp.Item2.Length != 0) x.Increment();
				if (!b.isPositive) x.Negate();
				return x;
			}
		}
		public static MyInteger Remainder(MyInteger a, MyInteger b)
		{
			if (b.IsZero) throw new DivideByZeroException();
			else if (a.IsZero) return new MyInteger();
			else if (a.isPositive) return new MyInteger(b.isPositive, DivRem(a.digits, b.digits).Item2);
			else
			{
				var tmp = DivRem(a.digits, b.digits);
				var x = new MyInteger(true, tmp.Item2);
				if (tmp.Item2.Length != 0) x.Add(b);
				if (!b.isPositive) x.Negate();
				return x;
			}
		}
		public static MyInteger DivRem(MyInteger a, MyInteger b, out MyInteger r) { var t = DivRem(a, b); r = t.Item2; return t.Item1; }
		public static Tuple<MyInteger, MyInteger> DivRem(MyInteger a, MyInteger b)
		{
			if (b.IsZero) throw new DivideByZeroException();
			else if (a.IsZero) return new Tuple<MyInteger, MyInteger>(new MyInteger(), new MyInteger());
			else if (a.isPositive)
			{
				var t = DivRem(a.digits, b.digits);
				return new Tuple<MyInteger, MyInteger>(new MyInteger(b.isPositive, t.Item1), new MyInteger(b.isPositive, t.Item2));
			}
			else
			{
				var tmp = DivRem(a.digits, b.digits);
				var q = new MyInteger(true, tmp.Item1);
				var r = new MyInteger(true, tmp.Item2);
				if (tmp.Item2.Length != 0) { q.Increment(); r.Add(b); }
				if (!b.isPositive) { q.Negate(); r.Negate(); }
				return new Tuple<MyInteger, MyInteger>(q, r);
			}
		}
		...
		private static Tuple<int[], int[]> DivRem(int[] a, int[] b)
		{
			return null;
		}
		public static MyInteger RightShift(MyInteger a, int x)
		{
			if (x < 0) return LeftShift(a, -x);
			else if (x == 0) return +a;
			else return new MyInteger(a.isPositive, RightShift(a.digits, x));
		}
		public static MyInteger LeftShift(MyInteger a, int x)
		{
			if (x < 0) return RightShift(a, -x);
			else if (x == 0) return +a;
			else return new MyInteger(a.isPositive, LeftShift(a.digits, x));
		}
		private static int[] RightShift(int[] a, int x)
		{
			var b = new int[a.Length - x];
			for (var i = 0; i < b.Length; i++) b[i] = a[i + x];
			return b;
		}
		private static int[] LeftShift(int[] a, int x)
		{
			var b = new int[a.Length + x];
			for (var i = 0; i < a.Length; i++) b[i + x] = a[i];
			return b;
		}
		public static MyInteger Pow(MyInteger a, MyInteger b)
		{
			if (a.IsZero)
			{
				if (!b.isPositive || b.IsZero) throw new DivideByZeroException();
				else return new MyInteger();
			}
			else if (a.digits.Length == 1 && a.digits[0] == 1)
			{
				if (a.isPositive) return +One;
				else return new MyInteger(b.IsEven, new int[] { 1 });
			}
			else if (b.IsZero) return +One;
			else if (!b.isPositive) return new MyInteger();
			var p = +One;
			var x = a;
			while (b.digits.Length > 0)
			{
				if ((b.digits[0] & 1) == 1) p *= x;
				b /= Two;
				x *= x;
			}
			return p;
		}
		public static MyInteger Multiply(MyInteger a, MyInteger b)
		{
			return a.IsZero || b.IsZero ? new MyInteger() : new MyInteger(a.isPositive == b.isPositive, Multiply(a.digits, b.digits));
		}

		#region ???????????

		public static MyInteger operator +(MyInteger a) { return new MyInteger(a.isPositive, a.digits); }
		public static MyInteger operator -(MyInteger a) { return a.IsZero ? new MyInteger() : new MyInteger(!a.isPositive, a.digits); }
		public static MyInteger operator ++(MyInteger c) { var d = +c; d.Increment(); return d; }
		public static MyInteger operator --(MyInteger c) { var d = +c; d.Decrement(); return d; }
		public static MyInteger operator +(MyInteger a, MyInteger b) { return Add(a, b); }
		public static MyInteger operator -(MyInteger a, MyInteger b) { return Subtract(a, b); }
		public static MyInteger operator *(MyInteger a, MyInteger b) { return Multiply(a, b); }
		public static MyInteger operator /(MyInteger a, MyInteger b) { return Divide(a, b); }
		public static MyInteger operator %(MyInteger a, MyInteger b) { return Remainder(a, b); }
		public static bool operator ==(MyInteger a, MyInteger b) { return Compare(a, b) == 0; }
		public static bool operator !=(MyInteger a, MyInteger b) { return Compare(a, b) != 0; }
		public static bool operator <(MyInteger a, MyInteger b) { return Compare(a, b) < 0; }
		public static bool operator >(MyInteger a, MyInteger b) { return Compare(a, b) > 0; }
		public static bool operator <=(MyInteger a, MyInteger b) { return Compare(a, b) <= 0; }
		public static bool operator >=(MyInteger a, MyInteger b) { return Compare(a, b) >= 0; }
		public static MyInteger operator >>(MyInteger a, int x) { return RightShift(a, x); }
		public static MyInteger operator <<(MyInteger a, int x) { return LeftShift(a, x); }
		public static MyInteger operator ^(MyInteger a, MyInteger b) { return Pow(a, b); }

		#region ????

		public static explicit operator ulong(MyInteger a)
		{
			if (!a.isPositive) throw new OverflowException();
			var x = 0uL;
			var e = 1uL;
			var lim = a.digits.Length - 1;
			for (var i = 0; i < lim; i++) { x += e * (ulong)a.digits[i]; e *= Base; }
			x += e * (ulong)a.digits[lim];
			return x;
		}
		public static explicit operator long(MyInteger a)
		{
			if (a.IsZero) return 0;
			var x = 0L;
			var e = 1L;
			var lim = a.digits.Length - 1;
			for (var i = 0; i < lim; i++) { x += e * a.digits[i]; e *= Base; }
			x += e * a.digits[lim];
			return x * (a.isPositive ? 1 : -1);
		}
		public static explicit operator uint(MyInteger a)
		{
			if (!a.isPositive) throw new OverflowException();
			var x = 0u;
			var e = 1u;
			var lim = a.digits.Length - 1;
			for (var i = 0; i < lim; i++) { x += e * (uint)a.digits[i]; e *= Base; }
			x += e * (uint)a.digits[lim];
			return x;
		}
		public static explicit operator int(MyInteger a)
		{
			if (a.IsZero) return 0;
			var x = 0;
			var e = 1;
		...
		public static implicit operator MyInteger(ulong x)
		{
			if (x == 0) return new MyInteger();
			var a = new List<int>();
			while (x > 0) { a.Add((int)(x % Base)); x /= Base; }
			return new MyInteger(true, a.ToArray());
		}
		public static implicit operator MyInteger(long x)
		{
			if (x == 0) return new MyInteger();
			var flag = x > 0 ? 1 : -1;
			x *= flag;
			var a = new List<int>();
			while (x > 0) { a.Add((int)(x % Base)); x /= Base; }
			return new MyInteger(flag == 1, a.ToArray());
		}
		public static implicit operator MyInteger(uint x)
		{
			if (x == 0) return new MyInteger();
			var a = new List<int>();
			while (x > 0) { a.Add((int)(x % Base)); x /= Base; }
		...
		public static implicit operator MyInteger(int x)
		{
			if (x == 0) return new MyInteger();
			var flag = x > 0 ? 1 : -1;
			x *= flag;
			var a = new List<int>();
			while (x > 0) { a.Add(x % Base); x /= Base; }
			return new MyInteger(flag == 1, a.ToArray());
		}
		...
		public override bool Equals(object x) { if (x == null) return false; var p = x as MyInteger; return (object)p != null && this == p; }
		public override int GetHashCode()
		{
			var h = isPositive.GetHashCode();
			h ^= digits.Length.GetHashCode();
			for (var i = 0; i < digits.Length; i++) h ^= digits[i];
			return h;
		}
		public override string ToString()
		{
			...
			var i = N - 1;
			sb.Append(digits[i--]);
			for (; i >= 0; i--) sb.Append(digits[i].ToString().PadLeft(LogBase, '0'));
			return sb.ToString();
		}
		public MyInteger Parse(string str)
		{
			...
			else if (str[i] < '0' || str[i] > '9') throw new FormatException();
			while (i < str.Length && str[i] == '0') i++;
			if (i == str.Length) return new MyInteger();
			var x = new int[(str.Length - i + LogBase - 1) / LogBase];
			var rem = (str.Length - i) - x.Length * LogBase;
			for (; i < str.Length; i += LogBase)
			{

			}
			return new MyInteger(flag, x);
		}
		public bool Equals(MyInteger other) { return this == other; }
		public static int Compare(MyInteger x, MyInteger y)
		{
			if (ReferenceEquals(x, y)) return 0;
			else if ((object)x == null) return -1;
			else if ((object)y == null) return 1;
			var N = x.digits.Length;
			var M = y.digits.Length;
			if (x.isPositive && y.isPositive)
			{
				if (N < M) return -1;
				else if (N > M) return 1;
				else if (x.IsZero) return 0;
				var j = N - 1;
				while (j >= 0 && x.digits[j] == y.digits[j]) j--;
				return j >= 0 ? x.digits[j].CompareTo(y.digits[j]) : 0;
			}
			else if (x.isPositive) return 1;
			else if (y.isPositive) return -1;
			else
			{
				if (N < M) return 1;
				else if (N > M) return -1;
				var j = N - 1;
				while (j >= 0 && x.digits[j] == y.digits[j]) j--;
				return j >= 0 ? y.digits[j].CompareTo(x.digits[j]) : 0;
			}
		}
		public int CompareTo(MyInteger x) { return Compare(this, x); }
		public int CompareTo(object x) { return x == null ? 1 : Compare(this, x as MyInteger); }
