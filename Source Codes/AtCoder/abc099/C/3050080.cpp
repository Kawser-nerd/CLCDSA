...
void init(void){
    ...
    for (int i = 1; i < 100005; i++){
        int min = 100005;
        for (int j = 0; i - mod_pow(9, j) >= 0; j++){
            if (min > kyori[i - mod_pow(9, j)]) min = kyori[i - mod_pow(9, j)];
        }
        for (int j = 0; i - mod_pow(6, j) >= 0; j++){
            if (min > kyori[i - mod_pow(6, j)]) min = kyori[i - mod_pow(6, j)];
        }
        if (kyori[i] > min + 1) kyori[i] = min + 1;
    }
}
