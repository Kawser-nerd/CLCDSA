...
int B,n,A[100];
int64 f[100][100][100];
int64 g[100][10000],h[100][10000];
int64 factor[100];

int64 divide(int64 a,int64 b)
{
	int64 x,y;
	euclide<int64>(b,MODE,x,y);
	x*=a;
	x%=MODE;
	if (x<0) x+=MODE;
	return x;
}
int64 calc(int s,int b,int exp)
{
	int64 R1=g[s+b][exp];
	int64 R2=0;
	if (s>0)
		R2=(g[s+b-1][exp]*(int64)s)%MODE;
	int64 R=divide(R1+R2,factor[b]);
	return R;
}
int64 solve(int d,int s,int p)
{
	if (d<0) return (int)(p==0);
	int64 &ret=f[d][s][p];
	if (ret>=0) return ret;
	ret=0;
	for (int s2=s;s2<=B;s2++)
		for (int p2=0;p2<=B;p2++)
		{
			int exp=p*B+A[d]-p2;
			if (exp<0) continue;
			int64 t1=calc(s,s2-s,exp);
			if (t1==0) continue;
			int64 t2=solve(d-1,s2,p2);
			if (t2==0) continue;
			t1%=MODE;
			t2%=MODE;
			ret=(ret+t1*t2)%MODE;
...
int main(int argc,char **args)
{
	...
	{
		__filename="D";
//		__filename="D-small-attempt0";
//		__filename="D-large";
	}
	else
	{
		if (argc!=3){printf("bad arguments\n");return 0;}
		__filename="D-large";__mod=atoi(args[1]);__key=atoi(args[2]);
	}
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		cin>>value>>B;
		n=0;
		for (int64 m=value;m>0;m/=B) A[n++]=(int)(m%B);
		...
		for (int i=1;i<100;i++)
		{
			factor[i]=factor[i-1]*(int64)i;
			factor[i]%=MODE;
		}
		for (int key=1;key<B;key++)
		{
			memset(h,0,sizeof(h));
			for (int i=0;i<=B;i++)
				for (int j=0;j<10000;j++) if (g[i][j]>0)
					{
						h[i][j]=(h[i][j]+g[i][j])%MODE;
						if (i+1<=B && j+key<10000)
							h[i+1][j+key]=(h[i+1][j+key]+g[i][j])%MODE;
					}
			memcpy(g,h,sizeof(g));
		}
		for (int i=0;i<=B;i++) for (int j=0;j<10000;j++) if (g[i][j]>0)
			g[i][j]=(g[i][j]*factor[i])%MODE;
		memset(f,255,sizeof(f));
		int64 R=solve(n-1,0,0);
		printf(" %d\n",(int)R);
		fflush(stdout);
	}
	return 0;
