...
int func_main(){
    ...
    while (!(var_i > var_B)){
        var_tmp = Var("");
        var_j = Var(var_i.asString().size());
        for (int cnt_0=0;cnt_0<Var(var_i.asString().size()).asNumber();cnt_0++){
            var_tmp = Var(var_tmp.asString() + letterOf(var_j, var_i).asString());
            var_j = (var_j - Var(1));
        }
        if ((var_i == var_tmp)){
            var_ans = (var_ans + Var(1));
        }
        var_i = (var_i + Var(1));
    }
    cout << var_ans << endl;
