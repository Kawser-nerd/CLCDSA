...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        Arrays.fill(memow, -1);
        Arrays.fill(memob, -1);
        System.out.println(mSum(atw(0, -1) , atb(0, -1)));
    }

    static long[] memow;
    static long[] memob;

    public static long atw(int i, int par) {
        if (memow[i] >= 0) return memow[i];
        long res = 1;
        for (Integer t : nodes[i]) {
            if (t == par) continue;
            long sum = mSum(atw(t, i), atb(t, i));
            res = mMul(res, sum);
        }
        return memow[i] = res;
    }

    public static long atb(int i, int par) {
        if (memob[i] >= 0) return memob[i];
        long res = 1;
        for (Integer t : nodes[i]) {
            if (t == par) continue;
            res = mMul(res, atw(t, i));
        }
        return memob[i] = res;
    }
