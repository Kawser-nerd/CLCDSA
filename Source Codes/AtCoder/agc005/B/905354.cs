...
class Program
{
    ...
    static void Main()
    {
        ...
        var ufr = new UnionFind(n + 1);
        var ufl = new UnionFind(n + 1);
        long sum = 0;
        for (int i = n; i > 0; i--)
        {
            int j = id[i];
            ...
            sum += i * (ufr.unionnum(j) + ufl.unionnum(j) - 1 + (ufr.unionnum(j) - 1) * (long)(ufl.unionnum(j) - 1));
            if (j + 1 < n && i < a[j + 1])
                ufl.union(j, j + 1);
            if (j > 0 && i < a[j - 1])
                ufr.union(j, j - 1);
...
class UnionFind
{
    ...
    public bool isunion(int x, int y) { return find(x) == find(y); }
    public int unionnum(int x) { return num[find(x)]; }
