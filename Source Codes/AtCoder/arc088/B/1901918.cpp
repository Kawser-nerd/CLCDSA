...
int solve(string str) {
	...
	REP(i, parse.size()) {
		if (parse[i].first == 1) {
			int hasamu, kasaneru;
			hasamu = min(sum + parse[i].second, (int)str.length() - sum);
			kasaneru = max(sum, (int)str.length() - sum - parse[i].second);

			rin = min(rin, max(hasamu, kasaneru));
		}
