...
public static class Program
{
    enum State
    {
        Blank, ALeft, ARight, BUpper, BLower
    }
    ...
    public static void Solve()
    {
        ...
        if (n == 1 && b > 0) Answer(NO);
        if (m == 1 && a > 0) Answer(NO);

        var x = (m / 2) * (n / 2);
        var y = 0;
        var field = new State[n, m];
        var i = 0;
        var j = 0;
        ...
        if (n % 2 == 1 && m % 2 == 1)
        {
            ...
            for (; j <= m - 3 && a > 0; j += 2)
            {
                field[i, j] = State.ALeft;
                field[i, j + 1] = State.ARight;
                a--;
            }
            ...
            for (; i <= n - 2 && b > 0; i += 2)
            {
                field[i, j] = State.BUpper;
                field[i + 1, j] = State.BLower;
                b--;
            }
        ...
        while (x > 0 && (a > 0 || b > 0))
        {
            ...
            if (a >= b && a > 0)
            {
                if (i > n - (n % 2) - 2) goto exitLoop;

                if (a >= 2)
                {
                    field[i + 1, j] = State.ALeft;
                    field[i + 1, j + 1] = State.ARight;
                    a--;
                }
            ...
            {
                if (j > m - (m % 2) - 2) goto exitLoop;

                if (b >= 2)
                {
                    field[i, j + 1] = State.BUpper;
                    field[i + 1, j + 1] = State.BLower;
                    b--;
                }
        ...
        if (y == 0 && (a > 0 || b > 0)) Answer(NO);

        if (a == 2)
        {
            i = 0;
            j = m - (m % 2) - 2;
            field[i, j] = field[i + 1, j] = State.ALeft;
            field[i, j + 1] = field[i + 1, j + 1] = State.ARight;
            a -= 2;
        }
        else if (b == 2)
        {
            i = 0;
            j = m - (m % 2) - 2;
            field[i, j] = field[i, j + 1] = State.BUpper;
            field[i + 1, j] = field[i + 1, j + 1] = State.BLower;
            b -= 2;
        }
        else if (a <= 1 && b <= 1)
        {
        ...
        if (a > 0 || b > 0) Answer(NO);

        Console.WriteLine(YES);

        foreach (var p in Range(n))
        {
            foreach (var q in Range(m))
            {
                switch (field[p, q])
                {
                    case State.Blank:
...
public static partial class Extensions
{
    ...
    public static T[] SortDescending<T>(this T[] array)
        where T : IComparable<T>
    {
        Array.Sort(array, Comparer<T>.Create((x, y) => y.CompareTo(x)));
        return array;
    }

    public static T[] SortBy<T, TKey>(this T[] array, Func<T, TKey> keySelector)
        where TKey : IComparable<TKey>
    {
        Array.Sort(array, Comparer<T>.Create((x, y) => keySelector(x).CompareTo(keySelector(y))));
        return array;
    }

    public static T[] SortByDescending<T, TKey>(this T[] array, Func<T, TKey> keySelector)
        where TKey : IComparable<TKey>
    {
        Array.Sort(array, Comparer<T>.Create((x, y) => keySelector(y).CompareTo(keySelector(x))));
        return array;
    }
}
