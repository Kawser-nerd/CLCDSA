...
int64 solve()
{
	...
	while (1)
	{
		...
		if (cnt%2==1)
		{
			...
			S[pos]=0;
		if (S[pos-1]>0)
			M.erase(MP(S[pos-1],pos-1));
		if (S[pos+1]>0)
			M.erase(MP(S[pos+1],pos+1));
		S[pos-1]+=cnt/2;
		M.insert(MP(S[pos-1],pos-1));
		S[pos+1]+=cnt/2;
		M.insert(MP(S[pos+1],pos+1));
	}
	return R;
...
int main(int argc,char **args)
{
	...
	{
//		__filename="C";
		__filename="C-small-attempt1";
//		__filename="C-large";
	}
	else
	{
		if (argc!=3){printf("bad arguments\n");return 0;}
		__filename="C-large";__mod=atoi(args[1]);__key=atoi(args[2]);
	}
