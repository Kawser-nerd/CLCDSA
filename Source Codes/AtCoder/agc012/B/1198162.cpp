...
int N,M,K,A,B,E,V[NMAX], C[NMAX], D[NMAX];
int dp[NMAX][12] = {};
    ...
    for (int i = 1; i <= K; ++i)
    {
        ...
        for(int dist = D[i]; dist >= 0; --dist){
            ...
            while(!q.empty()) {
                ...
                // cout << v << " " << len << endl;
                for(auto to: G[v]){
                    if(dist && !dp[to][dist-1]) qn.push(to);
                }
                for(int j = dist; !dp[v][j] && j >= 0 ; --j) dp[v][j] = i;
            }
            if(qn.empty())break;
            q = qn;
        }
