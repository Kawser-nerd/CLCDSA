...
class Main {
	void solve () {
		...
		int dist = calcManhatDistance(0, 0, x, y);
		
		if (t == 1) out.println(dist + count);
		else {
			if (dist%2 == count%2) out.println(Math.max(0, dist-count));
			else out.println(Math.max(1, dist-count));
		}
	}
