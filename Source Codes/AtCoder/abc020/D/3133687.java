...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        for (int i = 1; i * i <= K; i++) {
            if (K % i == 0) {
                div.add(i);
                if (i * i != K) div.add((int) K / i);
            }
        }
        long res = 0;
        for (Integer d : div) {
            long par = modMul(lcmRush(N / d, K / d), K);
            res = modSum(res, par);
        }
        System.out.println(res);
    }

    static ArrayList<Integer> factorizationGomi(int n) {
    ...
    public static long lcmRush(long n, long k) {
        ArrayList<Integer> fact = factorizationGomi((int) k);
        int fsize = fact.size();
        long sum = 0;
