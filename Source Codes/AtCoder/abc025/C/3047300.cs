...
namespace AtTest.C_Challenge
{
    class ABC_025
    {
        ...
        static void Method(string[] args)
        {
            ...
            {
                bool[,] chokudaiGrid = new bool[3, 3];
                int chokudaiCnt = 0;
                for (int j = 0; j < 9; j++)
                {
                    if (((i >> j) & 1) == 1)
                    {
                        chokudaiGrid[j / 3, j % 3] = true;
                        chokudaiCnt++;
                    }
                }
                if (chokudaiCnt != 5) continue;
                ...
                for(int j = 0; j < 2; j++)
                {
                    for(int k = 0; k < 3; k++)
                    {
                        if (chokudaiGrid[j, k] && chokudaiGrid[j + 1, k])
                        {
                            chokudai += bs[j, k];
                        }
                        else
                ...
                for (int j = 0; j < 3; j++)
                {
                    for (int k = 0; k < 2; k++)
                    {
                        if (chokudaiGrid[j, k] && chokudaiGrid[j, k+1])
                        {
                            chokudai += cs[j, k];
                        }
                        else
                ...
                Console.WriteLine(chokudai.ToString() + " " + naoko.ToString());
            }*/
        ...
        static int[] DFS(int turnCnt, GridState[,] gridInfo)
        {
            /*if (turnCnt == 9)
            {
                for (int i = 0; i < 9; i++)
                {
                    if (gridInfo[i / 3, i % 3] == GridState.undefined)
                    {
                        gridInfo[i / 3, i % 3] = GridState.chokudai;
                        break;
                    }
                }
                int[] score = GetScore(gridInfo);
                return score;
            }*/
