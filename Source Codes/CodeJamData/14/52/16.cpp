...
int mem[110][110 * 210][210];

int search(int k, int p, int d) {
  if (k == N) return 0;
  if (H[k] - d <= 0) return search(k + 1, p, 0);
  if (mem[k][p][d] != -1) return mem[k][p][d];
  int r = 0;

  // Free attack
  if (p > 0) {
    r = max(r, search(k, p - 1, d + P) + (H[k] - d - P <= 0 ? G[k] : 0));
  }

  // Normal Attack
  {
    if (H[k] - d - P <= 0) {
      r = max(r, search(k + 1, p, Q) + G[k]);
    } else {
      r = max(r, search(k, p, d + P + Q));
    }
  }

  // Pass
  {
    r = max(r, search(k, p + 1, d + Q));
  }
