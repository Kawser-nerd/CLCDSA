...
int main() {
    ...
    rep(i,N){
        rep(j,N){
            rep(k,N){
                rep(l,N){
                    int left = X[i], right = X[j], upper = Y[k], lower = Y[l];
                    if( left > right || upper < lower ) continue;
                    
                    int cnt=0;
                    rep(t,N){
                        if ( X[t] >= left && X[t] <= right && Y[t] >= lower && Y[t] <= upper ) cnt++;
                    }
                    
                    if( cnt>=K && (ll)(upper-lower)*(right-left) < minv ) minv = (ll)(upper-lower)*(right-left);
                }
            }
        }
    }
