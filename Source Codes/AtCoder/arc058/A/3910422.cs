...
namespace AtCoder {
    ...
    static class Program {
        static void Main() {
            ...
            //            n.Iterate(Plus(1)).First(x => x.ToDigits().Intersect(d).isEmpty()).WriteLine();

            while (true) {
                // WriteLine(n);
                var digits = n.ToDigits().ToList();
                if (digits[0].In(d)) {
                    n = (digits[0] + 1) * (int) 10. Pow((long) digits.Count - 1);
                    continue;
                }

                //  WriteLine($"->{n}");

                if (n.ToDigits().Intersect(d).isEmpty()) {
                    n.WriteLine();
                    return;
                }
                n++;
