...
namespace AtCoder {
    class MainClass {
        ...
        void Solve() {
            ...
            var al = new AssociativeArray<int>(l);
            var ar = new AssociativeArray<int>(r);

            foreach(int size in al.dic.Keys){
                if(ar.isContainsK(size)){
                    pair += Math.Min(al.dic[size], ar.dic[size]);
                }
            }
    ...
    class IO {
        ...
        public void ol<T>(params T[] a) { foreach (T v in a) o(a); br(); }
    ...
    static class ArrayEX {
        ...
        public static T E<T>(this T[] a, int i) { return i < a.Length ? a[i] : a[i % a.Length]; }
    }
}