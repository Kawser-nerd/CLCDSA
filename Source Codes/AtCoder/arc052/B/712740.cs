...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		int N, Q;
		Tuple<int, int, int>[] cones;
		Tuple<int, int>[] qs;
		void Read()
		{
			sc.Read(out N, out Q);
			cones = sc.ReadManyLines<int, int, int>(N);
		...
		void SolveOne()
		{
			foreach (var q in qs)
			{
				int A = q.Item1, B = q.Item2;
				var sum = 0.0;
				foreach (var cone in cones)
				{
					double x = cone.Item1, r = cone.Item2, h = cone.Item3;
					var V = r * r * h;
					if (B <= x || x + h <= A) continue;
					else if (A <= x && x + h <= B) sum += V;
					else if (A <= x) { var t = (x + h - B) / h; sum += V * (1 - t * t * t); }
					else if (x + h <= B) { var t = (x + h - A) / h; sum += V * t * t * t; }
					else { var t = (x + h - A) / h; var s = (x + h - B) / h; sum += V * (t * t * t - s * s * s); }
				}
				pr.WriteLine(sum * Math.PI / 3.0);
			}
		}
	}
}
