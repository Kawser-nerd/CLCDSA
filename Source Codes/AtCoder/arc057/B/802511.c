...
./Main.c:7:14: warning: implicit declaration of function ‘puts’ [-Wimplicit-function-declaration]
  if(S[N]==K){puts("1");return 0;}
              ^
./Main.c:10:29: warning: implicit declaration of function ‘printf’ [-Wimplicit-function-declaration]
  for(i=N;~i;i--)if(D[i]<=K){printf("%d\n",i);return 0;}
                             ^
./Main.c:10:29: warning: incompatible implicit declaration of built-in function ‘printf’
./Main.c:10:29: note: include ‘<stdio.h>’ or provide a declaration of ‘printf’