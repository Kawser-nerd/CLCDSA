...
public class Set<T>
{
    ...
    Stack<Node> stack = new Stack<Node>(30);
    Node insert2()
    {
        if (root.Count == 0) return u;
        Node t = root;
        var flag = 0L;
        while (t.Count > 0)
        {
            stack.Push(t);
            var cmp = comparer.Compare(t.Key, u.Key);
            flag <<= 1;
            if (cmp > 0) t = t.Left;
            else if (cmp == 0)
            {
                if (IsMultiSet) t = t.Left;
                else { stack.Clear(); return root; }
            }
            else { flag++; t = t.Right; }
        }
        while (stack.Count > 0)
