...
namespace Program
{
    public class Solver
    {
        public void Solve()
        {
            var n = ri;
            var m = ri;
            var s = Enumerate(n, x => rs);
            var G = Enumerate(n + m + 2 * n * m + 2, x => new List<Edge>());
            var src = n + m + 2 * n * m;
            var sink = src + 1;
            for (int i = 0; i < n; i++)
                for (int j = 0; j < m; j++)
                {
                    if (s[i][j] == '.') continue;
                    if (s[i][j] == 'S') { G.AddDirectedEdge(src, i * m + j, 1000000000); G.AddDirectedEdge(i * m + j, n * m + i * m + j, 1000000000); }
                    else if (s[i][j] == 'T') { G.AddDirectedEdge(n * m + i * m + j, sink, 1000000000); G.AddDirectedEdge(i * m + j, n * m + i * m + j, 1000000000); }
                    else { G.AddDirectedEdge(i * m + j, n * m + i * m + j, 1); }
                    G.AddDirectedEdge(n * m + i * m + j, 2 * n * m + i, 1000000000);
                    G.AddDirectedEdge(2 * n * m + i, i * m + j, 1000000000);
                    G.AddDirectedEdge(n * m + i * m + j, 2 * n * m + n + j, 1000000000);
                    G.AddDirectedEdge(2 * n * m + n + j, i * m + j, 1000000000);
                }
            var ans = Flow.GetMaxFlow(G, src, sink);
            if (ans > 2 * n * m) IO.Printer.Out.WriteLine(-1);
