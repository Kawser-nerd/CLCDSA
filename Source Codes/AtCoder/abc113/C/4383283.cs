...
namespace Contest
{
    ...
    class Program
    {
        ...
        public void Solve()
        {
            ...
            for (int i = 0; i < M; i++)
            {
                City[i] = new S() { P = sc.NextInt(), Y = sc.NextInt(), index = i };
            }
            var cnt = new int[N + 1];

            Array.Sort(City, (a, b) => a.Y.CompareTo(b.Y));

            for (int i = 0; i < M; i++)
            {
                cnt[City[i].P]++;

                City[i].ans = $"{City[i].P:D6}{cnt[City[i].P]:D6}";
            }

            Array.Sort(City, (a, b) => a.index.CompareTo(b.index));
