...
int main(void) {
    ...
    s.front() = numeric_limits<ll>::min() / 10, s.back() = numeric_limits<ll>::max() / 10;
    t.front() = numeric_limits<ll>::min() / 10, t.back() = numeric_limits<ll>::max() / 10;
    rep(i, A) cin >> s[i + 1];
    rep(i, B) cin >> t[i + 1];
    ...
    rep(i, Q){
        ...
        auto a = upper_bound(all(s), x);
        auto b = upper_bound(all(t), x);
        ll rs = *a - x;
        ll rt = *b - x;
        ll ls = x - *(a - 1);
        ll lt = x - *(b - 1);
        cout << min({
            max(ls, lt),
            max(rs, rt),
            2 * min(rt, ls) + max(rt, ls),
            2 * min(rs, lt) + max(rs, lt)
        }) << endl;
    }
