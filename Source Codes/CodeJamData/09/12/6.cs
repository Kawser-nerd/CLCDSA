...
namespace GoogleCodeJam
{
    public class Crossing
    {
        ...
        {
            StreamReader inFile = new StreamReader(iPath);
            StreamWriter outFile = new StreamWriter(oPath);

            int cases = int.Parse(inFile.ReadLine());
            char[] split = { ' ' };
            for (int i = 0; i < cases; i++)
            {
                string[] line = inFile.ReadLine().Split(split);
                rows = int.Parse(line[0]);
                columns = int.Parse(line[1]);
                lights = new TrafficLight[rows * columns];
                for (int row = 0; row < rows; row++)
                {
                    line = inFile.ReadLine().Split(split);
                ...
                CodeJam.WriteCase(outFile, i + 1, Solve().ToString());
            }
