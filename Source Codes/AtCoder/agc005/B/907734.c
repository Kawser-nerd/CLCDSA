...
void calc(int value, int index) {
    ...
    for (;;) {
        if (tmp->lower <= index && index <= tmp->upper) {
            ...
            len2 = (long long)(tmp->upper - index + 1);
            len3 = (long long)(index - tmp->lower + 1);
            //sum += ((len1 * (len1 + 1LL) - (len2 * (len2 + 1LL)) - (len3 * (len3 + 1))) ) * (long long)value;
            sum += len2 * len3 * (long long)value;
            if (tmp->lower == index) {
                tmp->lower++;
