...
namespace CodeJam
{
    ...
    public class C : SolverBase
    {
        ...
        private string Solve(int n, string nbr)
        {
            ...
            bool[] twos = new bool[n];
            int[] cirLen = new int[n];
            for (int i = 0; i < n; ++i)
            {
                cirLen[i] = Check(ref bff, i, ref maxLen, ref twos);
                res = Math.Max(res, cirLen[i]);
            }
            ...
            if (best != res)
            {
                throw new Exception("fail!" + " " + best + " " + res);
            }
            */
        ...
        private int Check(ref int[] bff, int start, ref int[] maxLen, ref bool[] twos)
        {
            bool[] used = new bool[maxLen.Length];
            int len = 1;
            used[start] = true;
            int curr = start;
            while (!used[bff[curr]])
            ...
            maxLen[bff[curr]] = Math.Max(maxLen[bff[curr]], len - 2);
            return 0;
        }
