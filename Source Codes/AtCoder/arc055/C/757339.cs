...
public class Program
{
    ...
    class RollingHash
    {
        ...
        readonly ulong Mod0 = (ulong)1e9 + 7, Mod1 = (ulong)1e9 + 9;
        ...
        public RollingHash(string A, int seed = -1)
        {
            ...
            for (int i = 0; i < 2; i++)
                Init((ulong)random.Next(10000, 20000), i == 0 ? Mod0 : Mod1, ref A, ref Hash[i], ref Pow[i]);
        }
        ...
        {
            return ((Mod0 + Hash[0][R] - Hash[0][L]) * Pow[0][N - L] % Mod0 << 32) +
                    (Mod1 + Hash[1][R] - Hash[1][L]) * Pow[1][N - L] % Mod1;
        }
    }
