        ...
        class Solver {
            public List<string> SolveAll(int threadId, StreamReader sr) {
                ...
                for (int testCaseIx = 0; testCaseIx < TestCaseCount; testCaseIx++) {
                    ...
                    int N = cs[0];
                    int P = cs[1];
                    List<int> gs = SplitToInts(sr.ReadLine());
                    // ------------ 読み込み ------------
                    #region
                    ...
                    #endregion
                    // ------------ 呼び出し ------------
                    string output = Solve(testCaseIx, N, P, gs);
                    // ------------ 呼び出し ------------
                    outputs.Add(output);
                }
                return outputs;
            }
            string Solve(int testCaseIx, int N, int P, List<int> gs) {
                ...
                if (P == 3) {
                    int min = Math.Min(counts[1], counts[2]);
                    counts[1] -= min;
                    counts[2] -= min;
                    ans += min;
                    int c1 = counts[1] / 3;
