...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		{
			sc.Read(out N, out Q);
			q = sc.ReadManyLines<long>(Q);
		}
		void SolveOne()
		{
			if (Q == 0) { for (var i = 0; i < N; i++) pr.WriteLine(1); return; }
			var st = new Stack<long>();
			st.Push(q[Q - 1]);
			for (var i = Q - 2; i >= 0; i--) if (q[i] < st.Peek()) st.Push(q[i]);
			var qs = new List<long>(st);
			var ans = new long[N];
			{
				if (N == qs[0]) qs.RemoveAt(0);
				else if (N > qs[0]) N = (int)qs[0];
				Q = qs.Count;
				if (Q == 0) { for (var i = 0; i < N; i++) pr.WriteLine(1); return; }
				var V = N + Q;
				var es = new List<Tuple<int, long>>[V];
				for (var i = 0; i < V; i++) es[i] = new List<Tuple<int, long>>();
				var rd = new long[V];
				var d = new Dictionary<long, int>();
				for (var i = 0; i < N; i++) { rd[i] = i + 1; d[i + 1] = i; }
				for (var i = N; i < V; i++) { rd[i] = qs[i - N]; d[qs[i - N]] = i; }
				foreach (var q in qs)
				{
					var x = q;
					var id = d[x];
					while (x > 0)
					{
						id = Func.FirstBinary(0, id, _ => rd[_] > x) - 1;
						es[id].Add(new Tuple<int, long>(d[q], x / rd[id]));
						x = x % rd[id];
					}
				}
				var cnt = new long[V];
				cnt[V - 1] = 1;
				for (var i = V - 2; i >= 0; i--) foreach (var e in es[i]) cnt[i] += e.Item2 * cnt[e.Item1];
				var sum = new long[N + 1];
				for (var i = 0; i < N; i++) sum[i + 1] = sum[i] + cnt[i];
				for (var i = 0; i < N; i++) ans[i] = sum[N] - sum[i];
			}
			pr.WriteLine(ans);
		}
	}
