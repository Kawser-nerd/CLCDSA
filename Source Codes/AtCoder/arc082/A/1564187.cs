...
namespace Program {
    class MainClass {
        ...
        void Solve() {
            ...
            int ans = 0;
            N.REP(i => {
                if (Math.Abs(most - A[i]) <= 1)
                    ans++;
            });

            io.o(ans);
            //TLE?????
            var aa = new AssociativeArray<int>();
            aa.Add(A);
            N.REP(i => { aa.Add(A[i] - 1); });
            N.REP(i => { aa.Add(A[i] + 1); });
            var most1 = aa.ValueSortedKey().Last();
        }
        ////////////////////////////////////////////////////////////
    ...
    class AssociativeArray<T> : IEnumerable {//hack:KeyValuePair??????????
        ...
        public T[] ValueSortedKey() {//test
            return dic.OrderBy(kp => kp.Value).ThenBy(kp => kp.Key).ToDictionary(kp => kp.Key, kp => kp.Value).Keys.ToArray();
        }
        //:sort->array
    }
    class Index {
    ...
    class Kruskal : ShortestPath {//TODO:test
        public long Run() {
            ...
            foreach (var t in li) { if (!uf.Same(t.Item1, t.Item2)) { ret += t.Item3; uf.Unite(t.Item1, t.Item2); } }
            return ret;
        }
    }
    #endregion
    ...
    static class IEnumerableEx {
        public static void ForEach<T>(this IEnumerable<T> s, Action<T> act) { foreach (var v in s) act(v); }
        public static void ForEach<T>(this IEnumerable<T> s, Action<T, int> act) {
            foreach (var v in s.Select((x, i) => new { V = x, I = i })) act(v.V, v.I);
        }
    }
    #endregion
}