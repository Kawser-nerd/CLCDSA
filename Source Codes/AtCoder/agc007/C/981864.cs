...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			sc.Read(out N, out d, out x);
		}
		void SolveOne()
		{
			pr.WriteLine((N + 1) * (d + (N - 0.5) * x) * (Harmonic(N + 1) - 1));
		}
		// 1+1/2+...+1/n
		double Harmonic(int n)
		{
			...
			{
				var s = 0.0;
				for (var i = 1; i <= n; i++) s += 1.0 / i;
				return s;
			}
			else
			{
				var n2 = (double)n * n;
				return 0.57721566490153286061 + Math.Log(n) + 1 / (2.0 * n) - (1 + (-0.1 + (1 / 21.0 + (-0.05 + 1 / (11 * n2)) / n2) / n2) / n2) / (12 * n2);
			}
		}
	}
}
