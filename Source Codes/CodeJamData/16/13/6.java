...
public class C implements Runnable{
	...
	void solve() {
		int n = readInt();
		int[] friends = readIntArrayWithDecrease(n);
		
		boolean[] isWeddingBase = new boolean[n];
		for (int i = 0; i < n; ++i) {
			isWeddingBase[i] = (i == friends[friends[i]]);
		}
		
		int[] guestsCount = new int[n];
		...
		for (int i = 0; i < n; ++i) {
			...
			for (; steps[cur] == -1 && !isWeddingBase[cur]; cur = friends[cur], ++size) {
				steps[cur] = size;
			}
			
			if (isWeddingBase[cur]) {
				guestsCount[cur] = max(guestsCount[cur], size);
			} else {
				int circleSize = size - steps[cur];
				maxCircleSize = max(maxCircleSize, circleSize);
			}
		}
		
		int weddingSizesSum = 0;
		for (int i = 0; i < n; ++i) {
			if (isWeddingBase[i] && i < friends[i]) {
				int weddingSize = (guestsCount[i] + 1) + (guestsCount[friends[i]] + 1);
				weddingSizesSum += weddingSize;
			}
		}
		
		int answer = max(maxCircleSize, weddingSizesSum);
		out.println(answer);
	}
