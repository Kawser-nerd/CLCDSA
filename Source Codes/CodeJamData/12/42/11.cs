...
partial class Solution
{
    ...
    void Solve()
    {
        ...
        if (L < W)
        {
            int t = L; L = W; W = t; flip = true;
        }

        var rr = SplitLine_int();
        List<Tuple<int, int>> rrr = new List<Tuple<int, int>>();
        for (int i = 0; i < N; i++)
        {
            rrr.Add(Tuple.Create(rr[i], i));
        }
        rrr.Sort();
        rrr.Reverse();
        ...
        int y = 0;
        int nexty = 0;
        foreach (Tuple<int,int> ri in rrr)
        {
            int r = ri.Item1;
            int index = ri.Item2;
        restart:
            ...
            else
            {
                ...
                result[index] = Tuple.Create(x + r, y);
                x += 2 * r;
            }
        }
        ...
        {
            Debug.Assert(foo.Item1 <= W);
            Debug.Assert(foo.Item2 <= L);
            if (flip) { Write(" {0} {1}", foo.Item2, foo.Item1); }
            else Write(" {0} {1}", foo.Item1, foo.Item2);
        }
        WriteLine();
    }
