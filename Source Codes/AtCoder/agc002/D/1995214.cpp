...
struct PersistentArray {
    ...
    const T& get(long long i) const {
        auto node = root;
        for (int d = depth; d--; i >>= B)
            node = node->children[i & (1 << B) - 1];
        return node->value;
    }
    T& set(long long i) {
