...
public class Main {
    ...
    public static void main(String[] args){
    	String s=next();
    	int[] stack=new int[8000];
    	int size=0;
    	for(int i=0;i<s.length();i++){
    		if(s.charAt(i)=='F') stack[size]++;
    		else size++;
    	}
    	size++;
    	int[] tate=new int[(size+1)/2];
    	int[] yoko=new int[size/2];
    	...
    	int haba=16010;
    	int[] xs=new int[haba],ys=new int[haba];
    	int gx=haba/2,gy=haba/2;
    	xs[tate[0]+gx]=1;
    	for(int i=1;i<tate.length;i++){
    		...
    		for(int j=0;j<xs.length;j++){
    			...
    			if((j+d)<xs.length && xs[j+d]==1){
    				nxs[j]=1;
    			}
    		}
    		xs=nxs;
    		//pl(Arrays.toString(xs));
    	}
    	ys[gy]=1;
