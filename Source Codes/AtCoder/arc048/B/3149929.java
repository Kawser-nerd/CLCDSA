...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        String[][] res = new String[(int) 1e6 + 1][4];
        for (int i = 0; i < N; i++) {
            int x = p[i].x;
            int y = p[i].y;
            int win = lowerBound(pa, x) + cou[x][(y + 1) % 3];
            int hiki = cou[x][y] - 1;
            int lose = N - 1 - win - hiki;
            res[x][y] = win + " " + lose + " " + hiki;
        }
        for (int i = 0; i < N; i++) {
            System.out.println(res[ori[i].x][ori[i].y]);
        }
    }
