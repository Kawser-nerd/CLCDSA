...
public class Main {
    ...
    public static void main(String[] args){
        ...
        while(Long.highestOneBit(a)==Long.highestOneBit(b)){
            a-=Long.highestOneBit(a);
            b-=Long.highestOneBit(b);
        }
        if(b==0){
            long res=2*Long.highestOneBit(a);
            pl(res);
            flush();
            return;
        }
        long angel=Long.highestOneBit(a);
        long res=(angel-1-b+1);//0------...
        long kasu=a-angel;
        if(kasu==0){
            res+=(angel-1-b+1);
            res+=1;//angel
        ...
        //0 to kasu or b to angel-1
        long jou=2*Long.highestOneBit(kasu)-1;
        jou=Math.min(jou,b-1);
        res+=(angel-1-b+1);
        res+=jou+1;
        pl(res);
        flush();
