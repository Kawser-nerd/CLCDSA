...
class Solver {
    ...
    public void solve (MyInput in, PrintWriter out, int testCase) {
        int n = in.nextInt();
        Pair <Double, Integer> all[] = new Pair[20 * n];
        for (int i = 0; i < n; i++) {
            all[i] = new Pair<>(Double.MAX_VALUE, -1);
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            Pair <Double, Integer> temp[] = new Pair[20 * n];
            for (int j = 0; j < h; j++) {
                double t = (double)(360 - d) / 360 * (m + j);
                for (int k = 0; k < 20 * n; k++) {
                    temp[k] = new Pair<>(t, cnt);
                    t += m + j;
                }
                cnt++;
        ...
        for (int i = 0; i < 20 * n; i++) {
            if (!reach[all[i].getY()]) {
                cur--;
                reach[all[i].getY()] = true;
            }
            else {
                cur++;
            }
            if (i + 1 < 4 * n && Math.abs(all[i].getX() - all[i + 1].getX()) < EPS) {
                continue;
            }
    ...
    private Pair <Double, Integer>[] merge(Pair <Double, Integer> a[], Pair <Double, Integer> b[]) {
        int n = a.length;
        Pair <Double, Integer> ret[] = new Pair[n];

        int i = 0;
        int j = 0;
        while (i + j < n) {
            if (a[i].getX() < b[j].getX()) {
                ret[i + j] = a[i];
                i++;
            }
            else {
