...
namespace Contest
{
    ...
    class Program
    {
        ...
        private long NextSnuke(long n)
        {
            ...
            {
                d *= 10;
                long nn = d * (long)Math.Floor((double)n / d + 1) - 1;
                double ss = Calc(nn);
                if (ss < min)
                {
                    ans = nn;
        ...
        private int Compare(long a, long sa, long b, long sb)
        {
            return (a * sb).CompareTo(b * sa);
        }
