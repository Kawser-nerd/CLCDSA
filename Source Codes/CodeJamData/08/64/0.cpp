...
bool find(int v,int nright)
{
	for (int i=0;i<nright;i++) if (!visited[i] && G[v][i])
	{
		visited[i]=true;
		if (p2[i]<0 || find(p2[i],nright))
		{
			p1[v]=i;
			p2[i]=v;
			return true;
...
int max_matching(int nleft,int nright)
{
	...
	for (int i=0;i<nleft;i++) for (int j=0;j<nright;j++) if (G[i][j] && p1[i]<0 && p2[j]<0)
	{
		p1[i]=j;
		p2[j]=i;
		result++;
	...
	for (int i=0;i<nleft;i++) if (p1[i]<0)
	{
		...
		if (find(i,nright)) result++;
	}
	return result;
}
int check(int v1,int f1,int v2,int f2)
{
	vector<int> state;
	state.push_back(v1);
	state.push_back(f1);
	...
	vector<int> A1,A2;
	for (int i=0;i<SIZE(G1[v1]);i++) if (G1[v1][i]!=f1) A1.push_back(G1[v1][i]);
	for (int i=0;i<SIZE(G2[v2]);i++) if (G2[v2][i]!=f2) A2.push_back(G2[v2][i]);
	int nleft=SIZE(A1);
	int nright=SIZE(A2);
	for (int i=0;i<nleft;i++) for (int j=0;j<nright;j++) 
		check(A1[i],v1,A2[j],v2);
	for (int i=0;i<nleft;i++) for (int j=0;j<nright;j++) 
		G[i][j]=check(A1[i],v1,A2[j],v2);
	int tmp=max_matching(nleft,nright);
	int ret=-1;
	if (tmp==nright) ret=1;
	else ret=0;
	return M[state]=ret;
