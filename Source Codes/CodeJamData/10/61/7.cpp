...
void solveCase() {
  ...
  while(S[Size(S)-1] < 'A') S.resize(Size(S)-1);
  ...
  for(int ini=0; ini<=N-len; ini++) {
    int last = ini+len;
    
    tab[ini][len] = 30000;

    if(len == 1) tab[ini][len] = 3;
    if(len > 1 && S[ini+len-1] == S[ini]) {
      // printf("! %d %d\n", ini, len);
      tab[ini][len] <?= tab[ini][len-1] + 1;
      }

    for(int cut=1; cut < len; cut++)
      tab[ini][len] <?= tab[ini][cut] + tab[ini+cut][len-cut];
