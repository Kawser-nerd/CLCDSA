...
namespace AtCoder
{
    public class Program
    {
        ...
        private void Solve()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                var b_minus = (i & 4) > 0 ? -1 : 1;
                var c_minus = (i & 2) > 0 ? -1 : 1;
                var d_minus = (i & 1) > 0 ? -1 : 1;

                if (a + b * b_minus + c * c_minus + d * d_minus == 7)
                {
                    Console.WriteLine($"{a}{ch(b_minus)}{b}{ch(c_minus)}{c}{ch(d_minus)}{d}=7");
                    return;
                }
