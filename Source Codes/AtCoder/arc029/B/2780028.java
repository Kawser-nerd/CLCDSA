...
public class Main {
	private static void solve() {
		...
		for (int i = 0; i < n; i++) {
			...
			// solve acos+bsin=c
			double min = Math.abs(Math.atan2(b, a));
//			System.out.println(min/Math.PI*180);
			double max = Math.PI / 2;
			for (int j = 0; j < 60; j++) {
				double mid = (min + max) * 0.5;
				if (Math.cos(mid) * a + Math.sin(mid) * b > c) {
					min = mid;
				} else {
					max = mid;
				}
			}
			double one = Math.cos(min) * a + Math.sin(min) * b;
			double another = Math.sin(min) * a + Math.cos(min) * b;
//			System.out.println(c + " " + d + " : " + one + " " + another);
			if (another > d + 1e-3 || one > c + 1e-3) {
				out("NO");
			} else {
				out("YES");
			}
