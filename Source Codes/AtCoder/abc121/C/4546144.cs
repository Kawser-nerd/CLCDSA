...
static class MainClass
{
    public static void Main(string[] args)
    {
        ...
        var lss = new List<KeyValuePair<long, long>>();
        for (int i = 0; i < nm[0]; i++)
        {
            var ab = Console.ReadLine().SplittedStringToInt32List();
            lss.Add(new KeyValuePair<long, long>(ab[0], ab[1]));
        }
        var sorted = new Stack<KeyValuePair<long, long>>(lss.OrderByDescending(x => x.Key).ToList());
        var m = nm[1];
        var sum = 0l;
        while (true)
        {
