...
class LazySegTree {
	...
	private void eval(int k, int len) {
		dat[k] = applyLazy(dat[k], lazy[k], len);
		if (k < n - 1) {
			lazy[k * 2 + 1] = mergeLazy(lazy[k * 2 + 1], lazy[k]);
			lazy[k * 2 + 2] = mergeLazy(lazy[k * 2 + 2], lazy[k]);
		}
		lazy[k] = ID_LAZY;
	}
	...
	private final long ID_LAZY = Long.MAX_VALUE;

	private final long merge(long x, long y) {
		return x < y ? x : y;
	}

	private final long mergeLazy(long x, long y) {
		return x < y ? x : y;
	}

	private final long applyLazy(long x, long lazy, int length) {
		return x < lazy ? x : lazy;
	}
...
public class Main {
	private static void solve() {
		...
		int L = nei();
		int[][] lrc = nis2(n, 3);
		Arrays.sort(lrc, (a, b) -> a[0] - b[0]);
		LazySegTree dp = new LazySegTree(L);
		for (int i = 0; i < n; i++) {
			int l = lrc[i][0];
			int r = lrc[i][1];
			int c = lrc[i][2];
			long a = l == 0 ? 0 : dp.query(l - 1, l);
			dp.op(a + c, l, r);
//			dp.dump();
		}
		out(dp.query(L - 1, L));
