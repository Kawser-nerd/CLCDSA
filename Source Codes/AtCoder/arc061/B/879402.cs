...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		public void Solve()
		{
			if (IsGCJ) for (var i = 0; i < Repeat; i++)
				{
					Read();
					pr.Write("Case #" + (i + 1) + ": ");
					SolveOne();
		...
		void Read()
		{
			sc.Read(out Y, out X, out N);
			ps = sc.ReadManyLines<int,int>(N);
		}
		SortedDictionary<Tuple<int,int>,long> bs;
		void Add(int x, int y)
		{
			var k = new Tuple<int,int>(x, y);
			if (bs.ContainsKey(k)) bs[k]++;
			else bs.Add(k, 1);
		}
		void SolveOne()
		{
			bs = new SortedDictionary<Tuple<int,int>,long>(new Comp());
			foreach (var p in ps)
			{
				int x = p.Item2-1, y = p.Item1-1;
				for (var dx = -1; dx <= 1; dx++)
					for (var dy = -1; dy <= 1; dy++)
					{
						int a = x + dx, b = y + dy;
						if (a >= 0 && a < X && b >= 0 && b < Y) Add(a, b);
					}
			}
			var ans = new long[10];
			foreach (var t in bs)
			{
				int x = t.Key.Item1, y = t.Key.Item2;
				if (x >= 1 && x < X - 1 && y >= 1 && y < Y - 1) ans[t.Value]++;
			}
			ans[0] = (X - 2L) * (Y - 2L) - ans.Sum();
			for (var i = 0; i < 10; i++) pr.WriteLine(ans[i]);
		}
	}
}
class Comp : IComparer<Tuple<int,int>>
{
	public int Compare(Tuple<int, int> x, Tuple<int, int> y)
	{
		var c = x.Item1.CompareTo(y.Item1);
		return c == 0 ? x.Item2.CompareTo(y.Item2) : c;
	}
}
...
class SkewHeap<T> : IEnumerable<T>
{
	...
	public SkewHeap(Comparison<T> c)
	{
		comp = c;
		Count = 0;
	}
	...
	private SkewHeap(Comparison<T> c, Node h) : this(c)
	{
		head = h;
	}
	...
	{
		var x = head.val;
		head = Meld(head.l, head.r);
		Count--;
		return x;
	}
	// a.comp must be equivalent to b.comp
	// a, b will be destroyed
	public static SkewHeap<T> Meld(SkewHeap<T> a, SkewHeap<T> b)
	{
		return new SkewHeap<T>(a.comp, a.Meld(a.head, b.head));
	}
	public void MeldWith(SkewHeap<T> a)
	{
		head = Meld(head, a.head);
	}
...
class BITSet : BinaryIndexedTree
{
	...
	{
		return Sum(item, item + 1) > 0;
	}
	public int Count(int item)
	{
		return Sum(item, item + 1);
	}
	// ?? = item ???????????(0-indexed)
	...
	{
		return Func.FirstBinary(0, Size, t => Sum(0, t + 1) >= rank + 1);
	}
}
class RangeBIT
{
	...
	void Add2(int which, int i, long value)
	{
		while (i < N)
		{
			bit[which, i] += value;
	...
	long Sum(int to)
	{
		return Sum2(0, to) + Sum2(1, to) * to;
	}
	public long Sum(int from, int to)
	{
		return Sum(to) - Sum(from);
	}
	long Sum2(int which, int i)
	{
		var sum = 0L;
		while (i > 0)
		{
...
class BinaryIndexedTree2D
{
	...
	{
		for (var i = x + 1; i <= X; i += i & (-i)) for (var j = y + 1; j <= Y; j += j & (-j)) bit[i, j] += value;
	}
	...
	{
		var sum = 0;
		for (var i = x; i > 0; i -= i & (-i)) for (var j = y; j > 0; j -= j & (-j)) sum += bit[i, j];
		return sum;
	}
}
class BinaryIndexedTree
{
	...
	{
		for (var i = index + 1; i <= Size; i += i & (-i)) bit[i] += value;
	}
	...
	{
		var sum = 0;
		for (var i = to; i > 0; i -= i & (-i)) sum += bit[i];
		return sum;
	}
}
class Amoeba
{
	...
	// reflection
	public const double Beta = 1 + 2.0 / Dimension;
	// expansion
	public const double Gamma = 0.75 - 0.5 / Dimension;
	// contraction
	public const double Delta = 1 - 1.0 / Dimension;
	// shrink
	...
	void PartialSort(int i, int j)
	{
		if (a[i].Second > a[j].Second) a.Swap(i, j);
	}
	...
	public void Search()
	{
		...
		var fr = r.Func();
		if (a[0].Second <= fr && fr < a[Dimension - 1].Second)
		{
			Accept(r, fr);
			return;
		}
	...
	void Shrink()
	{
		...
		{
			a[i].First.Multiply(Delta);
			a[i].First.Add(tmp);
			a[i].Second = a[i].First.Func();
		}
		Initiate();
	}
}
class AmoebaState
{
	...
	public AmoebaState(params double[] elements) : this()
	{
		elements.CopyTo(vec, 0);
	}
	public double this[int n] { get { return vec[n]; } set { vec[n] = value; } }
	public void Multiply(double r)
	{
		for (var i = 0; i < Dimension; i++) vec[i] *= r;
	}
	public void Add(AmoebaState v)
	{
		for (var i = 0; i < Dimension; i++) vec[i] += v.vec[i];
	}
	public static AmoebaState operator +(AmoebaState p)
	{
		return new AmoebaState(p.vec);
	}
	public static AmoebaState operator -(AmoebaState p)
	{
		var tmp = new AmoebaState(p.vec);
		tmp.Multiply(-1);
		return tmp;
	}
	public static AmoebaState operator /(AmoebaState p, double r)
	{
		var tmp = new AmoebaState(p.vec);
		tmp.Multiply(1 / r);
		return tmp;
	}
	public static AmoebaState operator *(double r, AmoebaState p)
	{
		var tmp = new AmoebaState(p.vec);
		tmp.Multiply(r);
		return tmp;
	...
	public static AmoebaState operator +(AmoebaState p, AmoebaState q)
	{
		var tmp = +p;
		tmp.Add(q);
		return tmp;
...
class BucketList<T> : ICollection<T>, IEnumerable<T>, ICollection, IEnumerable
{
	...
	public BucketList(Comparison<T> comp = null)
	{
		Head = null;
		Tail = null;
		NumOfBucket = 0;
	...
	public Tuple<BucketNode<T>, int> UpperBound(Predicate<T> pred)
	{
		...
		while (node.Prev != null && !pred(node.Prev.Value))
		{
			node = node.Prev;
			index--;
		}
	...
	public Tuple<BucketNode<T>, int> UpperBound(T item)
	{
		return LowerBound(x => comp(x, item) <= 0);
	}
	// (node, index)
	// index is the position of node in node.Parent
	public Tuple<BucketNode<T>, int> LowerBound(Predicate<T> pred)
	{
		...
		var index = 0;
		while (node.Next != null && !pred(node.Next.Value))
		{
			node = node.Next;
			index++;
		}
	...
	public Tuple<BucketNode<T>, int> LowerBound(T item)
	{
		return LowerBound(x => comp(x, item) >= 0);
	}
	...
	public void AddFirst(Bucket<T> bucket)
	{
		if (NumOfBucket == 0) InitiateWith(bucket);
		else AddBefore(Head, bucket);
	}
	public void AddLast(Bucket<T> bucket)
	{
		if (NumOfBucket == 0) InitiateWith(bucket);
		else AddAfter(Tail, bucket);
	}
	public void AddFirst(T item)
	{
		if (NumOfBucket == 0) InitiateWith(item);
		else AddBefore(Head.Head, item);
	}
	public void AddLast(T item)
	{
		if (NumOfBucket == 0) InitiateWith(item);
		else AddAfter(Tail.Tail, item);
	}
	public void Clear()
	{
		RemoveAll();
	...
	{
		if (NumOfBucket == 0) throw new InvalidOperationException();
		else Remove(Head.Head);
	}
	public void RemoveLast()
	{
		if (NumOfBucket == 0) throw new InvalidOperationException();
		else Remove(Tail.Tail);
	}
	...
	public void Remove(BucketNode<T> node)
	{
		Debug.Assert(node != null && node.Parent.Parent == this);
		if (!node.Parent.Remove(node)) Remove(node.Parent);
	}
	protected void RemoveRange(Bucket<T> from, Bucket<T> to, int indexFrom = -1, int indexTo = -1)
	{
		...
		if (indexFrom == 0 && indexTo == NumOfBucket - 1)
		{
			Clear();
			return;
		}
	...
	public void RemoveRange(BucketNode<T> from, BucketNode<T> to, int indexFrom = -1, int indexTo = -1)
	{
		...
		else
		{
			...
			{
				Remove(bucketFrom);
				RemoveRange(bucketTo.Head, to, 0, indexTo);
			}
			else if (indexTo == bucketTo.Count - 1)
			{
				Remove(bucketTo);
				RemoveRange(from, bucketFrom.Tail, indexFrom, bucketFrom.Count - 1);
			}
			else
			{
	...
	public BucketNode<T> Find(T item)
	{
		var node = LowerBound(item);
		if (node == null || comp(node.Item1.Value, item) != 0) return null;
		else return node.Item1;
	}
	public BucketNode<T> FindLast(T item)
	{
		var node = UpperBound(item);
		if (node == null || comp(node.Item1.Value, item) != 0) return null;
		else return node.Item1;
	}
	public IEnumerator<T> GetEnumerator()
	...
	{
		var ub = LowerBound(item);
		if (ub != null) AddBefore(ub.Item1, item);
		else AddLast(item);
	}
	IEnumerator IEnumerable.GetEnumerator()
	{
	...
	bool CheckBucket(Bucket<T> bucket)
	{
		return bucket.Count > 0 && bucket.Count <= BucketSize && bucket.Parent == this;
	}
	public void Start(Func<string, T> parser, Func<T> random)
	{
		...
		while (true)
		{
			...
			{
				if (command.Length > 1) Add(parser(command[1]));
				else Add(random());
			}
			else if (command[0].StartsWith("r"))
			{
				if (command.Length > 1) Remove(parser(command[1]));
				else Remove(random());
			}
			...
			else if (command[0].StartsWith("e"))
			{
				var tmp = x;
				x = y;
				y = tmp;
			...
			{
				RemoveRange(x, y, x.Index, y.Index);
				x = null;
				y = null;
			}
	...
	void SetVariable(string[] command, ref BucketNode<T> x, Func<string, T> parser, Func<T> random)
	{
		...
		else if (command[1].StartsWith("n"))
		{
			...
			{
				Console.WriteLine("x is the last element...");
		...
		else if (command[1].StartsWith("p"))
		{
			...
			{
				Console.WriteLine("x is the first element...");
				Console.ReadKey(true);
			}
		}
		else if (command[1].StartsWith("f"))
		{
			if (command.Length > 2) x = LowerBound(parser(command[2])).Item1;
			else x = LowerBound(random()).Item1;
		}
	}
}
...
class Bucket<T>
{
	...
	public Bucket(BucketList<T> parent, Bucket<T> prev, Bucket<T> next)
	{
		Parent = parent;
		Prev = prev;
		Next = next;
	...
	public bool AddAfter(BucketNode<T> node, BucketNode<T> item)
	{
		return AddAfter(node, item.Value);
	}
	public bool AddBefore(BucketNode<T> node, BucketNode<T> item)
	{
		return AddBefore(node, item.Value);
	}
	...
	public bool InitiateWith(T item)
	{
		return InitiateWith(new BucketNode<T>(item, this, null, null));
	}
	public void RemoveAll()
	{
		Head = Tail = null;
	...
	{
		if (Count == 0) return InitiateWith(item);
		else return AddBefore(Head, item);
	}
	public bool AddLast(T item)
	{
		if (Count == 0) return InitiateWith(item);
		else return AddAfter(Tail, item);
	}
	...
	bool CheckNode(BucketNode<T> node)
	{
...
class BucketNode<T>
{
	...
	public BucketNode(T item, Bucket<T> parent, BucketNode<T> prev, BucketNode<T> next)
	{
		Value = item;
		Parent = parent;
		Prev = prev;
...
class UndirectedGraph<V, E> : DirectedGraph<V, E>
{
	...
	public IEnumerable<NodeInfo<V>> GetArticulationPoints()
	{
		...
		dfs = (u, prev) => {
			...
			foreach (var e in edgesFrom[u])
			{
				...
				if (visited[v])
				{
					if (v != prev) lowest[u] = Math.Min(lowest[u], order[v]);
				}
				else
				{
					parent[v] = u;
		...
		for (var v = 0; v < numberOfNodes; v++)
		{
			if (isroot[v])
			{
				if (children[v].Count > 1) yield return nodes[v];
			}
			else
			{
				if (isarticulation[v]) yield return nodes[v];
			}
		}
	}
	...
	public override string ToString()
	{
		return ToString(v => v.ToString(), e => e.ToString());
	}
}
...
class EdgeInfo<E> : Pair<Pair<int, int>, E>
{
	...
	public EdgeInfo<E> Reverse()
	{
		return new EdgeInfo<E>(To, From, Information);
	}
}
class DirectedGraph<V, E> : IEnumerable<NodeInfo<V>>
{
	...
	public IEnumerable<HalfEdgeInfo<E>> EdgesFrom(int node)
	{
		return edgesFrom[node];
	}
	public int InDegree(int node)
	{
		return edgesTo[node].Count;
	}
	public int OutDegree(int node)
	{
		return edgesFrom[node].Count;
	}
	public IEnumerable<HalfEdgeInfo<E>> EdgesTo(int node)
	{
		return edgesTo[node];
	}
	...
	public DirectedGraph(int V, IEnumerable<EdgeInfo<E>> edges) : this(V)
	{
		foreach (var e in edges) AddEdge(e.From, e.To, e.Information);
	}
	...
	public void AddEdge(int from, int to, E information)
	{
		AddEdge(new EdgeInfo<E>(from, to, information));
	}
	public void AddEdge(V from, V to, E information)
	{
		AddEdge(new EdgeInfo<E>(SearchNode(from).Code, SearchNode(to).Code, information));
	}
	public NodeInfo<V> SearchNode(V node)
	{
		return nodes.FirstOrDefault(e => e.Information.Equals(node));
	}
	public EdgeInfo<E> SearchEdge(E edge)
	{
		return edges.Find(e => e.Information.Equals(edge));
	}
	public IEnumerator<NodeInfo<V>> GetEnumerator()
	{
		foreach (var v in nodes) yield return v;
	}
	IEnumerator IEnumerable.GetEnumerator()
	{
		return GetEnumerator();
...
class AVLTree<T> : IEnumerable<T>, ICollection<T>, ICollection, IEnumerable
{
	public class AVLNode : IEnumerable<T>
	{
		...
		public AVLNode(AVLTree<T> tree) : this(default(T), tree)
		{
			height = 0;
			Parent = null;
		}
		public AVLNode(T x, AVLNode parent, AVLTree<T> tree) : this(x, tree)
		{
			this.height = 1;
			this.Parent = parent;
		}
	...
	public bool Remove(T item)
	{
		return Remove(item, sentinel.Left);
	}
	...
	public AVLNode LowerBound(Predicate<T> pred)
	{
		AVLNode node;
		LowerBound(pred, sentinel.Left, out node);
		return node;
	}
	public AVLNode UpperBound(Predicate<T> pred)
	{
		AVLNode node;
		UpperBound(pred, sentinel.Left, out node);
		return node;
	}
	public AVLNode LowerBound(T item)
	{
		return LowerBound(x => comp(x, item) >= 0);
	}
	public AVLNode UpperBound(T item)
	{
		return UpperBound(x => comp(x, item) <= 0);
	}
	bool UpperBound(Predicate<T> pred, AVLNode node, out AVLNode res)
	{
		...
		if (pred(node.Item))
		{
			if (!UpperBound(pred, node.Right, out res)) res = node;
			return true;
		}
	...
	bool LowerBound(Predicate<T> pred, AVLNode node, out AVLNode res)
	{
		...
		if (pred(node.Item))
		{
			if (!LowerBound(pred, node.Left, out res)) res = node;
			return true;
		}
	...
	public T Min()
	{
		return Min(sentinel.Left).Item;
	}
	public AVLNode MinNode()
	{
		return Min(sentinel.Left);
	}
	public T Max()
	{
		return Max(sentinel.Left).Item;
	}
	public AVLNode MaxNode()
	{
		return Max(sentinel.Left);
	}
	...
	public IEnumerator<T> GetEnumerator()
	{
		return sentinel.Left.GetEnumerator();
	}
	IEnumerator IEnumerable.GetEnumerator()
	{
		return GetEnumerator();
	...
	{
		foreach (var x in this) array[arrayIndex++] = x;
	}
	public int Count { get { return count; } }
	public bool IsReadOnly { get { return true; } }
	public void CopyTo(Array array, int index)
	{
		foreach (var x in this) array.SetValue(x, index++);
	}
	public bool IsSynchronized { get { return false; } }
	public object SyncRoot { get { return this; } }
	public override string ToString()
	...
	{
		return IsBalanced(sentinel.Left);
	}
	public bool IsValidBinarySearchTree()
	{
		return IsValidBinarySearchTree(sentinel.Left);
	}
	bool IsBalanced(AVLNode node)
	{
		return node == sentinel || (Math.Abs(node.Bias) < 2 && IsBalanced(node.Left) && IsBalanced(node.Right));
	}
	...
	bool Small(T item, AVLNode node)
	{
		return node == sentinel || (comp(item, node.Item) >= 0 && Small(item, node.Left) && Small(item, node.Right));
	}
	bool Large(T item, AVLNode node)
	{
		return node == sentinel || (comp(item, node.Item) <= 0 && Large(item, node.Left) && Large(item, node.Right));
	}
...
class PriorityQueue<T> : IEnumerable<T>, ICollection, IEnumerable, ICloneable
{
	...
	public PriorityQueue(IComparer<T> comp, int capacity = 0, IEnumerable<T> source = null) : this(comp.ToComparison(), source)
	{
	...
	public PriorityQueue(Comparison<T> comp, int capacity = 0, IEnumerable<T> source = null)
	{
		this.comp = comp == null ? Func.DefaultComparison<T>() : comp;
	...
	public IEnumerator<T> GetEnumerator()
	{
		var x = (PriorityQueue<T>)Clone();
		while (x.Count > 0) yield return x.Dequeue();
	}
	...
	public object Clone()
	{
		var x = new PriorityQueue<T>(comp, Count);
		x.list.AddRange(list);
		return x;
	}
	public void Clear()
	{
...
class Deque<T>
{
	...
	{
		array = new T[this.capacity = capacity];
		Count = 0;
		offset = 0;
	}
	...
	int GetIndex(int index)
	{
		...
		return tmp >= capacity ? tmp - capacity : tmp;
	}
	public T PeekFront()
	{
		return array[offset];
...
class Point : Pair<int, int>
{
	...
	public static Point operator /(Point p, int r)
	{
		return new Point(p.X / r, p.Y / r);
	}
	public static Point operator *(int r, Point p)
	{
		return new Point(p.X * r, p.Y * r);
	}
	public static Point operator *(Point p, int r)
...
class Printer : IDisposable
{
	...
	{
		file.WriteLine(b ? "YES" : "NO");
	}
	public void WriteLine<T>(IEnumerable<T> list)
	{
		foreach (var x in list) file.WriteLine(x);
	}
...
class Scanner : IDisposable
{
	...
	public T Get<T>()
	{
		return (T)Convert(file.ReadLine(), Type.GetTypeCode(typeof(T)));
	}
	...
	public Tuple<S, T, U> Get<S, T, U>()
	{
		S s;
		T t;
		U u;
		Read(out s, out t, out u);
		return new Tuple<S, T, U>(s, t, u);
	}
	public Tuple<S, T, U, V> Get<S, T, U, V>()
	{
		S s;
		T t;
		U u;
		V v;
		Read(out s, out t, out u, out v);
		return new Tuple<S, T, U, V>(s, t, u, v);
	}
	public Tuple<S, T, U, V, W> Get<S, T, U, V, W>()
	{
		S s;
		T t;
		U u;
		V v;
		W w;
		Read(out s, out t, out u, out v, out w);
		return new Tuple<S, T, U, V, W>(s, t, u, v, w);
	}
	public Tuple<S, T, U, V, W, X> Get<S, T, U, V, W, X>()
	{
		S s;
		T t;
		U u;
		...
		W w;
		X x;
		Read(out s, out t, out u, out v, out w, out x);
		return new Tuple<S, T, U, V, W, X>(s, t, u, v, w, x);
	}
	public Tuple<S, T, U, V, W, X, Y> Get<S, T, U, V, W, X, Y>()
	{
		S s;
		T t;
		U u;
		...
		X x;
		Y y;
		Read(out s, out t, out u, out v, out w, out x, out y);
		return new Tuple<S, T, U, V, W, X, Y>(s, t, u, v, w, x, y);
	}
	public Tuple<S, T, U, V, W, X, Y, Z> Get<S, T, U, V, W, X, Y, Z>()
	{
		S s;
		T t;
		U u;
		...
		Y y;
		Z z;
		Read(out s, out t, out u, out v, out w, out x, out y, out z);
		return new Tuple<S, T, U, V, W, X, Y, Z>(s, t, u, v, w, x, y, z);
	}
	public Pair<S, T> Pair<S, T>()
	{
		S s;
		T t;
		Read(out s, out t);
	...
	object Convert(string str, TypeCode type)
	{
		...
		else if (type == Type.GetTypeCode(typeof(Point)))
		{
			int s, t;
			Read(out s, out t);
	...
	public T[] ReadMany<T>()
	{
		...
		return file.ReadLine().Split(sep, StringSplitOptions.RemoveEmptyEntries).Select(str => (T)Convert(str, type)).ToArray();
	}
	public T[] ReadMany<T>(int n)
	{
		var type = Type.GetTypeCode(typeof(T));
		return file.ReadLine().Split(sep, StringSplitOptions.RemoveEmptyEntries).Take(n).Select(str => (T)Convert(str, type)).ToArray();
	}
	public T[] ReadManyLines<T>(int n, Func<T> selector)
	{
		return Enumerable.Range(0, n).Select(_ => selector()).ToArray();
	}
	public T[] ReadManyLines<T>(int n)
	{
		return Enumerable.Range(0, n).Select(_ => Get<T>()).ToArray();
	}
	public Tuple<S, T>[] ReadManyLines<S, T>(int n)
	{
		return Enumerable.Range(0, n).Select(_ => Get<S, T>()).ToArray();
	}
	public Tuple<S, T, U>[] ReadManyLines<S, T, U>(int n)
	{
		return Enumerable.Range(0, n).Select(_ => Get<S, T, U>()).ToArray();
	}
	public Tuple<S, T, U, V>[] ReadManyLines<S, T, U, V>(int n)
	{
		return Enumerable.Range(0, n).Select(_ => Get<S, T, U, V>()).ToArray();
	}
	public Tuple<S, T, U, V, W>[] ReadManyLines<S, T, U, V, W>(int n)
	{
		return Enumerable.Range(0, n).Select(_ => Get<S, T, U, V, W>()).ToArray();
	}
	public Tuple<S, T, U, V, W, X>[] ReadManyLines<S, T, U, V, W, X>(int n)
	{
		return Enumerable.Range(0, n).Select(_ => Get<S, T, U, V, W, X>()).ToArray();
	}
	public Tuple<S, T, U, V, W, X, Y>[] ReadManyLines<S, T, U, V, W, X, Y>(int n)
	{
		return Enumerable.Range(0, n).Select(_ => Get<S, T, U, V, W, X, Y>()).ToArray();
	}
	public Tuple<S, T, U, V, W, X, Y, Z>[] ReadManyLines<S, T, U, V, W, X, Y, Z>(int n)
	{
		return Enumerable.Range(0, n).Select(_ => Get<S, T, U, V, W, X, Y, Z>()).ToArray();
	}
	public T[,] ReadManyManyLines<T>(int X, int Y)
	{
		...
		for (var y = 0; y < Y; y++)
		{
			var tmp = ReadMany<T>(X);
			for (var x = 0; x < X; x++) array[x, y] = tmp[x];
		}
		return array;
	}
...
static class Func
{
	...
	public static Comparison<T> DefaultComparison<T>()
	{
		return (x, y) => Comparer<T>.Default.Compare(x, y);
	}
	public static Comparison<T> ToComparison<T>(this IComparer<T> comp)
	{
		return comp == null ? DefaultComparison<T>() : (x, y) => comp.Compare(x, y);
	}
	/// <summary>
	...
	public static T IndexAt<T>(this T[,] array, Pair<int, int> index)
	{
		return array[index.First, index.Second];
	}
	public static bool InRegion(this Pair<int, int> p, int X, int Y)
	{
		return p.InRegion(0, X, 0, Y);
	}
	public static bool InRegion(this Pair<int, int> p, int x, int X, int y, int Y)
	{
		return p.First >= x && p.Second >= y && p.First < X && p.Second < Y;
	}
	/// <summary>
	...
	{
		return Math.Abs((n / GCD(n, m)) * m);
	}
	public static long Divide(long n, long m)
	{
		return (n - Remainder(n, m)) / m;
	}
	...
	public static Tuple<long, long> SolveLinear(long n, long m)
	{
		if (n < 0)
		{
			var p = SolveLinear(-n, m);
			return p == null ? p : new Tuple<long, long>(-p.Item1, p.Item2);
		}
		if (m < 0)
		{
			var p = SolveLinear(n, -m);
		...
		if (n < m)
		{
			var p = SolveLinear(m, n);
			return p == null ? p : new Tuple<long, long>(p.Item2, p.Item1);
		}
	...
	{
		var n = (ulong)Math.Abs(a);
		var m = (ulong)Math.Abs(b);
	...
	public static long KnapsackN(long[] w, long[] v, int W)
	{
		...
		Func<int, int, List<Pair<long, long>>> gen = (start, end) => {
			...
			list.Sort((x, y) => {
				var c = x.First.CompareTo(y.First);
				return c == 0 ? x.Second.CompareTo(y.Second) : c;
			});
			var n = list.Count;
			if (n == 0) return list;
	...
	public static int MaxElement<T>(this IEnumerable<T> source) where T : IComparable<T>
	{
		return source.MaxElement((x, y) => x.CompareTo(y));
	}
	public static int MinElement<T>(IEnumerable<T> source, Comparison<T> comp)
	{
		return source.MaxElement((x, y) => comp(y, x));
	}
	public static int MinElement<T>(IEnumerable<T> source) where T : IComparable<T>
	{
		return source.MaxElement((x, y) => y.CompareTo(x));
	}
	public static void Shuffle<T>(IList<T> source, Random rand)
	{
		for (var i = source.Count - 1; i >= 0; --i) source.Swap(i, rand.Next(0, i + 1));
	}
	public static void Shuffle<T>(IList<T> source, RandomSFMT rand)
	{
		for (var i = source.Count - 1; i >= 0; --i) source.Swap(i, rand.Next(0, i + 1));
	}
	public static char NextChar(this Random rand)
	{
		return (char)(rand.Next(0, 'z' - 'a' + 1) + 'a');
	}
	public static char NextChar(this RandomSFMT rand)
	{
		return (char)(rand.Next(0, 'z' - 'a' + 1) + 'a');
	}
	public static string NextString(this Random rand, int length)
	{
		return new string(Enumerable.Range(0, length).Select(_ => rand.NextChar()).ToArray());
	}
	public static string NextString(this RandomSFMT rand, int length)
	{
		return new string(Enumerable.Range(0, length).Select(_ => rand.NextChar()).ToArray());
	}
	...
	public static int SetToMin(ref int min, int other)
	{
		return min = Math.Min(min, other);
	}
	public static int SetToMax(ref int max, int other)
	{
		return max = Math.Max(max, other);
	}
	public static long SetToMin(ref long min, long other)
	{
		return min = Math.Min(min, other);
	}
	public static long SetToMax(ref long max, long other)
	{
		return max = Math.Max(max, other);
	}
	...
	public static decimal Abs(this decimal n)
	{
		return Math.Abs(n);
	}
	public static short Abs(this short n)
	{
		return Math.Abs(n);
	}
	public static sbyte Abs(this sbyte n)
	{
		return Math.Abs(n);
	}
	...
	public static uint Min(params uint[] nums)
	{
		var min = uint.MaxValue;
		foreach (var n in nums) min = Math.Min(min, n);
		return min;
	}
	public static ulong Min(params ulong[] nums)
	{
	...
	public static uint Max(params uint[] nums)
	{
		var min = uint.MinValue;
		foreach (var n in nums) min = Math.Max(min, n);
		return min;
	}
	public static ulong Max(params ulong[] nums)
	{
...
class RandomSFMT : Random
{
	...
	void period_certification()
	{
		...
		inner &= 1;
		if (inner == 1) return;
		for (i = 0; i < 4; i++) for (j = 0, work = 1; j < 32; j++, work <<= 1) if ((work & ParityData[i]) != 0)
				{
					x[i] ^= work;
					return;
				}
	...
	public RandomSFMT(uint[] init_key)
	{
		InitMtEx(init_key);
	}
	...
	public int NextBit()
	{
		...
		{
			coin_bits = 31;
			return (int)(coin_save = NextMt()) & 1;
		}
		else return (int)(coin_save >>= 1) & 1;
	}
	public int NextByte()
	{
		if (--byte_pos == -1)
	...
	{
		return 1 + NextIntEx(int.MaxValue);
	}
	public override void NextBytes(byte[] buffer)
	{
		for (var i = 0; i < buffer.Length; i++) buffer[i] = (byte)NextByte();
	}
	public override int Next(int min, int max)
	{
		return min + NextIntEx(max - min);
	}
