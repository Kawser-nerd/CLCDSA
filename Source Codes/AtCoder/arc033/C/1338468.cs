...
public class Set<T>
{
    ...
    {
        if (t == nil) { t = new Node(key); t.Left = t.Right = nil; t.Update(); return true; }
        var cmp = comparer.Compare(t.Key, key);
        bool res;
        if (cmp > 0)
            res = insert(ref t.Left);
        else if (cmp == 0)
        {
            if (IsMultiSet) res = insert(ref t.Left);
            else return false;
        }
        else res = insert(ref t.Right);
        balance(ref t);
        return res;
    }
    void removeAt(ref Node t, int k)
    {
        ...
        if (cnt < k) removeAt(ref t.Right, k - cnt - 1);
        else if (cnt > k) removeAt(ref t.Left, k);
        else
        {
            if (cnt == 0) { t = t.Right; return; }
            if (t.Right.Count == 0) { t = t.Left; return; }


            t.Key = find(t, k - 1);
            removeAt(ref t.Left, k - 1);
        }
        balance(ref t);
    }
