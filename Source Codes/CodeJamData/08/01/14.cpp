...
RealCase::RealCase()
{
    int S = PARSE<int>(LINE());
    REP(s, S) engines.push_back(LINE());
    int Q = PARSE<int>(LINE());
    REP(q, Q) queries.push_back(LINE());
}

int memo[100][1000];

int RealCase::calc(int e, int q)
{
    if (q == SZ(queries)) return 0;
    int &m = memo[e][q];
    if (m >= 0) return m;
    if (queries[q] != engines[e]) return m = calc(e, q + 1);

    int res = inf;
    REP(n, SZ(engines)) if (n != e) res = min(res, 1 + calc(n, q + 1));
    return m = res;
}
...
void RealCase::solve(Result &result)
{
    ...
    int res = inf;
    REP(e, SZ(engines)) res = min(res, calc(e, 0));
    result.set(STR<int>(res));
}
