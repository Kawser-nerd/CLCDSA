...
class J
{
	...
	void Solve()
	{
		...
		pow = new double[10, X * Y];
		pow[0, 0] = -1;
		for (var i = 1; i < 10; i++) pow[i, 0] = i;
		for (var i = 0; i < 10; i++) for (var t = 1; t < X * Y; t++) pow[i, t] = pow[i, t - 1] * 0.99;
		if (!OK(0))
		{
			WriteLine(-1);
			return;
	...
	bool OK(double m)
	{
		...
		while (q.Count > 0)
		{
			...
			for (var i = 0; i < 4; i++)
			{
				...
				if (0 <= u && u < X && 0 <= v && v < Y && !used[u, v] && pow[map[u, v], t + 1] >= m)
				{
					used[u, v] = true;
