...
ld prob(int u, int t) {
  // from t, pick t-u cards
  // from C-t, pick N-(t-u) cards
  
  if(C-t < 0 || t-u < 0 || t-u > t || N-(t-u) < 0 || N-(t-u) > C-t)
    return 0;
  
  return binom[t][t-u] * binom[C-t][N-(t-u)] / binom[C][N];
  }
...
//  printf("%d/%d %d: %.8lf\n", N,C, t, double(pleft[t]));
//  printf("X = %.8lf p = %.8lf\n", double(X), double(p));
    }
  
  printf("Case #%d: %.8lf\n", cnum+1, double(pleft[C]));
  }
