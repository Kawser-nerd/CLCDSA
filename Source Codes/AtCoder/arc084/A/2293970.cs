...
namespace CS_Contest {
	...
	internal class Program {
		...
		public class Calc
		{
			public void Solve() {
			    int N = NextInt();
			    var A = NextIntList().ToArray();
			    var B = NextIntList().ToArray();
			    var C = NextIntList().ToArray();

                Array.Sort(A);
                Array.Sort(B);
                Array.Sort(C);

			    var ans = (from b in B let a = A.LowerBound(b) let c = N - C.UpperBound(b) select (a * (long) c)).Sum();
			    ans.WL();
                return;
		    }
        }
