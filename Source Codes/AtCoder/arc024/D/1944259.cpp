...
int ans[1001][1001]={0};
void solve(vector<pair<int,int>> &bas,int hi,int mg){
	if(hi>=mg){return;}
	int man=(hi+mg)/2;
	solve(bas,hi,man-1);
	solve(bas,man+1,mg);
	for(int i=hi;i<=mg;i++){ans[bas[man].fir][bas[i].sec]=1;}
}
int main(void){
	int n,i;cin>>n;
	vector<pair<int,int>>bas(n);
	for(i=0;i<n;i++){
		cin>>bas[i].fir>>bas[i].sec;
		ans[bas[i].fir][bas[i].sec]=1;
	}
	SO(bas);
	solve(bas,0,n-1);
	for(i=0;i<n;i++){ans[bas[i].fir][bas[i].sec]=0;}
	vector<pair<int,int>>ret;
	for(i=0;i<=1000;i++){
		for(int j=0;j<=1000;j++){
				if(ans[i][j]==1){ret.pub(mp(i,j));}
		}
	}
	cout<<ret.size()<<endl;
	for(auto it:ret){cout<<it.fir<<" "<<it.sec<<endl;}
	RE;
}