...
public class Program
{
    ...
    int[] BinarySearch(int N, int L, int R, Action Init, Action<int> Act, Predicate<int> OK)
    {
        ...
        Queue<int[]> cand = new Queue<int[]>(), nextCand;
        ...
        for (; cand.Count > 0; cand = nextCand, range = nextRange)
        {
            ...
            while (cand.Count > 0)
            {
                ...
                for (int i = 0; i < A.Length; i++)
                    if (OK(A[i])) { numL++; toLeft[i] = true; }
                    else toLeft[i] = false;
                int[] next1 = new int[numL], next2 = new int[A.Length - numL];
                for (int i = 0, ai = 0, bi = 0; i < A.Length; i++)
                    if (toLeft[i]) next1[ai++] = A[i];
                    else next2[bi++] = A[i];
                for (int i = mid; i < b; i++) Act(i);
                if (b - a == 1) { Array.ForEach(A, i => res[i] = b); continue; }
