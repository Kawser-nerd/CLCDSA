...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			foreach (var r in rs)
			{
				var u = r.Item1 - 1; var v = r.Item2 - 1;
				es[u].Add(v);
				es[v].Add(u);
			}
			color = new bool[N];
			used = new bool[N];
			var cs = new List<Tuple<int, bool>>(); // (size, isBipartite)
			for (var u = 0; u < N; u++) if (!used[u]) cs.Add(DFS(u, -1));
			var n1 = cs.Count(x => x.Item1 == 1);
			var K = N - n1;
			var np = cs.Count(x => x.Item1 > 1 && x.Item2);
			var nn = cs.Count(x => x.Item1 > 1 && !x.Item2);
			var s = N * (long)N - K * (long)K + nn * nn + np * 2L * np + nn * 2L * np;
			pr.WriteLine(s);
		}
		Tuple<int, bool> DFS(int u, int p)
		{
			used[u] = true;
			...
			{
				if (used[v]) flag &= color[u] ^ color[v];
				else
				{
					var t = DFS(v, u);
					size += t.Item1;
			...
			return new Tuple<int, bool>(size, flag);
		}
