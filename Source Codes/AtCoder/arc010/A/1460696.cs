...
namespace AtCoder {
    class MainClass {
        ...
        void Solve() {
            ...
            io.o(isEnable ? "complete" : (day+1).ToString());
    ...
    class IO {
        ...
        public void i<T, U>(out T v1, out U v2) {
            var a = arr(); v1 = suitType<T>(a[0]); v2 = suitType<U>(a[1]);
        }
        ...
        public void ol<T>(params T[] a) { foreach (T v in a) Console.Write(v + " "); 
            Console.WriteLine(""); }
    ...
    class Mathf {
        ...
        public long Fact(long n) { return (n != 0) ? 
            (n % mod * (Fact(n - 1) % mod) % mod) : 1; }
