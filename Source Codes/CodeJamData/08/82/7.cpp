...
void solve_case(int casen)
{
	...
	vector<pair<int, int> > offers[308];
	int colors = 0;
	...
	for(int i = 0; i < N; ++i)
	{
		...
		int a, b;
		in >> c >> a >> b;
		typeof(color_map.begin()) ci = color_map.find(c);
		if(ci == color_map.end())
		{
			color_map[c] = colors++;
			ci = color_map.find(c);
		...
		offers[ci->second].push_back(make_pair(a, b));
	}
...
//	cerr << colors << endl;
	...
	{
		for(int c2 = c1; c2 < colors; ++c2)
		{
			for(int c3 = c2; c3 < colors; ++c3)
			{
				int p1 = 0;
				int p2 = 0;
				int p3 = 0;
				...
				{
					int up = 0;
//					cerr << offers[c1][p1].first << endl;
					for(; offers[c1][p1].first <= b && p1 < offers[c1].size(); ++p1)
						up = max(up, offers[c1][p1].second);
					for(; offers[c2][p2].first <= b && p2 < offers[c2].size(); ++p2)
						up = max(up, offers[c2][p2].second);
					for(; offers[c3][p3].first <= b && p3 < offers[c3].size(); ++p3)
						up = max(up, offers[c3][p3].second);
					b = up + 1;
					++num;
