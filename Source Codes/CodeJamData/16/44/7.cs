...
namespace GoogleCodeJam
{
	class Program
	{
		...
		static bool check(bool[][] ia)
		{
			...
			{
				bool[] seen = new bool[h.Count];
				for (int j = 0; j < h.Count; j++)
				{
					if (i == j) continue;

					foreach (int m in h[j])
					{
						seen[j] |= ia[i][m];
					}
				}
				if (seen.Count(x => x) >= h[i].Count)
					return false;
			}
			return true;
		}
		...
		static string solveCase(int num)
		{
			checked
			{
				...
				var a = readField(n, x => x == '1').SelectMany(x => x).ToArray();
				int bct = a.Count(x => x);
				int best = int.MaxValue;
				foreach (var c in new[] { true, false }.Pow(n * n))
				{
					var r = c.ToArray();
					bool bad = false;
					for (int i = 0; i < n * n; i++)
					...
					if (check(t))
						best = Math.Min(best, r.Count(x => x) - bct);
				}
