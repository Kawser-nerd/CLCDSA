...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void Read()
		{
			sc.Read(out N);
			A = sc.ReadMany<int>();
			rs = sc.ReadManyLines<int, int>(N - 1);
		}
		List<int>[] es;
		int root;
		void SolveOne()
		{
			...
			foreach (var r in rs)
			{
				es[r.Item1 - 1].Add(r.Item2 - 1);
				es[r.Item2 - 1].Add(r.Item1 - 1);
			}
			for (var i = 0; i < N; i++) if (es[i].Count == 1) { root = i; break; }
			pr.WriteLine(OK() ? "YES" : "NO");
		}
		bool OK()
		{
			foreach (var c in es[root]) return DFS(c, root) == A[root];
			return true;
		}
		long DFS(int u, int r)
		{
			long s = 0, M = -1;
			var n = es[u].Count - 1;
			foreach (var v in es[u]) { if (v == r) continue; var y = DFS(v, u); s += y; M = Max(M, y); if (y < 0) return -1; }
			if (n == 0) return A[u];
			else if (n == 1) return A[u] != M ? -1 : A[u];
			else
			{
				if (A[u] > s || A[u] < M) return -1;
				return 2L * A[u] - s;
			}
		}
