...
class Program
{
    ...
    void solve()
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            {
                int h = X + j;
                double b = (double)R * (H - j) / H;
                double a = (double)R * (H - j - 1) / H;
                D[h] += PI * (Pow(b, 2) * (H - j) - Pow(a, 2) * (H - j - 1)) / 3;
            }
        }
        var G = new double[20001];
