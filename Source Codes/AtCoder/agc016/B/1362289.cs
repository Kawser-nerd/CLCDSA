...
namespace Program
{
    public class Solver
    {
        public void Solve()
        {
            ...
            if (min == max)
            {
                if (min == n - 1) IO.Printer.Out.WriteLine("Yes");
                else if (n / 2 >= min) IO.Printer.Out.WriteLine("Yes");
                else IO.Printer.Out.WriteLine("No");
            }
            else
            {
                var k = a.Count(x => x == min);
                var rem = n - k;
                if (rem == 1) IO.Printer.Out.WriteLine("No");
                else if (k + rem / 2 < max || k + Min(1, rem) > max) IO.Printer.Out.WriteLine("No");
                else IO.Printer.Out.WriteLine("Yes");
            }
        }
