...
bool test(double T) {
  deque<pair<ll, double>> lo, hi;
  double v = 0.0;
  rep (i, N) {
    if (C[i] == X) v += T * R[i];
    if (C[i] < X) lo.emplace_back(X - C[i], T * R[i]);
    if (C[i] > X) hi.emplace_back(C[i] - X, T * R[i]);
  }
  ...
  while (v < V) {
    ...
    pair<ll, double> &l = lo[0], &h = hi[0];
    double t = min(l.first * l.second, h.first * h.second);
    v += t / l.first + t / h.first;
    l.second -= t / l.first;
    h.second -= t / h.first;
    if (l.second < EPS) lo.pop_front();
    if (h.second < EPS) hi.pop_front();
  }
