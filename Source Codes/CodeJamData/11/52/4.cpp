...
bool doIt(int L)
{
	...
	for (int i=0;i<maxn;i++)
	{
		...
		if (c[i]>0)
		{
			if (i+L-1>=maxn) return false;
			int cnt=c[i];
			for (int j=0;j<L;j++) if (c[i+j]<cnt) return false;
			for (int j=0;j<L;j++) c[i+j]-=cnt;
			if (i+L<maxn) d[i+L]+=cnt;
		}
	}
	return true;
}
...
/*
	...
	for (int set=0;set<two(n);set++) if (f[set]>=0)
		for (int subset=1;subset+set<two(n);subset++) if ((subset&set)==0)
		{
			vector<int> g;
			for (int i=0;i<n;i++) if (contain(subset,i)) g.push_back(a[i]);
			sort(g.begin(),g.end());
			bool good=true;
			for (int i=1;i<SIZE(g);i++)
				if (g[i]!=g[i-1]+1)
					good=false;
			if (good)
				checkmax(f[set+subset],min(f[set],SIZE(g)));
		}
	int ret=f[two(n)-1];
	if (ret!=H)
	{
...
//	freopen("B-small-attempt2.in","r",stdin);freopen("B-small-attempt2.out","w",stdout);
	char large_outputfilename[100]; sprintf(large_outputfilename,"B-large%d.out",d_testcase); freopen("B-large.in","r",stdin);freopen(large_outputfilename,"w",stdout);
	int testcase;
	scanf("%d",&testcase);
