...
public class Program
{
    public static void Solve()
    {
        ...
        if (n == 1 && m > 4 || n > 4 && m == 1) Answer(Max(n, m) - 2);
        if (n == 2 && m > 4 || n > 4 && m == 2) Answer(0);
        if (n * m > 50) Answer((n - 2) * (m - 2));

        var grid = new Grid2D<bool>((int)n, (int)m);
        ...
        {
            grid[p] = !grid[p];
            foreach (var q in grid.MooreNeighborhood(p))
                grid[q] = !grid[q];
        }

        var ans = Point2D.InnerPoints(grid.Size).Count(p => !grid[p]);
