...
int N, A;
bool win[110][110];  // win[i][j] := |i| is stronger than |j|
...
bool can(vector<bool> alive, int s, int t) {
  vector<int> v = {s};
  if (s != t) v.push_back(t);
  alive[s] = false;
  alive[t] = false;
 ...
 retry:
  rep (k, N) if (alive[k]) {
    for (int i = 0; i + 1 < (int)v.size(); ++i) {
      if (win[v[i + 1]][k] && win[k][v[i]]) {
        v.insert(v.begin() + i + 1, k);
        alive[k] = false;
        goto retry;
      }
    }
  ...
  // for (int a : v) debug("%d ", a); debug("\n");

  for (int i = 0; i + 1 < (int)v.size(); ++i) {
    if (!win[v[i + 1]][v[i]]) return false;
  }
  
  rep (k, N) if (alive[k]) {
    if (!win[v[0]][k]) return false;
  }
  return true;
}
...
vector<int> serious() {
  try {
    ...
    rep (i, N) {  // i-th element
      // debug("-----\n");
      // debug("i = %d\n", i);
      rep (k, N) {  // Try item |k|
        if (!alive[k]) continue;
        int th = (h == -1 || win[k][h]) ? k : h;
        // debug(" k = %d (%d)\n", k, th);
