...
// list declaration
VarList list_a({Var(7), Var(4), Var(2)});
...
int func_main(){
    var_swap = Var(1);
    while (!(var_swap == Var(" 0"))){
        var_swap = Var(0);
        var_i = Var(1);
        for (int cnt_1=0;cnt_1<(Var(list_a.size()) - Var(1)).asNumber();cnt_1++){
            if ((list_a.getLineOfList(var_i) < list_a.getLineOfList((var_i + Var(1))))){
                var_tmp = list_a.getLineOfList(var_i);
                list_a.setLineOfListTo(var_i, list_a.getLineOfList((var_i + Var(1))));
                list_a.setLineOfListTo((var_i + Var(1)), var_tmp);
                var_swap = Var(1);
            }
            var_i = (var_i + Var(1));
        }
    ...
    var_i = Var(1);
    for (int cnt_2=0;cnt_2<Var(list_a.size()).asNumber();cnt_2++){
        if (((var_i % Var(2)) == Var(1))){
            var_Alice = (var_Alice + list_a.getLineOfList(var_i));
        } else {
            var_Bob = (var_Bob + list_a.getLineOfList(var_i));
        }
        var_i = (var_i + Var(1));
    }
    cout << (var_Alice - var_Bob) << endl;
    return 0;
}