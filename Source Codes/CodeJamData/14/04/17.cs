...
namespace Codejam
{
    class D
    {
        ...
        private void Test(int tt)
        {
            ...
            List<Tuple<double, bool>> all = new List<Tuple<double, bool>>();

            for (int i = 0; i < N; i++)
			{
                naomi[i] = GetDouble();
                all.Add(new Tuple<double, bool>(naomi[i], true));
			}
            for (int i = 0; i < N; i++)
            {
                ken[i] = GetDouble();
                all.Add(new Tuple<double, bool>(ken[i], false));
            }
