...
struct SegTreeT{
    ...
    //     init(0, num);
    // }
    SegTreeT(vector<T>& data): nodes(data.size() * 4){
    ...
    void init(int st, int lt, vector<T>& data, size_t idx = 1){
        ...
        if(st == lt){
            ...
            init(st, (lt - st) / 2 + st, data, idx * 2);
            init((lt - st) / 2 + st + 1, lt, data, idx * 2 + 1);
        }
    }
    void add(int st, int lt, T val, size_t idx = 1){
        ...
        if(nodes[idx].inside(st, lt)){
            ...
            add(st, lt, val, idx * 2);
            add(st, lt, val, idx * 2 + 1);
        }
    }
    T getVal(int pos, size_t idx = 1){
        if(idx >= nodes.size() || nodes[idx].outside(pos)){ return 0; }
        return nodes[idx].value + getVal(pos, idx * 2) + getVal(pos, idx * 2 + 1);
    }
};
using SegTree = SegTreeT<int>;
