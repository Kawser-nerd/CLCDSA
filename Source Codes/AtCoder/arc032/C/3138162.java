...
public class Main {
	private static void solve() {
		...
		TreeMap<Integer, Integer> compress = new TreeMap<Integer, Integer>();
		int[] uncompress = new int[200100];
		...
		for (int i = 0; i < n; i++) {
			ws[i][0] = compress.get(ws[i][0]);
			ws[i][1] = compress.get(ws[i][1]);
		}

		int[] numjs = new int[T];
		int[][][] js = new int[T][][];

		Arrays.sort(ws, (a, b) -> a[0] - b[0]);
		...
		for (int i = 0; i < T; i++) {
			js[i] = new int[numjs[i]][2]; // {end, index}
			numjs[i] = 0;
		}
		for (int i = 0; i < n; i++) {
			int st = ws[i][0];
			...
			int idx = ws[i][2];
			js[st][numjs[st]][0] = en;
			js[st][numjs[st]][1] = idx;
			numjs[st]++;
		}

//		for (int i = 0; i < T; i++) {
//			System.out.println("time " + i);
//			for (int j = 0; j < numjs[i]; j++) {
//				System.out.println("end=" + js[i][j][0] + ", idx=" + js[i][j][1]);
//			}
//		}
		...
		for (int i = T - 1; i >= 0; i--) {
			...
			int endTime = i + 1;
			for (int j = 0; j < numjs[i]; j++) {
				int newDp = dp[js[i][j][0]][0] + 1;
				if (newDp > max || newDp == max && js[i][j][1] < maxIdx) {
					max = newDp;
					maxIdx = js[i][j][1];
					endTime = js[i][j][0];
				}
			...
			dp[i][1] = maxIdx;
			dp[i][2] = endTime;
			dp[i][3] = maxIdx == dp[i + 1][1] ? 0 : 1;
		}
		out(dp[0][0]);
		int idx = 0;
		String[] list = new String[dp[0][0]];
		int cnt = 0;
		while (idx < T) {
			if (dp[idx][3] == 1) {
				list[cnt++] = "" + dp[idx][1];
