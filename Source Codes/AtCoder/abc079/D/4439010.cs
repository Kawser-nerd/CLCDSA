...
namespace Practice
{
    class Program
    {
        ...
        const int kDigitTotal = 10;

        private static int[] costTable = new int[kDigitTotal];
        private static int[][] c = new int[kDigitTotal][];
        private static int[] color = new int[kDigitTotal];
        private static int[] path = new int[kDigitTotal];
        static void Solve()
        {
            var o = ReadAndParseIntArr();
            var h = o[0];
            var w = o[1];
            c = new int[kDigitTotal][];
            for(int i=0;i< kDigitTotal; ++i)
            {
                c[i] = ReadAndParseIntArr();
            }
            var a = new int[h][];
            for(int i=0;i<h;++i)
            {
                a[i] = ReadAndParseIntArr();
            }
            for(int k=0;k<kDigitTotal;++k)
            {
                for(int i=0;i<kDigitTotal;++i)
                {
                    for(int j=0;j<kDigitTotal;++j)
                    {
