...
namespace AtCoder {
    class MainClass {
        ...
        int x2, y2, x3, y3;
        void Solve() {
            io.i(out x1, out y1, out r);
            io.i(out x2, out y2, out x3, out y3);
            ...
            edge[0] = mat.Dis(x1, y1, x2, y2) <= r;
            edge[1] = mat.Dis(x1, y1, x3, y2) <= r;
            edge[2] = mat.Dis(x1, y1, x2, y3) <= r;
            edge[3] = mat.Dis(x1, y1, x3, y3) <= r;

            if (edge.All(f => f == true)) {
                red = true;
                blue = false;
                goto EXIT;
            }

            if (edge.Any(f => f == true)) {
                red = true;
                blue = true;
                goto EXIT;
            }

            if (x3 - x2 < 2 * r && y3 - y2 > 2 * r|| x3 - x2 > 2 * r && y3 - y2 < 2 * r) {
                red = true;
                blue = true;
                goto EXIT;
            }
    ...
    class Mat {
        ...
        public int[] DigVal(int n) {//????...
            ...
            //var a = n.ToString().Reversed().ToCharArray();
    ...
    class AssociativeArray<T> {
        ...
        public void init(T[] a) {
            dic = new Dictionary<T, int>(); Add(a);
        }
        public void Add(T[] a) {
            for (int i = 0; i < a.Length; i++) {
