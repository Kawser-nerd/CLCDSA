...
//        setLR(imosu, l, r);
        ...
        for (int i = 0; i < M; i++) {
//            int ind = rlowerBound(l, f[i]);
            int ind = lowerBound(l, f[i]);
            if (ind == l.size() || l.get(ind) != f[i]) ind--;
            if (ind < 0) continue;
            int lv = l.get(ind);
            int rv = r.get(ind);
            if (lv <= f[i] && t[i] < rv) res.add(i + 1);
        }
    ...
    public static <T extends Number> int rlowerBound(final List<T> lis, final T value) {
        ...
        if (ind == lis.size() || lis.get(ind) != value) ind--;
        return ind;
    }
