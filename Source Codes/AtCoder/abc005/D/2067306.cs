...
namespace ConsoleApplication {
    class MainClass {
        ...
        void sliding_window_init() {
            ...
            for(int i = N - 2; i >= 0; --i)
                memo[N - 1, i] += memo[N - 1, i + 1];
            for(int i = N - 2; i >= 0; --i)
                for(int j = N - 2; j >= 0; --j)
                    memo[i, j] += memo[i + 1, j] + memo[i, j + 1] - memo[i + 1, j + 1];
        }
        int sliding_window(int num) {
            var ret = 0;
            for(int h = 1; h <= Math.Min(num, N); ++h) {
                var w = Math.Min(num / h, N);
                if(N < h || N < w)
                    continue;
                ret = Math.Max(ret, memo[N - h, N - w]);
                for(int i = 0; i <= N - h; ++i) {
                    for(int j = 0; j <= N - w; ++j) {
                        var t = memo[i, j];
                        if(j + w < N && i + h < N)
