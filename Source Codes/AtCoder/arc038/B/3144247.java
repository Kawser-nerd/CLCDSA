...
public class Main {
    ...
    public static void solve() throws Exception {
        //long???????????????
        int h = ni(), w = ni();
        boolean[][] win = toBooleanTable(nctp(h, w, '#'), '#');
        for (int hi = h + 1; hi >= 0; hi--) {
            for (int wi = w + 1; wi >= 0; wi--) {
                if (win[hi][wi]) continue;
                if (!win[hi + 1][wi] || !win[hi + 1][wi + 1] || !win[hi][wi + 1]) win[hi][wi] = true;
            }
        }
        System.out.println(win[1][1] ? "First" : "Second");
    }
    ...
    public static boolean[][] toBooleanTable(char[][] s, char c) {
    ...
    public static int[][] toIntTable(char[][] a) {
