...
int dfs(int node) {
    ...
    auto dp = make_vector<int, 2>(xx.size() + 1, X + 1, inf);

    dp[0][0] = 0;

    for (int i : rep(0, xx.size())) {
        for (int j : rep(0, X + 1) | reversed) {
            if (j - xx[i] >= 0) {
                dp[i + 1][j] = min(dp[i + 1][j], dp[i][j - xx[i]] + yy[i]);
            }

            if (j - yy[i] >= 0) {
                dp[i + 1][j] = min(dp[i + 1][j], dp[i][j - yy[i]] + xx[i]);
            }
        }
    }
...
int main() {
    ...
    out(ret != inf ? "POSSIBLE" : "IMPOSSIBLE");
}