...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			if (A == 1)
			{
				foreach (var x in set) pr.WriteLine(x);
				return;
			}
			while (set.Min() * A < set.Max() && B > 0)
			{
				var x = set.MinNode();
				set.Add(x.Item * A);
				set.Remove(x.Item, x);
			...
			if (B == 0)
			{
				foreach (var x in set) pr.WriteLine(x % Mod);
				return;
			}
			var b = new long[N];
			var l = B % N;
			var p = Func.Pow(A, B / N, Mod);
			for (var i = 0; i < l; i++)
			{
				var x = set.MinNode();
				b[N - l + i] = x.Item * A % Mod * p % Mod;
				set.Remove(x.Item, x);
			}
			for (var i = l; i < N; i++)
			{
			...
			foreach (var x in b) pr.WriteLine(x);
		}
	}
}
