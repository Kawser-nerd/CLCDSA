...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		int Y, X, Q;
		bool[,] map;
		Tuple<int, int, int, int>[] qs;
		void Read()
		{
			sc.Read(out Y, out X, out Q);
			map = sc.Board(X, Y, (c, x, y) => c == '1');
			qs = sc.ReadManyLines<int, int, int, int>(Q);
		}
		void SolveOne()
		{
			var vs = new int[X + 1, Y + 1];
			for (var x = 0; x < X; x++) for (var y = 0; y < Y; y++) vs[x + 1, y + 1] = vs[x + 1, y] + vs[x, y + 1] - vs[x, y] + (map[x, y] ? 1 : 0);
			var ex = new int[X, Y + 1];
			for (var x = 0; x < X - 1; x++) for (var y = 0; y < Y; y++) ex[x + 1, y + 1] = ex[x + 1, y] + ex[x, y + 1] - ex[x, y] + (map[x, y] & map[x + 1, y] ? 1 : 0);
			var ey = new int[X + 1, Y];
			for (var x = 0; x < X; x++) for (var y = 0; y < Y - 1; y++) ey[x + 1, y + 1] = ey[x + 1, y] + ey[x, y + 1] - ey[x, y] + (map[x, y] & map[x, y + 1] ? 1 : 0);
			foreach (var q in qs)
			{
				int y1 = q.Item1 - 1, y2 = q.Item3, x1 = q.Item2 - 1, x2 = q.Item4;
				var v = vs[x2, y2] - vs[x2, y1] - vs[x1, y2] + vs[x1, y1];
				var a = ex[x2 - 1, y2] - ex[x2 - 1, y1] - ex[x1, y2] + ex[x1, y1];
				var b = ey[x2, y2 - 1] - ey[x2, y1] - ey[x1, y2 - 1] + ey[x1, y1];
				pr.WriteLine(v - a - b);
			}
		}
