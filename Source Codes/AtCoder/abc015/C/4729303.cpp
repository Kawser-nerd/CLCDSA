...
int main(void){
  short n, k, st; cin >> n >> k;
  bool koho[n][128]; // koho[p][q] := ???p+1???, q????????
  for(short i = 0; i < n; i++) for(short j = 0; j < 128; j++) koho[i][j] = false;
  for(short j = 0; j < k; j++){
    cin >> st; koho[0][st] = true;
  }
  for(short i = 1; i < n; i++){
    for(short j = 0; j < k; j++){
      cin >> st;
      for(short l = 0; l < 128; l++) if(koho[i-1][l]){
        koho[i][l ^ st] = true;
      }
    }
  }
  cout << (koho[n-1][0] ? "Found" : "Nothing") << BR;
  return 0;
}