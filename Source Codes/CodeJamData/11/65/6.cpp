...
int m,a,d;
double f[maxsize],g[maxsize],h[maxsize];
int b[maxsize];

void solve()
{
	for (int i=0;i<d;i++) g[i]=f[i];
	for (int i=0;i<d;i++)
	{
		f[i]=0;
		if (i==0) continue;
		for (int s=i;s>0;s--) if (s<=m)
		{
			for (int k=m;k>0;k--) if (k%s==0)
			{
				h[k]=0;
				h[k]+=0.5*((i+s>=d)?1.0:g[i+s]);
				double t1=(i-2*k+s<=0)?0:g[i-2*k+s];
				double t2=(k*2>m)?0:h[k*2];
				h[k]+=0.5*max(t1,t2);
			}
			if (h[s]>f[i]+1e-11) f[i]=h[s],b[i]=s;
		}
	}
}
...
//	freopen("E-large.in","r",stdin);freopen("E-large.out","w",stdout);
	...
	for (int case_id=1;case_id<=testcase;case_id++)
	{
		...
		scanf("%d%d%d",&a,&m,&d);
		for (int i=0;i<d;i++) b[i]=0,f[i]=0;
		for (int step=0;step<10000;step++) solve();
		printf("%.6lf %d\n",f[a],b[a]);
		fflush(stdout);
	}
	return 0;
