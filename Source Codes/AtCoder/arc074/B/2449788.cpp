...
#include<functional>
int main() {
	const auto m = IO.input<uint32>();
	std::vector<uint32> a(m * 3);
	for (auto &e : a)IO >> e;
	std::priority_queue<uint32, std::vector<uint32>, std::greater<uint32>> P;
	std::priority_queue<uint32> Q;
	std::vector<uint64> max(m + 1), min(m + 1);
	...
	uint64 ans = INT64_MIN;
	for (const auto i : irange<>(0, m + 1)) {
		smaxi(ans, max[i] - min[i]);
	}
	IO << ans << '\n';
	return 0;
}