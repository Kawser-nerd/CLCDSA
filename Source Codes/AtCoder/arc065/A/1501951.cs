...
namespace AtCoder {
    class MainClass {
        ...
        int Length;
        bool isEnable;
        readonly string[] add = { "dream", "dreamer", "erase", "eraser" };//5/7/5/6
        void Solve() {
            io.i(out S);
            Length = S.Length;
            S += "_".Repeat(10);
        ...
        bool rec(int cur) {
            ...
            if (S.Substring(cur, 5) == add[0] || S.Substring(cur, 5) == add[2]) {
                ret |= rec(cur + 5);
            }
            if (S.Substring(cur, 6) == add[3]) {
                ret |= rec(cur + 6);
            }
            if (S.Substring(cur, 7) == add[1]) {
                ret |= rec(cur + 7);
            }
            return ret;
        }
