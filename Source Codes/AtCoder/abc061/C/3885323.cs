...
public class Hello
{
    ...
    {
        long[] NK = SpRead(' ').Select(long.Parse).ToArray();
        Dictionary<int, long> dic = new Dictionary<int, long>();
        long N = NK[0], K = NK[1];
        int[] ab;
        for (int i = 0; i < N; i++)
        {
            ab = SpRead(' ').Select(int.Parse).ToArray();
            if (dic.ContainsKey(ab[0])) dic[ab[0]]+=ab[1];
            else dic.Add(ab[0], ab[1]);
        }
        var sorted = dic.OrderBy(v => v.Key);
        int ans = 0;
