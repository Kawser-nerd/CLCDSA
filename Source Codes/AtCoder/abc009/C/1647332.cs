	...
	internal class Program {
		...
		private class Calc {
			...
			public void Solve() {
				...
				var S = ReadLine();
				var list = S.OrderBy(x => x).ToList();

				Func<string, char, int> cnt = (T, c) =>
				{
					var s = S.Substring(0, T.Length + 1);
					var t = T + c;
					var add = 0;
					...
					var match = 0;
					for (char i = 'a'; i <= 'z'; i++) {
						match += Min(S.Count(i) - s.Count(i), S.Count(i) - t.Count(i));
					}
					add += (N - T.Length - 1) - match;
					return add;
				};
