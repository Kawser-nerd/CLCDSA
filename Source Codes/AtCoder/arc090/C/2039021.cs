...
class K
{
	...
	long Solve()
	{
		...
		var D = ds[t];
		// s -> u -e-> e.To -> t
		for (var u = 0; u < N; u++) if (done[u]) foreach (var e in g[u])
					if (done[e.To] && 2 * ds[u] < D && 2 * dt[e.To] < D && 2 * dt[u] > D && 2 * ds[e.To] > D && ds[u] + e.D == ds[e.To] && dt[e.To] + e.D == dt[u])
					{
						var k = cs[u] * ct[e.To] % MOD;
						ans = (ans - k * k % MOD) % MOD;
					}
		for (var u = 0; u < N; u++) if (done[u] && ds[u] == dt[u] && 2 * ds[u] == D)
			{
				var k = cs[u] * ct[u] % MOD;
				ans = (ans - k * k % MOD) % MOD;
			}
