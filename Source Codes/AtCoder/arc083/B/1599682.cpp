...
int main() {
    ...
    auto a = vin<int64_t>(n, n);

    vector<std::tuple<int64_t, int, int>> e;
    ...
    auto dist = make_vector<int64_t, 2>(n, n, inf);
    ...
    for(auto x : e) {
        ...
        if(s > d) {
            ...
            for(int i : rep(0, n)) {
                for(int j : rep(0, n)) {
                    dist[j][i] = dist[i][j] = min({dist[i][j], dist[i][u] + dist[u][v] + dist[v][j], dist[i][v] + dist[v][u] + dist[u][j]});
                }
            }
        } else if(s < d) {
            out(-1);
