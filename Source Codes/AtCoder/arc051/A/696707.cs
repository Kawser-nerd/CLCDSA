...
class Program
{
    ...
    static void Main()
    {
        ...
        var a = sc.IntArr;
        var b = sc.IntArr;
        if (a[0] + a[2] > b[2] || a[0] - a[2] < b[0] || a[1] + a[2] > b[3] || a[1] - a[2] < b[1])
            sw.WriteLine("YES");
        else
            sw.WriteLine("NO");
        if (dist(a[0], a[1], b[0], b[1]) > a[2] * a[2] || dist(a[0], a[1], b[0], b[3]) > a[2] * a[2] || dist(a[0], a[1], b[2], b[1]) > a[2] * a[2] || dist(a[0], a[1], b[2], b[3]) > a[2] * a[2])
            sw.WriteLine("YES");
        else
            sw.WriteLine("NO");
