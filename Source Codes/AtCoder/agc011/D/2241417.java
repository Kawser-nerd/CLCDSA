...
public class Main {
    ...
    public static void main(String[] args){
        ...
        int[] h=new int[n];
        for(int i=0;i<n;i++){
            h[i]=(str.charAt(i)=='A'?0:1);
        }
        //int[] ch=Arrays.copyOf(h,n);
        ...
        if(ind==-1){
            long rest=k-count;
            if(n%2==1) pr(rest%2==0?'A':'B');
            for(int i=0;i<n/2*2;i++){
                pr((i%2==0?'B':'A'));
            }
        }else{
            for(int i=ind;i<n;i++){
                pr((h[i]+parity)%2==0?'A':'B');
            }
            for(int i=0;i<ind;i++){
                pr((i+parity)%2==1?'A':'B');
            }
        ...
        n=52;k=52;pl(n+" "+k);
        for(int i=0;i<n;i++){
            pr((Math.random()<0.5)?'A':'B');
        }
        flush();*/
    }
    static int[] brute(int n,long k,int[] h){//O(n*n)
        int size=n;
        int isr=1;
        for(int i=0;i<k;i++){
            int ind=0;
            ...
            while(size>1 && h[size-2]==1 && h[size-1]==0) size-=2;
            if(size==0) break;
            if(size==1){
                i++;
                long rest=k-i;
