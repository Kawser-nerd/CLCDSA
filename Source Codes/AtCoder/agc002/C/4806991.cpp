...
int judge(void){
   REP(i, n-1) if(X[i] + X[i + 1] >= L) return i;
   return -1;
}
...
int main(void){
  ...
  REP(i, n) cin >> X[i];
  int u = judge();
  if(u == -1) std::cout << "Impossible" << BR;
  else{
    std::cout << "Possible" << BR;
    for(int i = 0; i != u; i++) std::cout << i + 1 << BR;
    for(int i = n-2; i != u; i--) std::cout << i + 1 << BR;
    std::cout << u + 1 << BR;
  }
  return 0;
}