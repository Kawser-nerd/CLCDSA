...
public class Main {
	private static void solve() {
		...
		int[] ansVec = new int[numPrimes];
		int[] factVec = new int[numPrimes];
		for (int i = 2; i <= t; i++) {
			int num = i;
			...
			for (int j = 0; j < numPrimes; j++) {
				ansVec[j] += factCoeff[i] * factVec[j];
			}
		}
		long ans = 1;
		for (int j = 0; j < numPrimes; j++) {
			ans = ans * modpow(primes[j], ansVec[j], mod) % mod;
		}
		out(ans);
