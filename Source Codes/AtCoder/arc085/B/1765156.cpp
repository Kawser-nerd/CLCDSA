...
int dfs1(int i, int j) {
    ...
    for (int k : rep(i, n)) {
        if (k != n - 1) {
            maxi = max(maxi, dfs2(k + 1, k));
        } else {
            maxi = max(maxi, abs(a[j] - a[n - 1]));
        }
    }

    return memo[0][i][j] = maxi;
}
...
int dfs2(int i, int j) {
    ...
    for (int k : rep(i, n)) {
        if (k != n - 1) {
            mini = min(mini, dfs1(k + 1, k));
        } else {
            mini = min(mini, abs(a[j] - a[n - 1]));
        }
    }

    return memo[1][i][j] = mini;
}
