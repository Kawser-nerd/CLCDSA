...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var isA = S[0] == 'A';
            var isC = Enumerable.Range (2, S.Count () - (2 - 1) - 3 + 1).Where (i => S[i] == 'C').Count () == 1;
            var isLower = S.Count () - S.Where (char.IsLower).Count () == 2;
            PrintIf (isA && isC && isLower, "AC", "WA");
            // Print ($"{isA} {isC} {isLower}");
        }
    }
