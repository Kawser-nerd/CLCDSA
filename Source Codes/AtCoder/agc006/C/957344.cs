...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			dN = N - 1;
			var dx = new int[dN];
			for (var i = 0; i < dN; i++) dx[i] = x[i + 1] - x[i];
			var perm = new int[dN];
			for (var i = 0; i < dN; i++) perm[i] = i;
			foreach (var q in a) perm.Swap(q - 2, q - 1);
			perm = Pow(perm, K);
			var dy = new int[dN];
			for (var i = 0; i < dN; i++) dy[i] = dx[perm[i]];
			var y = new long[N];
			y[0] = x[0];
			for (var i = 0; i < dN; i++) y[i + 1] = y[i] + dy[i];
			foreach (var z in y) pr.WriteLine(z);
		}
		int[] Pow(int[] p, long b)
		{
			var q = new int[dN];
			for (var i = 0; i < dN; i++) q[i] = i;
			var x = p;
			while (b > 0)
			{
				if ((b & 1) == 1) q = Comp(q, x);
