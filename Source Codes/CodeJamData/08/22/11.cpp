...
public:
    ...
    int find(int v) {
        vector<int> path(1, v);
        int parent = pr[v].first;
        while(parent != path.back())
        {
            path.push_back(parent);
            parent = pr[parent].first;
        }
        REP(n, path.size()) pr[path[n]].first = parent;
        return parent;
    }
...
RealCase::RealCase()
{
    if (primes.empty()) GENPRIMES(primes, 2000000);
    cin >> A >> B >> P;
}
...
void RealCase::solve(Result &result)
{
    ...
    LL res = 0;
    for (LL i = A; i <= B; ++i) if (uf.find(i - A) == i - A) ++res;
    result.set(STR(res));
}
