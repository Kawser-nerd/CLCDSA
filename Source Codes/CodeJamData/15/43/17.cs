...
namespace CodeJam
{
    ...
    class Bilingual : CodeJamProblem
    {
...
Case #4: 8";
        ...
        public override void Read(FileReader reader)
        {
            ...
            {
                string aux = "";
                var words = reader.ReadSpaceSeparatedStrings();
                sentences[i] = new int[words.Count()];
                int j = 0;
                foreach (string word in words)
                {
                    int index;
                    if (!map.TryGetValue ( word , out index))
                    {
                        map.Add(word, index = currentIndex);
                        currentIndex++;
                    }
                    sentences[i][j] = index;
                    j++;
        ...
        private int Count(int combination, bool[] isfrench, bool[] isenglish, bool[] originalisCounted)
        {
            bool[] counted = new bool[totalWords];
            bool[] newisFrench = new bool[totalWords];
            bool[] newisEnglish = new bool[totalWords];
            int count = 0;
           
            for ( int i = 2 ; i < sentences.Length  ; i++)
            {
                bool isEnglish = BitIsSet ( combination , i-2);
                if ( isEnglish)
                {
                    foreach ( var j in sentences[i])
                    {
                        if ((isfrench[j] || newisFrench[j]) && !counted[j] && !originalisCounted[j])
                        {
                            counted[j] = true;
                            count++;
                        }
                ...
                else
                {
                    foreach (var j in sentences[i])
                    {
                        if ((isenglish[j] || newisEnglish[j]) && !counted[j] && !originalisCounted[j])
                        {
                            counted[j] = true;
                            count++;
                        }
        ...
        {
            bool[] isfrench= new bool[totalWords];
            bool[] isenglish= new bool[totalWords];

            bool[] originalisCounted = new bool[totalWords];
            for ( int i = 0; i <sentences[0].Length ; i++)
                isenglish [sentences[0][i]]= true;

            int count = 0;
            for (int i = 0; i < sentences[1].Length; i++)
            {
                isfrench[sentences[1][i]] = true;
                if (isenglish[sentences[1][i]])
                {
                    count++;
                    originalisCounted[sentences[1][i]] = true;
                }
            }

            
            int max = (int)Math.Pow(2, sentences.Length-2);
            int mincount = int.MaxValue;
            for (int i = 0; i < max; i++ )
            {
                var aux = Count(i, isfrench, isenglish, originalisCounted);
                mincount = Min(mincount, count + aux);
            }
