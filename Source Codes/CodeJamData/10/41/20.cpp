...
//	freopen("A-small-attempt2.in","r",stdin);freopen("A-small-attempt2.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int i=-size;i<=size;i++)
			for (int j=-(size-abs(i));j<=size-abs(i);j+=2)
			{
				int value;
				scanf("%d",&value);
				X[n]=i;
		...
		int R=10000000;
		for (int dx=-200;dx<=200;dx++)
			for (int dy=-200;dy<=200;dy++) if (G[dx+250][dy+250])
			{
				int m=0;
				checkmax(m,abs(size+dx)+abs(dy));
				checkmax(m,abs(-size+dx)+abs(dy));
				checkmax(m,abs(dx)+abs(size+dy));
				checkmax(m,abs(dx)+abs(-size+dy));
				int c=sqr(m+1)-n;
				if (c<R) R=c;
			}
