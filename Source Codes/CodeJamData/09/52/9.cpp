...
int m,n;
char line[10000000],str[10000000];
int C[maxn][26];
int A[4],B[4],E[4];
int FA[maxk],inv[MODE];
int R[maxk],D[maxn][4];
int PN[maxk];
int key;
int S0,S1;
pair<ipair,ipair> Q0[10000000];
pair<ipair,ipair> Q1[10000000];

void DFS(int set,int multiply,int depth)
{
	if (set==0)
	{
		for (int k=depth;k<=m;k++)
			R[k]=(R[k]+(multiply*PN[k-depth]%MODE*FA[k]%MODE*inv[FA[k-depth]]%MODE))%MODE;
		return;
	}
	int k=0;
	for (k=0;1;k++)
	...
	for (int set2=1;set2<two(4);set2++)
		if ((set&set2)==set2 && contain(set2,k))
		{
			for (int i=0;i<n;i++)
			{
				int G=multiply;
...
void solve(int cnt)
{
	S0=0;
	S1=0;
	Q0[S0++]=MP(MP(0,0),MP(0,1));
	for (int k=1;k<=m;k++)
	{
		S1=0;
		for (int cl=0;cl<S0;cl++)
		{
			int a=Q0[cl].first.first;
			int b=Q0[cl].first.second;
			int c=Q0[cl].second.first;
			int d=Q0[cl].second.second;
			for (int i=0;i<n;i++)
			{
				int a2=a,b2=b,c2=c;
				if (cnt>=1) a2+=D[i][0];
		...
		S0=0;
		for (int i=0;i<S1;i++)
			if (S0>0 && Q1[i].first==Q0[S0-1].first && Q1[i].second.first==Q0[S0-1].second.first)
			{
				Q0[S0-1].second.second+=Q1[i].second.second;
				if (Q0[S0-1].second.second>=MODE)
					Q0[S0-1].second.second-=MODE;
			}
			else
				Q0[S0++]=Q1[i];
		for (int i=0;i<S0;i++)
		{
			int multiply=1;
			int E[3];
			E[0]=Q0[i].first.first;
			E[1]=Q0[i].first.second;
			E[2]=Q0[i].second.first;
			int d=Q0[i].second.second;
			for (int p=0;p<cnt;p++)
				for (int q=0;q<B[p];q++)
...
//	freopen("B-large.in","r",stdin);freopen("B-large.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int k=1;k<=m;k++) printf(" %d",R[k]);
		printf("\n");
		fflush(stdout);
	}
	return 0;
