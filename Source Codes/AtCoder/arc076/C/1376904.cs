...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		int N, X, Y;
		Tuple<int, int, int, int>[] ps;
		void Read()
		{
			sc.Read(out X, out Y, out N);
			ps = sc.ReadManyLines<int, int, int, int>(N);
		}
		bool OnEdge(int a, int b) => a == 0 || b == 0 || a == X || b == Y;
		void SolveOne()
		{
			pr.WriteLine(YesNo() ? "YES" : "NO");
		}
		struct P
		{
			public int X, Y, I;
			public P(int x, int y, int i) { X = x; Y = y; I = i; }
		}
		bool YesNo()
		{
			var edge = new List<int>();
			for (var i = 0; i < N; i++) if (OnEdge(ps[i].Item1, ps[i].Item2) && OnEdge(ps[i].Item3, ps[i].Item4)) edge.Add(i);
			if (edge.Count < 2) return true;
			var left = new List<P>();
			var right = new List<P>();
			var top = new List<P>();
			var bottom = new List<P>();
			foreach (var i in edge)
			{
				var p = new P(ps[i].Item1, ps[i].Item2, i);
				if (p.X == 0) left.Add(p);
				else if (p.X == X) right.Add(p);
				else if (p.Y == 0) bottom.Add(p);
				else top.Add(p);
				p = new P(ps[i].Item3, ps[i].Item4, i);
				if (p.X == 0) left.Add(p);
				else if (p.X == X) right.Add(p);
				else if (p.Y == 0) bottom.Add(p);
				else top.Add(p);
			...
			right.Sort((a, b) => b.Y.CompareTo(a.Y));
			top.Sort((a, b) => a.X.CompareTo(b.X));
			bottom.Sort((a, b) => b.X.CompareTo(a.X));
			var s = new List<int>();
			foreach (var p in left) s.Add(p.I);
			foreach (var p in top) s.Add(p.I);
			foreach (var p in right) s.Add(p.I);
			foreach (var p in bottom) s.Add(p.I);
			...
			foreach (var n in s)
			{
				...
				else if (stack.Count == 0 || stack.Pop() != n) return false;
			}
			return true;
		}
