        ...
        class Solver {
            public List<string> SolveAll(int threadId, StreamReader sr) {
                ...
                for (int testCaseIx = 0; testCaseIx < TestCaseCount; testCaseIx++) {
                    ...
                    int N = cs[0];
                    int K = cs[1];
                    decimal U = decimal.Parse(SplitToStrings(sr.ReadLine())[0]);
                    List<decimal> ps = SplitToStrings(sr.ReadLine()).Select(s => decimal.Parse(s)).ToList();
                    // ------------ 読み込み ------------
                    #region
                    ...
                    #endregion
                    // ------------ 呼び出し ------------
                    string output = Solve(testCaseIx, N, K, U, ps);
                    // ------------ 呼び出し ------------
                    outputs.Add(output);
                }
                return outputs;
            }
            string Solve(int testCaseIx, int N, int K, decimal U, List<decimal> PS) {
                List<decimal> ps = PS.ToList();
                ps.Sort();
                decimal u = U;
                if (ps.Count > 1) {
                    while (u > 0.0m) {
                        int count = 1;
                        decimal next = 1000000;
                        ...
                        decimal maxcharge = next - ps[0];
                        if (u > maxcharge * count) {
