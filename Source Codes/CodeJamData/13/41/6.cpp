...
void solve() {
  ...
  rep (i, evt.size()) {
    ...
    rep (j, tck.size()) {
      ll dx = x - prv_x;
      ll c = (N - tck[j].first) * dx - dx * (dx - 1) / 2;
      c %= MOD;
      // printf("%lld->%lld: %lld * %lld\n", prv_x, x, c, tck[j].second);
      (ans -= c * tck[j].second) %= MOD;

      tck[j].first += dx;
    }

    if (evt[i].second < 0) {
      tck.pb(mp(0, -evt[i].second));
    } else {
      ll n = evt[i].second;
      for (int k = int(tck.size()) - 1; k >= 0; --k) {
        ll t = min(n, tck[k].second);
        tck[k].second -= t;
        n -= t;
      }
    }
