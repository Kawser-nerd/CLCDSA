...
namespace AtCoderTemplate {
    class Program {
        static void Main (string[] args) {
            var AB = ReadInts ();
            PrintIf (Enumerable.Range (1, 3).Any (C => AB[0] * AB[1] * C % 2 == 1), "Yes", "No");
        }
    }
    ...
    static class MyExtensions {
        ...
        public static IEnumerable<IEnumerable<T>> Perm<T> (this IEnumerable<T> source, int n) {
            if (n == 0 || source.IsEmpty () || source.Count () < n) {
                ...
                return nexts.SelectMany (next => Perm (next.next, n - 1).Select (item => item.Concat (new List<T> { next.selected })));
            }
        }
