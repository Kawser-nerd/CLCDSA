...
signed main(){
    ...
    in(N);
    rep(i,N+1) grid[i].resize(N+1);
    rep(j,N) rep(i,N) scanf("%lld",&grid[i+1][j+1]);

    rep(y,N+1){
        REP(x,1,N+1) grid[x][y]+=grid[x-1][y];
    }
    REP(y,1,N+1){
        rep(x,N+1) grid[x][y]+=grid[x][y-1];
    }
    ...
    rep(sy,N){
        rep(sx,N){
            REP(y,sy+1,N+1){
                REP(x,sx+1,N+1){
                    int sz=(y-sy)*(x-sx);
                    table[sz]=max(table[sz],grid[x][y]-grid[x][sy]-grid[sx][y]+grid[sx][sy]);
                }
            }
        }
    }
