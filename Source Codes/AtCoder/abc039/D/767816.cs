...
class Program
{
    ...
    {
        Func<int, int, bool> wkFunc = (pCurrX, pCurrY) =>
        {
            if (pCurrX < 0 || UB_X < pCurrX) return true;
            if (pCurrY < 0 || UB_Y < pCurrY) return true;
            return pBanArr[pCurrX, pCurrY] == '#';
        };

        if (wkFunc(pX - 1, pY - 1) == false) return false;
        if (wkFunc(pX - 1, pY) == false) return false;
        if (wkFunc(pX - 1, pY + 1) == false) return false;
        if (wkFunc(pX, pY - 1) == false) return false;
        if (wkFunc(pX, pY) == false) return false;
        if (wkFunc(pX, pY + 1) == false) return false;
        if (wkFunc(pX + 1, pY - 1) == false) return false;
        if (wkFunc(pX + 1, pY) == false) return false;
        if (wkFunc(pX + 1, pY + 1) == false) return false;
    ...
    static bool Is8KinbouAnyBlack(char[,] pBanArr, int pX, int pY)
    {
        Func<int, int, bool> wkFunc = (pCurrX, pCurrY) =>
        {
            if (pCurrX < 0 || UB_X < pCurrX) return false;
            if (pCurrY < 0 || UB_Y < pCurrY) return false;
            return pBanArr[pCurrX, pCurrY] == '#';
        };

        if (wkFunc(pX - 1, pY - 1)) return true;
        if (wkFunc(pX - 1, pY)) return true;
        if (wkFunc(pX - 1, pY + 1)) return true;
        if (wkFunc(pX, pY - 1)) return true;
        if (wkFunc(pX, pY)) return true;
        if (wkFunc(pX, pY + 1)) return true;
        if (wkFunc(pX + 1, pY - 1)) return true;
        if (wkFunc(pX + 1, pY)) return true;
        if (wkFunc(pX + 1, pY + 1)) return true;
