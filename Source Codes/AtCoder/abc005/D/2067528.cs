...
namespace ConsoleApplication {
    class MainClass {
        ...
        void f() {
            memo = D.Clone() as int[,];
            for(int i = 1; i <= N; ++i) memo[i, 0] += memo[i - 1, 0];
            for(int i = 1; i <= N; ++i) memo[0, i] += memo[0, i - 1];
            for(int i = 1; i <= N; ++i) 
            for(int j = 1; j <= N; ++j) 
            ...
            for(int b = 0; b < u; ++b)
            for(int l = 0; l < r; ++l)
            res[(u - b) * (r - l)] = Math.Max(res[(u - b) * (r - l)], 
                                              memo[u, r] - memo[b, r] - memo[u, l] + memo[b, l]);
            for(int i = 1; i <= N * N; ++i) res[i] = Math.Max(res[i], res[i - 1]);
        }
    ...
    class Mat {
        ...
        public long C(long n, long r) {
            ...
            if(n < 0 || r < 0 || n < r)
                ...
                return (Fact(n) % mod * Pow((Fact(n - r) % mod * Fact(r) % mod) % mod, mod - 2) % mod) % mod;
        }
