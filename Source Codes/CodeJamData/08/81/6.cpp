...
void solveCase(int cnum) {
  ...
  cld a1 = cld(t0[0], t0[1]);
  cld a2 = cld(t0[2], t0[3]);
  cld a3 = cld(t0[4], t0[5]);
  
  cld b1 = cld(t1[0], t1[1]);
  cld b2 = cld(t1[2], t1[3]);
  cld b3 = cld(t1[4], t1[5]);
  
  cld Ac = (b1-b2) / (a1-a2);
  cld Bc = b1 - a1 * Ac;
  
  cld point = Bc / (cld(1,0) - Ac);

  int res = 0;
  
  printf("Case #%d: %lf %lf\n", cnum+1, double(point.real()), double(point.imag()));
  }
