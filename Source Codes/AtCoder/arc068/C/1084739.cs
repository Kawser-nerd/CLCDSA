...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			var seg = new RangeSegmentTree(N + 1);
			Array.Sort(qs, (_x, _y) => (_x.Item2 - _x.Item1).CompareTo(_y.Item2 - _y.Item1));
			var x = 0;
			for (var d = 1; d <= N; d++)
			{
				var ans = qs.Length - x;
				for (var i = 0; i <= N; i += d) ans += seg[i];
				pr.WriteLine(ans);
				while (x < qs.Length && qs[x].Item2 - qs[x].Item1 + 1 == d) { seg.AddRange(qs[x].Item1, qs[x].Item2 + 1, 1); x++; }
			}
		}
