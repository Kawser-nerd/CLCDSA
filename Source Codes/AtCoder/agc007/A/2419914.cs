...
public class Hello
{
    ...
    public static bool   goDfs (bool[,] map)
    {
        ...
        var st = new Stack<P>();
        st.Push(new P { x = 0, y = 0, via = "" });
        while (st.Count() > 0)
        {
            var t = st.Pop();
            if (t.x == h - 1 && t.y == w - 1 && checkVia(map, t.via)) return true;
            if (t.y + 1 < w && map[t.x, t.y +1] )  st.Push(new P { x = t.x, y = t.y + 1, via = t.via + "r" });
            if (t.x + 1 < h && map[t.x+1, t.y ]) st.Push(new P { x = t.x+1, y = t.y , via = t.via + "d" });
        }
        return false;   
    }
