...
def solve(input_set):
    M, N, seats = input_set
    rows = make_array(M)
    trans_tab = maketrans('.x', '01')
    for i in xrange(M):
        rows[i] = int(seats[i].translate(trans_tab), 2)
    masks = []
    for mask in xrange(1 << N):
        if (mask & (mask << 1)) == 0 and (mask & (mask >> 1)) == 0:
            masks.append(mask)
    num = make_array((M, 1 << N), 0)
    for mask in masks:
        if (mask & rows[0]) == 0:
            num[0][mask] = ones(mask)
    for i in xrange(1, M):
        for last_mask in masks:
            for mask in masks:
                if (mask & (last_mask << 1)) == 0 and (mask & (last_mask >> 1)) == 0 and (mask & rows[i]) == 0:
                    num[i][mask] = max(num[i][mask], num[i - 1][last_mask] + ones(mask))
    return max(num[M - 1])
