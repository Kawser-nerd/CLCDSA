...
./Main.c:1:79: warning: implicit declaration of function ‘printf’ [-Wimplicit-function-declaration]
 long x,s,t;main(n){for(scanf("%d%ld",&n,&x);x;x-=x>t-2?s+=t/2,t-1:1)t=2L<<n--;printf("%ld",s);}
                                                                               ^
./Main.c:1:79: warning: incompatible implicit declaration of built-in functio...