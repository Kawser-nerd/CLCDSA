...
namespace Program
{
    public class Solver
    {
        ...
        public void Solve()
        {
            ...
            var connect = new bool[n * (n - 1) / 2];
            foreach (var x in sc.Integer(n - 1))
                connect[x - 1] = true;
            const long MOD = (long)1e9 + 7;
            var E = n * (n - 1) / 2;
            var cmp = new Cmp();
            var dp = Enumerate(E, x => new SortedMap<int[], long>(cmp));
            {
                var ini = new int[31];
                ini[1] = n;
                dp[0][ini] = 1;
            ...
            for (int i = 0; i < E; i++)
            {
                var next = Enumerate(E, x => new SortedMap<int[], long>(cmp));
                if (!connect[i])
                {
                    for (int k = 1; k < E; k++)
                        foreach (var x in dp[k])
                        {
                            //Console.WriteLine("{0},{2}: {1}", k, a.AsJoinedString(), x.Value);
                            next[k - 1][x.Key] = (next[k - 1][x.Key] + k * x.Value) % MOD;
                        }
                }
                else
                {
                    for (int k = 0; k < E; k++)
                        foreach (var x in dp[k])
                        {
                            ...
                            for (int p = 1; p <= n; p++)
                            {
                                if (a[p] >= 2)
                                {
                                    var c = (a[p] * (a[p] - 1) / 2) * p * p;
                                    a[p] -= 2;
                                    a[p + p]++;
                                    var b = a.ToArray();
                                    next[k + p * p - 1][b] += c * x.Value;
                                    next[k + p * p - 1][b] %= MOD;
                                    a[p + p]--;
                                    a[p] += 2;
                                }
