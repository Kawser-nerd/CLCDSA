...
public class Main {
    ...
    public static void solve() throws Exception {
        ...
        st.set(Calendar.YEAR, 2012);
        st.set(Calendar.MONTH, 0);
        st.set(Calendar.DATE, 1);
        now.set(Calendar.YEAR, 2012);
        boolean[] hol = new boolean[367];//??
        for (int i = 0; i < 366; i++) {
            if (i % 7 == 0 || i % 7 == 6) hol[i] = true;
        }
        for (int i = 0; i < N; i++) {
            int mon = Integer.parseInt(s[i].split("/")[0]) - 1;
            int date = Integer.parseInt(s[i].split("/")[1]);
            now.set(Calendar.MONTH, mon);
            now.set(Calendar.DATE, date);
            int diff = getDiffDays(now, st);
            for (int j = diff; j < 366; j++) {
                if (!hol[j]) {
                    hol[j] = true;
                    break;
                }
