...
public class Main {
    ...
    public static void main(String[] args){
        ...
        int[] conK=new int[n],conS=new int[n];// migi ni renzoku de nanko
        for(int i=n-1;i>=0;i--){
            if(i==(n-1)){
                conK[i]=0;
                conS[i]=0;
        ...
        long[] dp=new long[n];
        long[] dpK=new long[n],dpS=new long[n];
        for(int i=n-1;i>=0;i--){
            if(i==(n-1)){
                dpK[i]=1;
                dpS[i]=1;
            ...
            long targetK=s[i]+a;
            int indK=lastLowerIndex(s,targetK);
            if(indK==i){
                dpK[i]=dp[i+1];
            }else{
                if(conS[i+1]<(indK-i-1)){//siro renzoku ga kanou ka douka
                    dpK[i]=0;
                }else{
                    dpK[i]=dpS[indK];
                }
            ...
            long targetS=s[i]+b;
            int indS=lastLowerIndex(s,targetS);
            if(indS==i){
                dpS[i]=dp[i+1];
            }else{
                if(conK[i+1]<(indS-i-1)){//kuro renzoku ga kanou ka douka
                    dpS[i]=0;
                }else{
                    dpS[i]=dpK[indS];
                }
            ...
            dp[i]=(dpS[i]+dpK[i])%mod;
        }
        pl(dp[0]);
        flush();
        //try{Thread.sleep(10);}catch(Exception e){}
    }
    static long brute(int n,long a,long b,long[] s){
        long[] dp=new long[n];
        for(int i=n-1;i>=0;i--){
            long pk=Long.MIN_VALUE,ps=pk;
            pk=s[i];
            int j=i+1;
            for(;j<n;j++){
                if((s[j]>=(pk+a)) && (s[j]>=(ps+b))){
                    break;
                }else if((s[j]>=(pk+a))){
                    pk=s[j];
                }else if((s[j]>=(ps+b))){
