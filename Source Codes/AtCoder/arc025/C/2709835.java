...
public class Main {
	private static void solve() {
		...
		for (int i = 0; i < m; i++) {
			...
			int b = abc[i][1] - 1;
			int c = abc[i][2];
			edges[a][numEdges[a]][0] = b;
			edges[a][numEdges[a]][1] = c;
			edges[b][numEdges[b]][0] = a;
			edges[b][numEdges[b]][1] = c;
			numEdges[a]++;
			numEdges[b]++;
		}
		...
		for (int target = 0; target < n; target++) {
			for (int i = 0; i < n; i++) {
				costs[i][0] = i;
				costs[i][1] = i == target ? 0 : Long.MAX_VALUE / 250;
				visited[i] = false;
			}
			PriorityQueue<long[]> q = new PriorityQueue<long[]>((a, b) -> (Long.compare(a[1], b[1])));
			...
			while (!q.isEmpty()) {
				...
				long cost = po[1];
				visited[idx] = true;
				for (int i = 0; i < numEdges[idx]; i++) {
					int to = edges[idx][i][0];
					int ec = edges[idx][i][1];
					if (!visited[edges[idx][i][0]] && costs[to][1] > cost + ec) {
						q.remove(costs[to]);
						costs[to][1] = cost + ec;
						q.add(costs[to]);
					}
			...
			for (int i = 0; i < n; i++) {
				sortedCosts[i] = costs[i][1] == 0 ? Long.MAX_VALUE / 250 : costs[i][1];
			}
			Arrays.sort(sortedCosts);
			int kame = 0;
			int usagi = 0;
			while (kame < n - 1) {
				while (sortedCosts[usagi] * t <= sortedCosts[kame] * r) {
					usagi++;
				}
				ans += n - 1 - usagi - (kame >= usagi ? 1 : 0);
				kame++;
			}
		}
		out(ans);
