...
class Program
{
    ...
    static void Main()
    {
        ...
        var sc = new Scan();
        sc.Multi(out r, out b);
        sc.Multi(out x, out y);
        long le = 0, ri = Math.Min(r / x, b);
        long ma = Math.Max(num(le), num(ri));
        int c = 0;
        while (le < ri && c < 100000)
        {
            ++c;
            long ml = (le * 2 + ri) / 3, mr = (le + ri * 2) / 3;
            if (num(ml) < num(mr))
                le = ml;
            else if (num(ml) > num(mr))
                ri = mr;
            else
            {
                ma = Math.Max(num(le), num(ri));
                ++le;
                --ri;
            }
        }
    ...
    static long num(long red)
    {
        return red + Math.Min(r - red * x, (b - red) / y);
    }
