...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		void SolveOne()
		{
			...
			var s = 0;
			for (var x = 0; x < X; x++) for (var y = 0; y < Y; y++) if (map[x, y]) s++;
			if (jx.Count > 0 && jy.Count > 0) pr.WriteLine(1);
			else if (jx.Count == 0 && jy.Count == 0) pr.WriteLine(Func.Pow(s, K - 1, Mod));
			else
			{
				var t = jy.Count;
				if (jy.Count == 0)
				...
				var u = 0;
				for (var x = 0; x < X - 1; x++) for (var y = 0; y < Y; y++) if (map[x, y] && map[x + 1, y]) u++;
				var mat = new Matrix(2, 2);
				mat[0, 0] = s;
				mat[1, 0] = u;
				mat[1, 1] = t;
				mat = mat.Pow((ulong)(K - 1), Mod);
				var a = (mat[0, 0] - mat[1, 0]) % Mod;
				if (a < 0) a += Mod;
				pr.WriteLine(a);
			}
		}
