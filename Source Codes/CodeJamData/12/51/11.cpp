...
inline bool oless( int i, int j )
{
  #define F(i, j) (t[i] / p[i] / p[j] + t[j] / p[j])
  double x = F(i, j) - F(j, i);
  return x < -eps || (x < eps && i < j);
}
...
int main()
{
  ...
  forn(tt, tn)
  {
    ...
    forn(i, n)
      scanf("%d", &t[i]), o[i] = i;
    forn(i, n)
      scanf("%lf", &p[i]), p[i] *= 1e-2, p[i] = 1 - p[i];
    sort(o, o + n, oless);
    forn(i, n)
      printf("%d%c", o[i], EOL(i, n));
  }
