...
namespace Program
{
    public class Solver
    {
        public void Solve()
        {
            ...
            //greedy matching
            {
                ...
                for (int i = n - 1; i >= 0; i--)
                {
                    ...
                    if (p < s.Count)
                    {
                        ...
                        var tt = ys.BinarySearch(a[i]);
                        if (tt < 0) tt = ~tt;
                        var ttt = ys.BinarySearch(b[i]);
                        if (ttt < 0) ttt = ~ttt;
                        s.RemoveAt(p);
                        G[i].Add(new P(n + tt, 1));
                        G[n + tt].Add(new P(n + t, 0));
                        G[n + ttt].Add(new P(i, 0));
                    }
                    else
                    {
                        p = s.LowerBound(b[i]);
            ...
            //for (int i = 0; i < G.Length; i++) foreach (var to in G[i]) IO.Printer.Out.WriteLine("{0} {1} {2}", i, to.Key, to.Value);
            //return;
            //augment
            {
                while (deq.Count > 0)
